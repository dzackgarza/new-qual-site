...
– How are the derived and lower/upper central series defined? What $\mathrm { t y p e ( s ) }$ of the groups above does each series correspond to?
...
• How do you compute the totient $\varphi ( p )$ for p prime? Or $\varphi ( n )$ for n composite?
...
Let G be a finite group with n distinct conjugacy classes. Let $g _ { 1 } \cdots g _ { n }$ be representatives of the conjugacy classes of G. Prove that if $g _ { i } g _ { j } = g _ { j } g _ { i }$ for all $i , j$ then G is abelian.
...
b. Suppose that G is abelian. Prove that $| G | = | A |$ . Deduce that every abelian transitive subgroup of $S _ { n }$ has order n.
