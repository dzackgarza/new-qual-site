...
Let $\phi : G L ( 2 , \mathbb { R } ) \to \mathbb { R } ^ { * }$ be defined by $A \mapsto d e t ( A )$ . Let $A \in G L ( 2 , \mathbb { R } )$ This means that A is invertible thus the $d e t ( A )$ is not zero, hence the $d e t ( A )$ is in R∗. So φ maps to $\mathbb { R } ^ { * }$ as claimed. Now let $A , B \in G L ( 2 , \mathbb { R } )$ . Then $\phi ( A B ) = d e t ( A B ) = d e t ( A ) d e t ( B ) = \phi ( A ) \phi ( B )$ , so $\phi$ is a homomorphism.

## # 2: Prove that the mapping given in Example 3 is a homomorphism.

Let $\phi : \mathbb { R } ^ { * }  \mathbb { R } ^ { * }$ be defined by $x \mapsto | x |$ . Then $\phi ( x y ) = | x y | = | x | | y | = \phi ( x ) \phi ( y )$ so $\phi$ is a homomorphism.

## # 3: Prove that the mapping given in Example 4 is a homomorphism.

Let $\phi : \mathbb { R } [ x ]  \mathbb { R } [ x ]$ be defined by $f \mapsto f ^ { \prime }$ . Then for $f , g \in \mathbb { R } [ x ] , \phi ( f + g ) = ( f + g ) ^ { \prime } =$ $f ^ { \prime } + g ^ { \prime } = \phi ( f ) + \phi ( g )$ so φ is a homomorphism.
...
Let $\phi : \mathbb { R } [ x ]  \mathbb { R } [ x ]$ be defined by $f \mapsto \int f$ Then $\phi ( f + g ) = \textstyle \int ( f + g ) + c $ where $c = - ( f + g ) ( 0 )$ and $\textstyle \int ( f + g )$ is the polynomial that is the antiderivative without a constant term. Now, $\begin{array} { r } { \phi ( f ) + \bar { \phi } ( g ) = \int f + \int g + c _ { 1 } + c _ { 2 } } \end{array}$ where $c _ { 1 } = - f ( 0 )$ and $c _ { 2 } = - g ( 0 )$ . Hence $\phi ( f + g ) = \phi ( f ) + \phi ( g )$ for all $f , g \in \mathbb { R } [ x ]$ so $\phi$ is a homomorphism.

Now, the kernel of $\phi$ are the set of things that map to the identity, 0. So $\begin{array} { r } { K e r \phi = \left\{ f \vert \int f = \right. } \end{array}$ $\begin{array} { r } { 0 \} = \{ a _ { 0 } + a _ { 1 } x + \dotsc + a _ { n } x ^ { n } \vert 0 + a _ { 0 } * x + \frac { a _ { 1 } } { 2 } x ^ { 2 } + \dotsc + \frac { a _ { n } } { n + 1 } x ^ { n + 1 } = 0 \} = \{ a _ { 0 } = a _ { 1 } = \dotsc = a _ { n } = 0 \} . } \end{array}$ $0 \} = \{ 0 \}$
...
Let $\phi : G \to H$ be a homomorphism and $\sigma : H  K$ also be a group homomorphism. Then $\sigma \phi : G \to K$ and $\sigma \phi ( x y ) = \sigma ( \phi ( x ) \phi ( y ) ) = \sigma ( \phi ( x ) ) \sigma ( \phi ( y ) ) = \sigma \phi ( x ) \sigma \phi ( y )$ so the composition is a homomorphism.
...
• φ(ri ◦ rj f ) = φ(ri+j f ) = −1 = 1 · −1 = φ(ri) · φ(rj f )

$$
\bullet \ \phi ( r ^ { i } f \circ r ^ { j } ) = \phi ( r ^ { i } r ^ { n - j } f ) = \phi ( r ^ { i + n - j } f ) = - 1 = - 1 \cdot 1 = \phi ( r ^ { i } f ) \cdot \phi ( r ^ { j } )
$$

Kerφ = {gφ(g) = 1} = { rotations in G} =< r >

## # 14: Explain why the correspondence x 7→ 3x from $\mathbb { Z } _ { 1 2 }$ to $\mathbb { Z } _ { 1 0 }$ is not a homomorphism.

If the correspondence is a homomorphism, then it should preserve the operation. Let’s show this is not true bia a counter example. Take $6 , 7 \in \mathbb { Z } _ { 1 2 }$ Then $\phi ( 6 + 7 ) = \phi ( 1 ) = 3$ . But $\phi ( 6 ) + \phi ( 7 ) = 1 8 + 2 1 = 8 + 1 = 9$ . Since $3 \neq 9$ in $\mathbb { Z } _ { 1 0 }$ the operation is not preserved.