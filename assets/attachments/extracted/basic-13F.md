...
(b) Show there exists continuous nondecreasing f(t) on [0, 1] such that $f ( 0 ) = 0$ and $f ( 1 ) = 1$ and such that $L ( \gamma ) = 2$ when $\gamma ( t ) = ( t , f ( t ) )$ ,
...
5. A function $f : \mathbb { R } ^ { d } \to \mathbb { R }$ is said to be convex if

$$
f ( t x + ( 1 - t ) y ) \leq t f ( x ) + ( 1 - t ) f ( y ) , \quad \text { for all } x , y \in \mathbb { R } ^ { d } , 0 \leq t \leq 1 .
$$
...
10. Let A be an n by n Hermitian matrix and let, for $j \in [ 1 , n ] , A _ { j }$ be the submatrix consisting of the entries of A in the first j rows and columns of A. Suppose that $d e t ( A _ { j } ) \neq 0$ for all j and $d e t ( A _ { 1 } ) > 0$ Give and prove a rule in terms of the signs of the $d e t ( A _ { j } )$ to determine the signature of the Hermitian form defined by A.
