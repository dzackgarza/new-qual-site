...
**All 59 items are stale.** They reference old wiki paths (`wiki/Algebra/`, `wiki/Complex_Analysis/`, etc.) that no longer exist.
The wiki was restructured to lowercase (`wiki/algebra/`, `wiki/complex-analysis/`, etc.).
...
`100001` is the marker an importer left.
...
True about the filesystem.
What to do about each is a reading, so none carries a checkbox.

### one-markdown-child-directories (15)

Folders holding one page besides the index.
A chapter with one section written reads the same from the filesystem as a page in the wrong place.
...
Pages whose every line is a heading or a wikilink.
These retype card data; TODO.md section 11 owns them.
