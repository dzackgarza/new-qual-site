...
| --- | --- | --- |
| `attachments/Pasted image 20220914124903.png` | source-repair | The coordinate-ring-domain question in `0_Study Guides/021 Varieties Problems.html` contains this image reference inside a solution block, but the deployed tree has no such file. Recover the solution image or its native source. `P-AGXVARCOORDDOMAIN` contains the question without that solution. |
| `attachments/Pasted image 20220314125429.png`, `attachments/Pasted image 20220213162548.png`, `attachments/Pasted image 20220208145954.png` | source-repair | The historical broken-link diagnostic names these in Schemes/Cohomology. They are absent from the deployed inventory. Their mathematical payload is unknown; the diagnostic cannot establish that a particular theorem or proof is missing. Check the original Markdown/source history. |
