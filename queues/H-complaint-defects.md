...
Every entry below is an observed mathematical or transcription defect in an authored card,
recorded in `COMPLAINTS.md` and never tracked anywhere a worker selects work from. They are
listed here so each one can be taken, repaired against the source, and closed like any other
unit. Read the full entry in `COMPLAINTS.md` before touching a card — the heading is an index,
not the statement of the defect.

A card leaves this list by being repaired against its source, in a commit that also updates the
`COMPLAINTS.md` entry. Ticking a box here without a card changing is the failure this queue
