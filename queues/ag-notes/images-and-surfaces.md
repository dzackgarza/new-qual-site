...
| `Pasted image 20220921204101.png` | migrated | `T-TORDIV` has $K = -\sum D_\rho$ with the proof by the invariant logarithmic top form, the chart $\AA^1 \times \GG_m^{n-1}$ and the simple pole along each boundary divisor, and the projective-space specialization. |
