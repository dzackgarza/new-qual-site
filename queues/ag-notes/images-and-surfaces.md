...
| `Hartshorne/5_Hartshorne/5_2x.html` | migrated | V.2.1–17 statements, subparts and footnotes -> `P-AGH521BASEUNIQUE` through the source-ordered V.2 entries ending `P-AGH5217CONORMALSPLIT`, including stability, characteristic-p examples, all ruled-surface inequalities and the characteristic-dependent conormal answer. The source star on V.2.17 is the appearance comment `V.2.17*` in `corpus/collections/SRC-TEXT-HART77/index.md` (ae11b8bc3). |
...
| `Hartshorne/5_Hartshorne/5_4x.html` | migrated | V.4.1–16 text and hints -> `P-AGH541CONICSQUADRIC` through `P-AGH5416FERMATCUBIC` in source order. The Pascal figure (Fig. 22) is embedded on `P-AGH545PASCAL` and the chain and E6 diagrams are tikzcd blocks on `P-AGH5411WEYLGROUPS` (53d939241). Stars on V.4.8, V.4.11(c), V.4.15(e) are the appearance comments `V.4.8*`, `V.4.11, part (c)*`, `V.4.15, part (e)*` (ae11b8bc3). |
| `Hartshorne/5_Hartshorne/5_5x.html` | migrated | V.5.1–8 statements and hints -> `P-AGH551RESOLVERATFN` through `P-AGH558E8SINGULARITY`; the E8 diagram is a tikzcd block on `P-AGH558E8SINGULARITY` (53d939241). The V.5.5 hint's parenthetical "(Fig. 23)" points to a textbook figure that no source copy contains (no image in `5_Hartshorne/figures/` of the deployed tree, the orals vault `Hartshorne_Problems/5_Hartshorne/figures/`, or `/var/www/Notes/orals/figures/` on the source host); the hint's statement $\widetilde{D}^2=D^2-1$ is complete without it, so only the dangling figure reference is not carried. |
...
| `Hartshorne/5_Hartshorne/figures/2022-10-22_21-13-10.png` | migrated | Pascal configuration (six conic points, intersection points P, Q, R and their line) -> `assets/algebraic-geometry/curves-and-surfaces/pascal-theorem-six-points-on-conic.png`, embedded in `P-AGH545PASCAL` (53d939241). |
| `Hartshorne/5_Hartshorne/figures/2022-10-22_21-16-28.png` | migrated | Chain diagram for V.4.11(a) -> tikzcd block ($n-1$ vertices, consecutive vertices joined) in part (a) of `P-AGH5411WEYLGROUPS` (53d939241). |
| `Hartshorne/5_Hartshorne/figures/2022-10-22_21-17-05.png` | migrated | E6 graph (five-vertex chain, one branch at the middle vertex) -> tikzcd block in part (b) of `P-AGH5411WEYLGROUPS` (53d939241). |
| `Hartshorne/5_Hartshorne/figures/2022-10-22_21-57-34.png` | migrated | E8 graph (seven-vertex chain, branch at the third vertex from one end) -> tikzcd block at the end of part (b) of `P-AGH558E8SINGULARITY` (53d939241). |
...
| `Pasted image 20220315153140.png`; `Pasted image 20220315153154.png` | migrated | `FE-SHFISOSTALKS` retains the double-cover example with coefficients $\ZZ$, shows its pushforward is locally constant but not constant, and gives the open-inclusion example of a pushforward that is not locally constant. |
| `Pasted image 20220921204101.png` | migrated | `T-TORDIV` states $K = -\sum D_\rho$ and its projective-space specialization. Per `QUAL-10` the crop's proof sketch (the invariant logarithmic top form with a simple pole along each boundary divisor) is not carried for this standard result. |
...
| `2022-01-09_12-23-55.png` | migrated | `T-MORFIBDIM` states affine Noether normalization by a linear map over an infinite field; its remark records that over $\FF_2$ no linear map is finite on the cubic $xy(x+y)+1$. |
| `2022-01-09_12-24-40.png` | migrated | (a) `T-MORFIBDIM` gives projective Noether normalization by linear projection, now over an infinite field. (b) The source's lift of maps $X \to Y$ with $Y$ normal is replaced on `D-QJ5M9` by the universal property: every dominant morphism from a normal integral scheme to $X$ factors uniquely through $\nu \colon \widetilde{X} \to X$, with existence, the affine integral-closure construction, and finiteness and birationality for varieties, cited to Hartshorne II Ex. 3.8. |
...
| `2022-01-09_12-59-50.png` | migrated | `T-SRFCAST` now states the source scope (smooth separated surface of finite type, proper integral curve $E$) and both directions of the contraction criterion, cited to Hartshorne V.5.7; the infinitely many $(-1)$-curves claim now requires 9 points in general position in the sense of `P-AGH5415GENPOSITION`. |
...
Every syllabus crop below is transcribed on `wiki/algebraic-geometry/syllabus.md`. A named topic does not create an obligation to invent a chapter during migration.
