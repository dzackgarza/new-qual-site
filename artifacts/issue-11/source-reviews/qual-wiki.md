...
The source repair commit restores the former seven unavailable figure paths and the remaining files from the previously tracked topology figure link.
The recovered source payloads are byte-identical to their permanent target assets.
The seven former blockers now have direct source-to-target SHA-1 evidence:
...
The corrected source revision is published at `dzackgarza/qual-wiki` `main`. No source item remains blocked in this boundary.
