...
def unsolved(parsed: list[ParsedCard]) -> list[ParsedCard]:
    ...
    """
    return [item for item in parsed if item.card.kind in ASKED and not any(kind == "solution" for kind, _text in item.sections)]
...
def render(cards: list[ParsedCard]) -> str:
    ...
    lines = [
        ...
        "",
        f"{len(cards)} problem and exercise cards have no solution section. These are",
