...
GITHUB_ICON = """<svg viewBox="0 0 16 16" aria-hidden="true" focusable="false">
        <path fill="currentColor" d="
          ...
          .4.08.55-.18.55-.39 0-.19-.01-.83-.01-1.51-2.01.38-2.53-.5-2.69-.96
          -.09-.23-.48-.96-.82-1.15-.28-.15-.68-.53-.01-.54.63-.01 1.08.59 1.23.83
          .72 1.23 1.87.88 2.33.67.07-.53.28-.88.51-1.08-1.78-.21-3.64-.9-3.64-4.02
          0-.89.31-1.62.82-2.19-.08-.21-.36-1.04.08-2.16 0 0 .67-.22 2.2.84
          A7.42 7.42 0 0 1 8 3.91c.68 0 1.36.09 2 .27 1.53-1.06 2.2-.84 2.2-.84
          .44 1.12.16 1.95.08 2.16.51.57.82 1.29.82 2.19 0 3.13-1.87 3.81-3.65 4.02
          .29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.47.55.39
          A8.1 8.1 0 0 0 16 8.13C16 3.64 12.42 0 8 0Z
        "/>
      </svg>"""
...
def _generated_at() -> datetime:
    """One UTC timestamp for the whole build process.

    `SOURCE_DATE_EPOCH` keeps reproducible builds reproducible when a caller
    supplies it; otherwise the first emitted page fixes the timestamp shared
    by every page generated in this process.
    """
...
def _site_footer() -> str:
    ...
    machine_time = generated.isoformat(timespec="seconds").replace("+00:00", "Z")
    human_time = generated.strftime("%Y-%m-%d %H:%M UTC")
    revision = os.environ.get("GITHUB_SHA")
    revision_html = ""
    if revision:
        short = escape(revision[:9])
        href = escape(f"{REPOSITORY_URL}/commit/{revision}", quote=True)
        revision_html = f' <span aria-hidden="true">·</span> <a href="{href}">Revision {short}</a>'
    return f"""<footer class="site-footer">
    <p><time datetime="{machine_time}">Generated {human_time}</time>{revision_html}</p>
    <a class="footer-repository" href="{REPOSITORY_URL}" aria-label="Source repository on GitHub">
      {GITHUB_ICON}
      <span>Source</span>
    </a>
  </footer>"""
            ...
            match link.target:
                ...
                case PageTarget():
                    ...
                    if children[link.key]:
                        # The folder's own page is the summary. Plain text
                        # keeps the disclosure triangle beside the label; an
                        # <a> inside <summary> shifts the triangle above it
