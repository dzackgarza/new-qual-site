...
whether a card can be reached by a reader, whether two collection cards claim
one exam. Those are the invariants this measures.
...
# provenance remark, and the same remark on two different exams is correct,
# not a duplicate. Duplicate *exams* are `duplicate-exams`.
...
def check_one_exam_one_source(parsed: list[ParsedCard]) -> Check:
    """An exam is institution + area + date. Two collection cards for one
    exam split every per-exam query in half."""
...
def orphan_ids(parsed: list[ParsedCard], wiki_pages: list[WikiPage], root: Path = REPO) -> set[str]:
    ...
    referenced: set[str] = {item.card.id for item in parsed if item.card.kind == "collection"} | set(_manifest_ids(root))
...
CHECKS = {
    ...
    "duplicate-exams": check_one_exam_one_source,
