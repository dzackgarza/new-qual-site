...
def _write_proof(path: Path, report: ReplayReport) -> None:
    ...
    lines.extend(
        [
            "",
            "The command cloned each source over SSH into a new temporary root, checked out the recorded commit, and verified a clean worktree. It compared every tracked path with the committed ledger, then verified migrated targets, source hashes, generated-source reasons, queued owners, dropped-source reasons, and the recorded G7 residual.",
            "",
