...
is a candidate, never an instruction to act.
    ...
    just provenance
    uv run python tools/provenance_hrefs.py
    uv run python tools/provenance_hrefs.py --json
    uv run python tools/provenance_hrefs.py --only empty-provenance
    uv run python tools/provenance_hrefs.py --only dead-provenance-hrefs
    uv run python tools/provenance_hrefs.py --only shared-provenance-hrefs
    uv run python tools/provenance_hrefs.py --only markdown-provenance-hrefs
    uv run python tools/provenance_hrefs.py --only image-provenance-hrefs
    uv run python tools/provenance_hrefs.py --only forbidden-provenance-hrefs
...
def check_dead_provenance_hrefs(
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        pending = {
            pool.submit(http_get_status, url, timeout=timeout): url for url in unique_urls
        }
