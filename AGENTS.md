...
- `queues/C-unsolved-cards.md` — the measurement. It lists every problem and
  exercise card with no `solution` section and no incoming `solves` relation.
...
- `queues/09-author-solutions.md` — the authored disposition (Queue 9,
  ...
  Lamport-style structured proof, add a `solution` section or an incoming
  `solves` relation, and commit before selecting the next card.
