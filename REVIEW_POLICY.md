...
| `CARD-05` | Asked-card semantics differ by storage kind | For problems and exercises, check whether the public statement/metadata treatment reflects that both are asked mathematical items rather than treating one as generic reference content. | [#70](https://github.com/dzackgarza/new-qual-site/issues/70) |
| `MODEL-01` | Detached solution object | Check whether a solution is being modeled as a standalone card/relation instead of semantic content on its owning problem/exercise. | [#65](https://github.com/dzackgarza/new-qual-site/issues/65) |
...
| `GUIDE-03` | Static problem bucket where practice discovery belongs in generator | Check whether a guide is baking a live topic search into a static list. Practice queries should preserve authored filters and deep-link to the generator, not become a second source of truth. | [#66](https://github.com/dzackgarza/new-qual-site/issues/66) |
