...
The authoritative rules for public copy are the named policies in [CONTRIBUTING.md](CONTRIBUTING.md) (`STANCE-*`, `PROSE-*`, `PR-*` and the rest of that catalogue); `COPY-*` below are reading prompts, not those rules.
Crawler reports filed before 2026-09-16 label `COPY-01`–`COPY-04` as `PROSE-01`–`PROSE-04`, which are unrelated to the `CONTRIBUTING.md` codes of the same name.
...
| `GUIDE-03` | Parallel metadata-selected problem-list/query surface outside the canonical browser | Check whether a guide or ordinary wiki page is materializing a metadata-selected problem list or carrying separate query/display configuration. Guide/wiki pages should own mathematical `topics:` metadata and get their `problems.html` link automatically. **Collection pages are different:** their purpose includes presenting the source's actual ordered problem contents, so they should author/materialize that source-order list directly, with source locators and section structure where present; a `problems.html?collection=...` link is supplementary navigation, not a replacement for the collection contents. | [#66](https://github.com/dzackgarza/new-qual-site/issues/66), [#63](https://github.com/dzackgarza/new-qual-site/issues/63) |
