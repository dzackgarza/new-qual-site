...
**Goal:** Let $n \ge 3$ be an integer, and let $S_n$ be the symmetric group on $n$ elements. Prove that the center $Z(S_n) = \{e\}$, where $e$ is the identity permutation.

<1>1. Definition of the center $Z(S_n)$:
    $Z(S_n) = \{\sigma \in S_n \mid \sigma \tau = \tau \sigma \text{ for all } \tau \in S_n\}$.
...
<1>3. If $\sigma \in S_n$ and $\sigma \neq e$, then $\sigma \notin Z(S_n)$.
    ...
    <2>4. Evaluate the compositions $(\sigma \circ \tau)$ and $(\tau \circ \sigma)$ at the element $i$:
        <3>1. $(\tau \circ \sigma)(i) = \tau(\sigma(i)) = \tau(j) = k$.
            Proof: By definition of $\sigma(i) = j$ and $\tau = (j\,k)$ sending $j \mapsto k$.
        <3>2. $(\sigma \circ \tau)(i) = \sigma(\tau(i)) = \sigma(i) = j$.
            Proof: Since $i \notin \{j, k\}$, $\tau(i) = i$. Then $\sigma(i) = j$.
        <3>3. $(\tau \circ \sigma)(i) \neq (\sigma \circ \tau)(i)$.
            Proof: $k \neq j$ by choice of $k \notin \{i, j\}$.
...
<1>4. Conclusion: $Z(S_n) = \{e\} = 1$.
    Proof: By <1>2, $\{e\} \subseteq Z(S_n)$, and by <1>3, no non-identity element belongs to $Z(S_n)$. Thus $Z(S_n) = 1$.
:::
