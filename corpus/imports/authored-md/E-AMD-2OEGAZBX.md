...
id: E-AMD-2OEGAZBX
kind: exercise
title: Show that $\Out(A_4)$ is nontrivial.
classification:
  areas:
  - algebra
  topics:
...
1. **Inner Automorphisms of $A_4$:**
   Since $A_4$ has trivial center ($Z(A_4) = 1$):
   $$
   \Inn(A_4) \cong A_4 / Z(A_4) \cong A_4.
   $$
   Thus $|\Inn(A_4)| = |A_4| = 12$.

2. **Constructing an Outer Automorphism via Conjugation in $S_4$:**
   $A_4$ is a normal subgroup of the symmetric group $S_4$.
...
3. **Showing $\phi_\tau \notin \Inn(A_4)$:**
   Consider the transposition $\tau = (12) \in S_4 \setminus A_4$.
...
4. **Conclusion:**
   The automorphism $\phi_{(12)}$ is an outer automorphism of $A_4$.
