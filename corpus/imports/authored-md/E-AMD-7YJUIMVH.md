...
Proof: By induction on $k$: Base case $k=0$ gives $T^0(v_i) = v_i = v_{i+0 \pmod n}$.
...
<2>3. Since $T^n(v_i) = I(v_i)$ on all basis vectors $v_i$, $T^n = I$ (the identity operator on $V$). Proof: Two linear operators agreeing on a basis are identical.
...
Proof: Standard factorization of $x^n - 1$ over $\mathbb{C}$.
...
<3>3. Q.E.D. Proof: Explicit verification of diagonalizability.
