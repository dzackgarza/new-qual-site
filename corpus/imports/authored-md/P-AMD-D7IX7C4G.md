...
<1>1. Restrict $f$ to the equatorial circle $S^1 \subset S^2$.
  ...
  <2>3. Every continuous odd map $g \colon S^1 \to S^1$ has odd degree $\deg(g) \equiv 1 \pmod 2$.
    ...
    <3>4. Thus $\deg(g) \neq 0$.
    <3>5. Proof: Standard degree computation of odd maps on the circle. Q.E.D.
...
<1>2. Extend $g$ to the upper hemisphere disk $D_+^2 \subset S^2$.
  ...
  <2>4. Proof: The inclusion $S^1 = \partial D^2 \hookrightarrow D^2$ is nullhomotopic since $D^2$ is contractible, so the induced homomorphism $g_* \colon \pi_1(S^1) \to \pi_1(S^1)$ is zero, meaning $\deg(g) = 0$. Q.E.D.
