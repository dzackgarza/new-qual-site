...
<1>2. Show that the punctured sphere $S^n \setminus \{p\}$ is contractible.
  ...
  <2>4. $H_0(z, 0) = z = \operatorname{id}_{S^n \setminus \{p\}}(z)$, and $H_0(z, 1) = \phi^{-1}(0) = N$ (constant map to the antipodal pole $N$).
  <2>5. $H_0$ is continuous because $\phi$, $\phi^{-1}$, and scalar multiplication are continuous.
  <2>6. Proof: By <2>1–<2>5, $S^n \setminus \{p\}$ is contractible to the point $N$. Q.E.D.

<1>3. Construct the nullhomotopy for $f$.
  <2>1. Define $H \colon X \times [0, 1] \to S^n$ by $H(x, t) = H_0(f(x), t)$.
  <2>2. Equivalently, using normalized straight-line geodesics in $\mathbb{R}^{n+1}$:
  Since $f(x) \neq -N$ for all $x \in X$, $(1-t)f(x) + t N \neq 0$ for all $t \in [0, 1]$, so
  $$H(x, t) = \frac{(1-t)f(x) + t N}{\|(1-t)f(x) + t N\|}$$
  is well-defined, continuous, and lands in $S^n$.
  <2>3. Check endpoints:
...
<1>4. Q.E.D.
  <2>1. Proof: <1>3 proves that $f$ is homotopic to the constant map $c_N$.
:::
