...
To prove that $R[t]$ is an integral domain, let $f(t), g(t) \in R[t]$ be two non-zero polynomials. We must show that $f(t)g(t) \neq 0$.

1. **Expressing polynomials in terms of leading coefficients:**
   Since $f(t) \neq 0$ and $g(t) \neq 0$, we can write:
   $$
...
2. **Computing the product $f(t)g(t)$:**
...
3. **Applying the integral domain property of $R$:**
...
4. **Conclusion:**
   Since the coefficient of $t^{n+m}$ in $f(t)g(t)$ is non-zero, the polynomial $f(t)g(t)$ is not the zero polynomial:
   $$
