...
1. **Lemma (Non-triviality of the center of a finite $p$-group):**
   ...
   - Since $e \in Z(G)$, $|Z(G)| \geq 1$. Because $p \mid |Z(G)|$, we have:
     $$
     |Z(G)| \geq p > 1.
     $$
   Thus the center $Z(G)$ is non-trivial.

2. **Induction Step using the Upper Central Series:**
   Define the upper central series $\{Z_i(G)\}$ of $G$ inductively by:
   $$
...
Hence, the upper central series of $G$ terminates at $G$ in at most $n$ steps ($Z_n(G) = G$).
