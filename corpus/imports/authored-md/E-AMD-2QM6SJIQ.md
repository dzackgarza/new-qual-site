...
1. **Roots and Splitting Field:**
   The roots of $x^n - 1$ in $\CC$ are the $n$-th roots of unity:
   $$
...
2. **Action of Automorphisms on Primitive Roots:**
   Let $\sigma \in \Gal(\QQ(\zeta_n)/\QQ)$.
...
3. **Isomorphism with the Group of Units $(\ZZ/n\ZZ)^\times$:**
   Define the map:
   $$
   ...
   - **Surjectivity (Irreducibility of Cyclotomic Polynomials):**
     The minimal polynomial of $\zeta_n$ over $\QQ$ is the $n$-th cyclotomic polynomial $\Phi_n(x)$, which is irreducible over $\QQ$ and has degree $\deg \Phi_n = \phi(n)$ (Euler's totient function).
     Therefore:
     $$
