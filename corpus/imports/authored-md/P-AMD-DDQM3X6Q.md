...
id: P-AMD-DDQM3X6Q
kind: problem
title: Show $\tilde H_i(\Sigma X) \cong \tilde H_{i-1}(X)$
classification:
  areas:
  - topology
  topics:
...
**Goal:** Let $X$ be a topological space. Prove that the suspension $\Sigma X = (X \times [-1, 1]) / (X \times \{1\} \sim N, X \times \{-1\} \sim S)$ satisfies $\widetilde{H}_i(\Sigma X) \cong \widetilde{H}_{i-1}(X)$ for all $i \in \mathbb{Z}$, and prove that $\Sigma S^n \cong S^{n+1}$ for all $n \ge 0$.
...
<1>1. Prove the suspension isomorphism $\widetilde{H}_i(\Sigma X) \cong \widetilde{H}_{i-1}(X)$.
  ...
  <2>5. Write the Mayer-Vietoris sequence in reduced homology for $(\Sigma X; U, V)$:
  $$\cdots \to \widetilde{H}_i(U) \oplus \widetilde{H}_i(V) \to \widetilde{H}_i(\Sigma X) \xrightarrow{\partial} \widetilde{H}_{i-1}(U \cap V) \to \widetilde{H}_{i-1}(U) \oplus \widetilde{H}_{i-1}(V) \to \cdots$$
  <2>6. Substituting the vanishing groups from <2>3 gives exact sequences:
  $$0 \longrightarrow \widetilde{H}_i(\Sigma X) \xrightarrow{\partial} \widetilde{H}_{i-1}(X) \longrightarrow 0.$$
  <2>7. Therefore, the connecting homomorphism $\partial \colon \widetilde{H}_i(\Sigma X) \xrightarrow{\cong} \widetilde{H}_{i-1}(X)$ is an isomorphism for all $i \in \mathbb{Z}$.
  <2>8. Proof: By Mayer-Vietoris sequence for the suspension open cover. Q.E.D.
...
<1>2. Prove $\Sigma S^n \cong S^{n+1}$.
  ...
  <2>2. Define a map $f \colon S^n \times [-1, 1] \to S^{n+1}$ by:
  $$f(u, t) = \left( \sqrt{1 - t^2} \, u, \, t \right) \in \mathbb{R}^{n+1} \times \mathbb{R}.$$
  <2>3. Verify the image and continuity of $f$:
  ...
  <2>5. $f$ is surjective: For any $(x, t) \in S^{n+1}$, $|t| \le 1$. If $|t| < 1$, set $u = x / \sqrt{1-t^2} \in S^n$, then $f(u, t) = (x, t)$. If $t = \pm 1$, $(x, t) = (0, \pm 1) = f(u, \pm 1)$.
  ...
  <2>8. Proof: By compact-to-Hausdorff homeomorphism theorem. Q.E.D.

<1>3. Q.E.D.
  <2>1. Proof: <1>1 establishes the suspension isomorphism and <1>2 establishes $\Sigma S^n \cong S^{n+1}$.
:::
