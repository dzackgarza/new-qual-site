...
<1>1. Proof of (a): completeness under the uniform norm.
    ...
    <2>4. Q.E.D.
        Proof: <2>2 and <2>3 show every Cauchy sequence converges in $C([0,1])$.

<1>2. Proof of (b): incompleteness under $\norm{\cdot}_1$.
    <2>1. For $n \geq 2$ define $f_n(x) = 0$ on $[0, \frac12 - \frac1n]$, $f_n(x) = 1$ on $[\frac12, 1]$, and linear in between. Each $f_n$ is continuous, and $\{f_n\}$ is Cauchy in $\norm{\cdot}_1$.
        Proof: continuity is clear for piecewise-linear functions. For $m \geq n$, $f_n$ and $f_m$ differ only on $[\frac12 - \frac1n, \frac12]$, where both take values in $[0,1]$; hence $\norm{f_n - f_m}_1 \leq m([\frac12 - \frac1n, \frac12]) = \frac1n \to 0$.
    <2>2. The pointwise limit of $f_n$ is $\chi_{[\frac12, 1]}$, and $f_n \to \chi_{[\frac12,1]}$ in $L^1$; but $\chi_{[\frac12,1]}$ is not continuous.
        Proof: $f_n(\frac12) = 1$ for all $n$; for $x < \frac12$, $f_n(x) = 0$ for all large $n$; for $x > \frac12$, $f_n(x) = 1$. And $\norm{f_n - \chi_{[\frac12,1]}}_1 \leq \frac1n \to 0$.
    ...
    <2>4. Q.E.D.
        Proof: <2>1 and <2>3 exhibit a Cauchy sequence with no limit in $C([0,1])$, so the space is not complete under $\norm{\cdot}_1$.
:::
