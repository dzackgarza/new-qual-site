...
<1>1. Preparatory bounds.
    ...
    <2>2. Interpolation bound: for $0 < p \leq q$, $\norm{f}_p \leq \norm{f}_q \cdot m(\{f \neq 0\})^{1/p - 1/q}$.
        ...
        $$\int |f|^p \leq \left(\int |f|^q\right)^{p/q} m(\{f \neq 0\})^{1 - p/q},$$
        and taking the $p$-th root gives the claim.
    <2>3. Q.E.D.
        Proof: These bounds are used in <1>2 and <1>4.
...
<1>2. Proof of (2): if $m\{f = 0\} > 0$, then $\norm{f}_p \to 0$ as $p \to 0^+$.
    ...
    <2>5. Q.E.D.
        Proof: This proves (2); in particular the limit in (1) exists in this case.
...
<1>3. The limit formula: for $f$ with $m\{f = 0\} = 0$ and $\log|f| \in L^1$, $\lim_{p \to 0^+} \log \norm{f}_p = \int_0^1 \log|f|$.
    ...
    <2>4. Reduce the general case to the bounded case by truncation: for $M > 1$ put $f_M \definedas \min(\max(|f|, 1/M), M)$; then $\log f_M \in [-\log M, \log M]$, $f_M \uparrow |f|$ as $M \to \infty$, and $\int \log f_M \uparrow \int \log|f|$.
        Proof: $f_M$ is bounded above and below; monotone convergence applies to $f_M \uparrow |f|$ and $\log f_M \uparrow \log|f|$ (extended reals; here $\log|f| \in L^1$ by the hypothesis of <1>3).
    ...
    <2>6. Hence $\limsup_{p \to 0^+} \log \norm{f}_p \leq \sup_M \lim_{p \to 0^+} \log \norm{f_M}_p = \sup_M \int \log f_M = \int \log|f|$.
        Proof: From <2>5, $\log \norm{f}_p = \sup_M \log \norm{f_M}_p$, and $\limsup_{p \to 0} \sup_M g_M(p) \leq \sup_M \limsup_{p \to 0} g_M(p)$; the inner limit is $\int \log f_M$ by <2>3 (bounded case), and the sup is $\int\log|f|$ by <2>4.
    <2>7. Q.E.D.
        Proof: By <2>1 and <2>6, $\lim_{p \to 0^+} \log \norm{f}_p = \int \log|f|$, so $\norm{f}_p \to \exp\left(\int \log|f|\right)$.

<1>4. Proof of (1): the limit always exists.
    <2>1. Case $m\{f = 0\} > 0$: the limit exists and equals $0$.
        Proof: By <1>2.
    <2>2. Case $m\{f = 0\} = 0$: the limit exists and equals $\exp\left(\int_0^1 \log|f|\right) \in [0, \infty)$.
        Proof: Apply <1>3. If $\log|f| \in L^1$, the formula gives a finite limit. If $\int \log|f| = -\infty$ (i.e. $\log^-|f|$ is not integrable), the same truncation argument applies: <1>3<2>3–<2>6 go through with $\sup_M \int\log f_M = -\infty$, giving $\log\norm{f}_p \to -\infty$ and $\norm{f}_p \to 0$; the lower bound of <1>3<2>1 ($\geq -\infty$) is then vacuous and the upper bound forces the limit. Hence the limit exists in all cases.
    <2>3. Q.E.D.
        Proof: The two cases of <2>1 and <2>2 exhaust all possibilities; in each the limit exists.
:::
