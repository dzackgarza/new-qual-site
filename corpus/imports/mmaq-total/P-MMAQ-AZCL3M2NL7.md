...
::: problem
(a) Let $f: D\rightarrow \mathbb C$ be a continuous function, where $D\subset \mathbb C$ is a domain.
Let $\alpha:[a,b]\rightarrow D$ be a smooth curve.
...
Provide an upper bound for $\abs{ \int_{C_R} \dfrac{\log{(z)} }{z^2} }$ which depends *only* on $R$ and other constants.
:::
