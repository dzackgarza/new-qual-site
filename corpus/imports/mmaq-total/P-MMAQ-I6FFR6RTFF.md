...
kind: problem
title: $\int_{\RR}|f(y-x)-f(y)|\,dy\to 0$ as $x\to 0$ for $f\in L^1(\RR)$
classification:
  areas:
  - real-analysis
  topics:
...
Proof: By <1>2. <2>2. For $\abs{x}$ small, $\norm{g(\cdot - x) - g}_1 < \eps/3$.
...
Proof: $\norm{f(\cdot-x) - g(\cdot-x)}_1 = \norm{(f - g)(\cdot - x)}_1 = \norm{f - g}_1$; combine with <2>2 and <2>3. <2>5. Q.E.D. Proof: $\eps > 0$ was arbitrary.
