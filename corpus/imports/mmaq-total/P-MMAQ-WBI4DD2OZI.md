...
<1>1. Statement of Fatou's Lemma.
    <2>1. Let $(X, \mathcal M, \mu)$ be a measure space and $\{f_n\}$ a sequence of measurable functions with $f_n \geq 0$ for all $n$. Then
        $$\int_X \liminf_{n \to \infty} f_n ~d\mu \leq \liminf_{n \to \infty} \int_X f_n ~d\mu.$$
        Proof: This is the statement to be proved; both sides may be $+\infty$.
    <2>2. Q.E.D.
        Proof: The statement is recorded for the proof below.
...
<1>2. Proof.
    ...
    <2>4. Take the limit as $k \to \infty$:
        $$\int_X \liminf_n f_n ~d\mu = \lim_k \int_X g_k ~d\mu \leq \lim_k \inf_{n \geq k} \int_X f_n ~d\mu = \liminf_n \int_X f_n ~d\mu.$$
        Proof: Left equality by <2>2; inequality by <2>3 (passing to the limit); right equality by definition of $\liminf$ applied to the sequence $\int_X f_n$.
    <2>5. Q.E.D.
        Proof: This proves Fatou's Lemma.
:::
