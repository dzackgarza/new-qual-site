...
Then, by Krein-Milman theorem, $\text{Ext}(H_\phi)\ne\varnothing$. Let $f\in\text{Ext}(H_\phi)$, we claim that $\|f\|=1$. Indeed, suppose $\|f\|<1$. Let $g$ be a linear functional, such that $g|_M=0$ but $\|g\|=1$. Then define $f_1=f+(1-\|f\|)g$ and $f_2=f-(1-\|f\|)g$. Then $f_1|_M = f|_M + (1-\|f\|)g|_M = \phi$ since $g|_M = 0$, and by the triangle inequality $\|f_1\| \le \|f\| + (1-\|f\|)\|g\| = \|f\| + (1-\|f\|) = 1$; the same identities with the sign reversed give $f_2|_M = \phi$ and $\|f_2\| \le 1$. However, $f=(f_1+f_2)/2$. This contradicts to $f\in\text{Ext}(H_\phi)$. Thus, $f\in S_{X^*}\cap O$. We are done.
:::
