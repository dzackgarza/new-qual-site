...
::: {.proof}
functional analysis generalization of polar coordinates.
:::
...
::: {.proof}
definition of the positive square root $|T| = \sqrt{T^* T}$.
:::
<2>2. In particular, $\ker(|T|) = \ker(T)$, and $\overline{\operatorname{im}(|T|)} = (\ker |T|)^\perp = (\ker T)^\perp$.
::: {.proof}
norm equality implies equal kernels.
:::
...
::: {.proof}
continuous extension of uniformly continuous maps.
:::
...
::: {.proof}
orthogonal decomposition $H = \overline{\operatorname{im}(|T|)} \oplus \ker(|T|)$.
:::
