...
::: {.proof}
Standard definition of the upper central series.
:::
...
::: {.proof}
If $k-1 \ge 1$, then $Z_{k-1}(G)$ must be $\{e\}$ by minimality of $k$.
:::
...
::: {.proof}
Equivalence of nilpotency characterizations.
:::
...
::: {.proof}
Commutator $[x, g] = e \iff x g = g x$.
:::
<2>4. Since $\gamma_c(G) \neq \{e\}$ and $\gamma_c(G) \subseteq Z(G)$, $Z(G)$ must be non-trivial.
::: {.proof}
A superset of a non-trivial set is non-trivial.
:::
