...
Proof: definition of the module structure.

<1>2. $M$ is cyclic, generated by $e_1 = (1, 0)^t$.
Proof: for any $v = (x, y)^t$, the equation $(tA + s)e_1 = v$ is the linear system $\begin{pmatrix} 1 & 1 \\ 2 & 0 \end{pmatrix}\begin{pmatrix} t \\ s \end{pmatrix} = \begin{pmatrix} x \\ y \end{pmatrix}$, whose matrix is invertible (determinant $-2 \neq 0$), so it always has a solution; hence $v = (tA + s)e_1 = (tx + s)e_1$ is in the cyclic submodule generated by $e_1$.

<1>3. $M$ is torsion.
Proof: by Cayley–Hamilton, $A$ satisfies its characteristic polynomial $\chi_A(x) = x^2 - 2x - 3$ (since $\operatorname{tr} A = 2$ and $\det A = -3$), so $p(x) = x^2 - 2x - 3$ annihilates every $v \in M$: $p(A)v = 0$ for all $v$.
