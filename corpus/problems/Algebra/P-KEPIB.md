...
::: {.proof}
Since $c_i \in K$ for all $i$, $p(x)$ is a well-defined polynomial in $K[x]$.
:::
<2>2. $p(x)$ is not the zero polynomial, i.e., $p(x) \neq 0$.
::: {.proof}
By <1>2.<2>4, not all coefficients $c_i$ are zero.
:::
<2>3. $p(\alpha) = \sum_{i=0}^n c_i \alpha^i = 0$.
::: {.proof}
By <1>2.<2>4. <2>4. Therefore, $\alpha$ is algebraic over $K$.
:::
::: {.proof}
$\alpha$ is a root of the non-zero polynomial $p(x) \in K[x]$.
:::
