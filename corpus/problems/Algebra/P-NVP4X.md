...
<2>2. Hence $n_{11} = 1$.
::: {.proof}
the only divisor of $5$ congruent to $1$ mod $11$ is $1$.
:::
...
::: {.proof}
$n_5 \equiv 1 \pmod 5$ and $n_5 \mid 11$, so $n_5 = 1$ or $11$.
:::

<1>4. $G \cong P \rtimes Q$ (semidirect product).
::: {.proof}
$P \trianglelefteq G$ (<1>2), $P \cap Q = 1$ (orders coprime), and $|PQ| = 55 = |G|$, so $G = P \rtimes Q$.
:::

<1>5. Classification.
<2>1. If $n_5 = 1$, then $Q \trianglelefteq G$ and $G \cong \ZZ/5 \times \ZZ/11 \cong \ZZ/55$.
::: {.proof}
both Sylow subgroups are normal and intersect trivially, so $G$ is their direct product.
:::
<2>2. If $n_5 = 11$, then $G$ is the nonabelian semidirect product $\ZZ/11 \rtimes \ZZ/5$.
::: {.proof}
$\operatorname{Aut}(\ZZ/11) \cong \ZZ/10$ has a unique subgroup of order $5$, giving a nontrivial action of $\ZZ/5$ on $\ZZ/11$; the resulting semidirect product is nonabelian (a Frobenius group of order $55$).
:::
