...
<1>1. Non-triviality of the center $Z(G)$: *Proof:* <2>1. By the class equation for $G$: $$|G| = |Z(G)| + \sum_{i=1}^k [G : C_G(x_i)],$$ where $x_1, \dots, x_k$ are representatives of the non-central conjugacy classes.
...
<1>2. Proof that $G$ is abelian ($|Z(G)| = p^2$): *Proof:* <2>1. Suppose for contradiction that $|Z(G)| = p$.
...
<2>5. Computing the product: $$x y = (g^a z_1)(g^b z_2) = g^{a+b} z_1 z_2 = g^{b+a} z_2 z_1 = (g^b z_2)(g^a z_1) = y x.$$ <2>6. Thus $G$ is abelian, which forces $Z(G) = G$ and $|Z(G)| = p^2$, a contradiction.
<2>7. Therefore $|Z(G)| = p^2$, so $Z(G) = G$, meaning $G$ is abelian.

<1>3. Classification of groups of order $p^2$: *Proof:* <2>1. By Lagrange's Theorem, every non-identity element of $G$ has order $p$ or $p^2$.
...
<1>4. Conclusion: Every group of order $p^2$ is abelian, and up to isomorphism the only such groups are: $$\mathbb{Z}_{p^2} \quad \text{and} \quad \mathbb{Z}_p \times \mathbb{Z}_p.$$ Q.E.D.
:::
