...
Proof: adjugate formula for matrix inverse.
<2>3. By Cramer’s rule, $(tI - A) B(t) = \det(tI - A) I = p(t) I$.
Proof: fundamental property of the adjugate matrix.
...
\]
Proof: equating coefficients of polynomials with matrix coefficients.
...
Proof: telescoping sum of matrix equations.
...
Proof: Smith Normal Form over the PID $\mathbb{Z}$.
...
Proof: isomorphic presentation of quotients.
...
Proof: $\mathbb{Z}/0\mathbb{Z} \cong \mathbb{Z}$ is infinite.
<2>4. When $\det(A) \neq 0$, $|\operatorname{coker}(A)| = \prod_{i=1}^n |d_i| = |\det(D)| = |\det(A)|$ since $\det P, \det Q \in \{\pm 1\}$.
Proof: product of orders of cyclic groups.
...
Proof: a matrix similar to the zero matrix is zero.
...
Proof: definition of diagonalizability and linear independence of eigenvectors.
...
Proof: $T$ is diagonalizable $\iff$ minimal polynomial splits with distinct roots.
...
Proof: commuting operators preserve eigenspaces.
...
Proof: restriction of diagonalizable operators to invariant subspaces is diagonalizable.
...
Proof: $F[x]$-submodules of $V$ are $E$-subspaces of $V$.
