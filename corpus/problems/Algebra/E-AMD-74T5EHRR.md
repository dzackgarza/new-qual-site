...
::: {.proof}
Two permutations that disagree on an element $i$ are distinct functions.
:::
<2>6. Consequently, $\sigma \notin Z(S_n)$.
::: {.proof}
$\sigma$ fails to commute with at least one element $\tau \in S_n$.
:::
<2>7. Q.E.D.
::: {.proof}
...
::: {.proof}
By <1>2, $\{e\} \subseteq Z(S_n)$, and by <1>3, no non-identity element belongs to $Z(S_n)$.
:::
Thus $Z(S_n) = 1$.
:::
