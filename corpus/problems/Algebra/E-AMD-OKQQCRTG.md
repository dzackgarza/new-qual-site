...
<1>1. A nontrivial finite $p$-group has nontrivial center.
    *Proof:*
    <2>1. Let $P$ be a group of order $p^n$ with $n \geq 1$, and take the class equation
        $$\abs P = \abs{Z(P)} + \sum_i [P : C_P(x_i)]$$
        over representatives $x_i$ of the conjugacy classes of size greater than $1$.
...
<1>4. Q.E.D.
    *Proof:* By step <1>3 the orders $\abs{Z_0} < \abs{Z_1} < \cdots$ strictly increase while $Z_m \neq G$, and they are bounded by $p^n$. So $Z_m = G$ for some $m \leq n$, which is the definition of $G$ being nilpotent.
:::
