...
*Proof:* <2>1. Let $P$ be a group of order $p^n$ with $n \geq 1$, and take the class equation $$\abs P = \abs{Z(P)} + \sum_i [P : C_P(x_i)]$$ over representatives $x_i$ of the conjugacy classes of size greater than $1$.
...
<1>3. If $Z_m \neq G$ then $Z_{m+1} \neq Z_m$.
*Proof:* <2>1. $G/Z_m$ is a nontrivial group whose order divides $p^n$, so it is a nontrivial $p$-group.
