...
\]
Proof: applying the permutation $T$ $n$ times.
...
Proof: linear independence of basis vectors.
...
m_T(x) = x^n - 1.
\]
Proof: monic divisibility of equal degree polynomials.
...
Proof: distinct $n$-th roots of unity.
...
Proof: diagonalizability criterion via minimal polynomials.
...
Proof: $T(\mathbf{w}_k) = \sum \omega^{-jk} \mathbf{v}_{j+1} = \omega^k \sum \omega^{-(j+1)k} \mathbf{v}_{j+1} = \omega^k \mathbf{w}_k$.
