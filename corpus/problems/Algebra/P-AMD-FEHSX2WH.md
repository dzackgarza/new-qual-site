...
Proof: definition of the nilradical.

<1>2. Let $M$ be any maximal ideal of $R$.
Proof: take an arbitrary maximal ideal.
...
Proof: <1>1 and <1>3, expanding by the binomial theorem.

<1>5. This contradicts $M$ being a proper ideal (it contains $1$).
Proof: <1>4.

<1>6. Hence $x \in M$ for every maximal ideal $M$.
Proof: <1>5 (contradiction forces $x \in M$).

<1>7. Therefore $x \in \bigcap_{M \text{ maximal}} M = J(R)$.
Proof: <1>6 and the definition of the Jacobson radical.

<1>8. Hence $\nilrad{R} \subseteq J(R)$.
Proof: <1>1 and <1>7 (for arbitrary $x \in \nilrad{R}$).
