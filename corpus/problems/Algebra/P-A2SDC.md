...
\]
Proof: right cosets partition $G$.

<1>2. Equality of left and right cosets:
<2>1. If $g \in H$, then $gH = H = Hg$.
Proof: closure of subgroup under multiplication.
<2>2. If $g \in G \setminus H$, then by <2>2 and <2>3:
\[
gH = G \setminus H = Hg.
\]
Proof: both equal the set-theoretic complement $G \setminus H$.
<2>3. Thus $gH = Hg$ for every $g \in G$.
Proof: <2>1 and <2>2 exhaust all cases $g \in G$.
