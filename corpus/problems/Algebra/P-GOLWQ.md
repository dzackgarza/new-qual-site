...
<1>1. The kernel of $\phi$ is an ideal of $F$: *Proof:* <2>1. By the First Isomorphism Theorem for rings, the kernel $\ker \phi = \{x \in F \mid \phi(x) = 0_R\}$ is an ideal of the field $F$.

<1>2. Classification of ideals in a field: *Proof:* <2>1. Let $I \subseteq F$ be an ideal of $F$.
...
<1>3. Dichotomy for $\phi$: *Proof:* <2>1. Applying <1>2 to $I = \ker \phi$, either $\ker \phi = F$ or $\ker \phi = (0)$.
<2>2. **If $\ker \phi = F$:** Then $\phi(x) = 0_R$ for all $x \in F$, so $\phi$ is the zero homomorphism.
<2>3. **If $\ker \phi = (0)$:** For any $x, y \in F$: $$\phi(x) = \phi(y) \implies \phi(x - y) = 0_R \implies x - y \in \ker \phi = (0) \implies x = y.$$ Thus $\phi$ is injective.

<1>4. Conclusion: Every ring homomorphism from a field is either identically $0$ or injective.
Q.E.D.
:::
