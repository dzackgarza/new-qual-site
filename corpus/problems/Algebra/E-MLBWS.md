...
Proof: definition of $K$.
<2>2. The set $\{u^i v^j : 0 \le i \le p-1,\ 0 \le j \le p-1\}$ forms a basis for $k$ over $K$.
Proof: $u$ and $v$ are algebraically independent over $\mathbb{F}_p$, so $[K(u):K] = p$ with basis $\{1, u, \dots, u^{p-1}\}$ and $[K(u,v):K(u)] = p$ with basis $\{1, v, \dots, v^{p-1}\}$.
...
Proof: $(a + b)^p = a^p + b^p$ in characteristic $p$, and $u^p, v^p, c^p \in K$.
...
Proof: $[E_c : K]$ divides $[k : K] = p^2$ and is strictly greater than 1, while $(u+cv)^p \in K$, so $[E_c : K] = p$.
