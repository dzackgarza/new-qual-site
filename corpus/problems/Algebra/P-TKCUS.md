...
::: {.proof}
Polynomial identity $y^q - 1 = (y-1)(y^{q-1} + \dots + 1)$ with $y = a^k$.
:::
...
::: {.proof}
Euclidean division and standard divisibility properties.
:::
<3>3. Q.E.D.
::: {.proof}
Follows from <3>1 and <3>2. <2>2. For any non-constant polynomial $g(x) \in \mathbb{F}_p[x]$ and $A, B \ge 1$, $(x^A - 1) \mid (x^B - 1)$ in $\mathbb{F}_p[x]$ if and only if $A \mid B$.
:::
::: {.proof}
Identical Euclidean division argument in polynomial rings: $x^B - 1 = x^r(x^{Aq} - 1) + (x^r - 1)$ where $B = Aq + r$ with $0 \le r < A$.
:::
...
::: {.proof}
Divisibility of $x^A - 1 \mid x^B - 1$ implies $A \mid B$.
:::
<2>4. By <1>1.<2>1 with $a = p \ge 2$, $(p^d - 1) \mid (p^n - 1)$ implies $d \mid n$.
::: {.proof}
Divisibility $(a^d - 1) \mid (a^n - 1)$ implies $d \mid n$.
:::
<2>5. Q.E.D.
::: {.proof}
