...
::: solution
**Goal:** given a permutation that moves some point, produce a transposition it does not commute with. A third point is needed to build that transposition, and $n \geq 4$ supplies one.
...
<1>4. Q.E.D.
    *Proof:* Steps <1>1 through <1>3 show that no $\sigma \neq \id$ is central, so $Z(S_n) = 1$.
:::
