...
Proof: subset relation is a partial order.
<2>3. Because $1_R \neq 0$, the zero ideal $(0)$ is proper ($1_R \notin (0)$), so $(0) \in \mathcal{P}$.
Thus $\mathcal{P} \neq \emptyset$.
Proof: $R$ is non-zero with identity.
...
Proof: unit criterion for ideals.
...
Proof: definition of maximal ideal.
