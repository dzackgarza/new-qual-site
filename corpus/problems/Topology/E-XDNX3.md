...
Proof: take an arbitrary point outside $A$.

<1>3. For each $a \in A$, there are disjoint open sets $U_a \ni x$ and $V_a \ni a$.
Proof: $X$ is Hausdorff and $x \neq a$.

<1>4. $\{V_a\}_{a \in A}$ is an open cover of $A$.
Proof: each $a \in A$ lies in $V_a$.
...
Proof: define a neighborhood of $x$.

<1>7. $U$ is open, contains $x$, and is disjoint from $A$.
Proof: $U$ is a finite intersection of open sets containing $x$; and $U \cap V_{a_i} = \varnothing$ for each $i$ (since $U \subseteq U_{a_i}$ and $U_{a_i} \cap V_{a_i} = \varnothing$), so $U \cap A = \varnothing$ by <1>5.

<1>8. Hence $x$ is an interior point of $X \setminus A$, so $X \setminus A$ is open.
Proof: <1>7 holds for every $x \in X \setminus A$.
