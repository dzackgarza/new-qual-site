...
Proof: every point lies in its own ball.
...
Proof: collect all these balls.

<1>4. $\mathcal B$ is countable.
Proof: it is a countable union of finite sets.

<1>5. $\mathcal B$ is a basis for the topology of $X$.
<2>1. Let $U$ be open and $x \in U$.
Proof: take an arbitrary point of an arbitrary open set.
...
Proof: if $y \in B(x_{n,j}, 1/n)$, then $d(x,y) \le d(x, x_{n,j}) + d(x_{n,j}, y) < 1/n + 1/n = 2/n < \epsilon$.
