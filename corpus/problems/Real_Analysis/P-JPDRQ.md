...
::: {.proof}
if $x_n \to x$ then every subsequence converges to $x$: for $\eps > 0$ all but finitely many terms lie within $\eps$ of $x$, so the same holds for the subsequence; limits are unique in $\RR$.
:::
...
::: {.proof}
pointwise limit is $\chi_{\{1\}}$ (for $x \in [0,1)$, $x^n \to 0$; at $1$, $1^n = 1$), but $\sup_{[0,1]}|x^n - \chi_{\{1\}}| = 1 \not\to 0$.
:::
...
::: {.proof}
peak at $n_1 < n_2$ with both peaks: $a_{n_1} \ge a_{n_2}$.
:::
...
::: {.proof}
$n_k$ is not a peak, so some later term is strictly larger; pick it greedily.
:::
...
::: {.proof}
definition of supremum; monotonicity.
:::
<2>2. Unbounded case: for any $M$, some $a_N > M$, and monotonicity gives $a_n \ge a_N > M$ for $n \ge N$.
::: {.proof}
unboundedness; monotonicity.
:::

<1>6. Bolzano–Weierstrass: every bounded sequence has a convergent subsequence.
::: {.proof}
by <1>4 the sequence has a monotone subsequence, which is bounded (the original sequence is bounded), so it converges by <1>5.
:::
:::
