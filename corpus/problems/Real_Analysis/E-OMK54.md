...
::: {.proof}
translation and dilation invariance of Lebesgue measure (for measurable $E$). <2>2. Both hold for non-negative simple functions, then for non-negative measurable $f$ by monotone convergence, then for $f \in L^1$ by positive/negative parts.
:::
::: {.proof}
linearity, then the standard MCT argument.
:::
...
::: {.proof}
approximate by open sets / finite unions of boxes (regularity); the measure of the symmetric difference $E \triangle (E + h) \to 0$ as $h \to 0$.
:::
<2>2. The claim holds for simple functions, then for all $f \in L^1$ by density and the $\eps/3$ argument.
::: {.proof}
simple functions are dense in $L^1$, and $\|\tau_h f - \tau_h s\|_1 = \|f - s\|_1$ by translation invariance.
:::
...
::: {.proof}
$Z = E \setminus F \subseteq E \setminus F_k$ for each $k$, so $m(Z) \le 1/k$ for all $k$; hence $m(Z) = 0$; $F$ is an $F_\sigma$ (countable union of closed sets), and $F \cap Z = \emptyset$.
:::
...
::: {.proof}
dual argument to <2>1 (outer regularity); $G$ is a $G_\delta$ and $m(Z') = 0$.
:::
...
::: {.proof}
$F_\sigma$ sets and $G_\delta$ sets are Borel, hence measurable; adding or removing null sets preserves measurability.
:::
<2>4. Q.E.D.
::: {.proof}
<2>1–<2>3.
...
::: {.proof}
upper bound $\|f\|_p \le \|f\|_\infty\mu(X)^{1/p} \to \|f\|_\infty$; lower bound $\|f\|_p \ge M\mu\{|f| > M\}^{1/p} \to M$ for $M < \|f\|_\infty$; sandwich.
:::
:::
