...
<1>2. For $x \in M$: $S_N x = x$ for all $N$, so $S_N x \to x = Px$.
    ::: {.proof}
    $U^n x = x$ for all $n$; $S_N x = \frac{1}{N}\sum_{n=0}^{N-1} x = x$.
    :::

<1>3. For $x = Uy - y \in \mathrm{ran}(U - I)$: $\|S_N x\| \to 0$.
    <2>1. $S_N (U - I) = \frac{1}{N}(U^N - I)$ (telescoping sum).
        ::: {.proof}
        $S_N U - S_N = \frac{1}{N}\sum_{n=0}^{N-1}(U^{n+1} - U^n) = \frac{1}{N}(U^N - I)$.
        :::
    <2>2. $\|S_N (Uy - y)\| = \frac{1}{N}\|U^N y - y\| \le \frac{2\|y\|}{N} \to 0$.
        ::: {.proof}
        <2>1 and the unitarity of $U$ ($\|U^N\| = 1$).
        :::

<1>4. For $x \in M^\perp = \overline{\mathrm{ran}(U - I)}$: $\|S_N x\| \to 0$.
    ::: {.proof}
    given $\eps > 0$, choose $z = Uy - y$ with $\|x - z\| < \eps/2$ (density, <1>1); then $\|S_N x\| \le \|S_N (x - z)\| + \|S_N z\| \le \|x - z\| + \|S_N z\|$ (since $\|S_N\| \le 1$: $\frac1N\sum_{n<N}\|U^n\| = 1$) $< \eps/2 + \eps/2$ for $N$ large by <1>3.
    :::

<1>5. For general $x = Px + (x - Px)$ with $Px \in M$, $x - Px \in M^\perp$: $\|S_N x - Px\| = \|S_N(x - Px)\| \to 0$.
    ::: {.proof}
    <1>2 gives $S_N Px = Px$; <1>4 applies to $x - Px \in M^\perp$.
    :::

<1>6. Q.E.D.
    ::: {.proof}
    <1>5 is exactly $\|S_N x - Px\|_H \to 0$ for all $x \in H$. (This is the mean ergodic theorem of von Neumann.)
    :::
:::
