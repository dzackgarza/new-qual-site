...
::: {.proof}
$\Lambda$ is a bounded linear functional, hence continuous, so its kernel is closed; it is a linear subspace as the kernel of a linear map.
:::

<1>2. $L^2(X) = M \oplus M^\perp$.
::: {.proof}
$M$ is a closed subspace of the Hilbert space $L^2(X)$; the orthogonal decomposition theorem gives $L^2(X) = M \oplus M^\perp$ (every $f$ writes uniquely as $m + m^\perp$).
:::

<1>3. Existence of $g$: there is $g \in L^2(X)$ with $\Lambda(f) = \int_X g\bar f$ for all $f \in L^2(X)$.
<2>1. If $\Lambda \equiv 0$, take $g = 0$.
::: {.proof}
the zero functional satisfies $\Lambda(f) = 0 = \int_X 0 \cdot \bar f$ for every $f \in L^2(X)$, so $g = 0$ represents $\Lambda$.
:::
...
::: {.proof}
$\Lambda(m) = 0$ by $m \in M$; decompose $f$'s component in $M^\perp = \spanof\{g_0\}$ as $\langle f, g_0\rangle g_0$ (the orthogonal projection onto a line).
:::
...
::: {.proof}
<2>3 and the computation $\overline{\overline{\Lambda(g_0)}g_0} = \Lambda(g_0)\bar g_0$-adjusted: $\int f\,\overline{g} = \int f\,\overline{\overline{\Lambda(g_0)}g_0} = \Lambda(g_0)\int f\bar g_0 = \langle f, g_0\rangle\Lambda(g_0)$.
:::
