...
::: {.proof}
compute $\hat f(\xi) = \int_{-1}^{1} e^{-ix\xi}\,dx = 2\sin(\xi)/\xi$; and $\int |\sin(\xi)/\xi|\,d\xi = \infty$ since $|\sin(\xi)| \geq 1/2$ on a fixed fraction of each period.
:::
<2>3. Q.E.D.
::: {.proof}
<2>1 and <2>2 exhibit an $L^1$ function whose Fourier transform is not in $L^1$.
:::
...
::: {.proof}
the inversion theorem applies because both $f$ and $\hat f$ are integrable.
:::
<2>2. The function $x \mapsto \int \hat f(\xi) e^{ix\xi}\,d\xi$ is bounded by $\|\hat f\|_1$ and is uniformly continuous.
::: {.proof}
$|\int \hat f(\xi) e^{ix\xi}\,d\xi| \leq \int |\hat f(\xi)|\,d\xi = \|\hat f\|_1$; and $|\int \hat f(\xi)(e^{i(x+h)\xi} - e^{ix\xi})\,d\xi| \leq \int |\hat f(\xi)|\,|e^{ih\xi} - 1|\,d\xi \to 0$ as $h \to 0$ by dominated convergence ($|e^{ih\xi} - 1| \leq 2$ and $\hat f \in L^1$), uniformly in $x$.
:::
<2>3. Q.E.D.
::: {.proof}
<2>1 identifies $f$ (a.e.) with the function in <2>2, so $f$ is bounded and uniformly continuous after modification on a null set.
:::
...
<1>4. Q.E.D.
::: {.proof}
<1>1 answers (1) negatively; <1>2 and <1>3 prove the claims in (2).
:::
:::
