...
Proof: Differentiate (v) with respect to $a$ at... rather, note $\int_0^\infty \frac{x^{a-1}}{1+x^n}\,dx = \frac{\pi}{n\sin(\pi a/n)}$; differentiating in $a$: $\int_0^\infty \frac{\log x \, x^{a-1}}{1+x^n}\,dx = \dd{a}\frac{\pi}{n\sin(\pi a/n)} = -\frac{\pi^2}{n^2}\cot\qty(\frac{\pi a}{n})\csc\qty(\frac{\pi a}{n})$.
