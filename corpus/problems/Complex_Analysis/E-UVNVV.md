...
::: {.proof}
<1>1 and <1>2: $f(z) - f(z_0) = (z - z_0)^{n+1}\phi(z) = ((z - z_0)\psi(z))^{n+1} = \zeta(z)^{n+1}$.
:::
...
<1>6. Q.E.D.
::: {.proof}
<1>3 shows the change of variables $z \leftrightarrow \zeta$ is one-to-one, and <1>5 shows $f$ in these coordinates is $(n+1)$-to-one; composing, $f$ is $(n+1)$-to-one near $z_0$.
:::
:::
