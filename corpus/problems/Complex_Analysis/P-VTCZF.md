...
::: {.proof}
A disk is convex, so by <1>2 every point of the disk has the same value as its center (join by the straight segment, which stays in the disk).
:::
...
<1>5. Q.E.D.
::: {.proof}
<1>4 shows $f$ is constant on the connected region $\Omega$.
:::
:::
