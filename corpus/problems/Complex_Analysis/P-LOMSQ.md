...
::: {.proof}
$Q = \theset{z \suchthat \Re z > 0, \Im z > 0}$.
:::

<1>3. $T$ maps the positive real axis to the lower semicircle $\theset{e^{i\theta} \suchthat -\pi < \theta < 0}$ from $1$ to $-1$.
::: {.proof}
For $x > 0$ real, $T(x) = \frac{x - i}{x + i}$ has modulus 1 and $T(1) = \frac{1-i}{1+i} = -i$; as $x$ runs from $0^+$ to $\infty$, $T(x)$ moves on the unit circle from $T(0) = -1$ through $-i$ to $T(\infty) = 1$, i.e. along the lower semicircle.
:::

<1>4. $T$ maps the positive imaginary axis to the real interval $(-1, 1)$.
::: {.proof}
For $y > 0$, $T(iy) = \frac{iy - i}{iy + i} = \frac{y-1}{y+1} \in (-1, 1)$, and as $y$ goes from $0^+$ to $\infty$ this runs over all of $(-1, 1)$ (from $-1$ to $1$).
:::

<1>5. $T(Q)$ is the lower half-disk $\theset{w \in \DD \suchthat \Im w < 0}$.
::: {.proof}
$Q$ is the region between the two rays of <1>2; its image under the Möbius map $T$ is the region bounded by the two image curves of <1>3 and <1>4, namely the lower semicircle and the diameter $(-1, 1)$.
:::
...
<1>6. Q.E.D.
::: {.proof}
<1>1 gives the map $T(z) = \frac{z-i}{z+i}$ from $\HH$ onto $\DD$, and <1>5 describes the image of the first quadrant as $\DD \cap \theset{\Im w < 0}$.
:::
:::
