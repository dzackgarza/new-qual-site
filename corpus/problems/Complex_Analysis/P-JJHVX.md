...
::: {.proof}
Repeat <2>2 indefinitely; the diameters shrink by a factor $1/2$ each step.
:::
<2>4. The nested triangles have a common point $z_0 \in T$ (indeed $z_0 \in \Omega$).
::: {.proof}
...
<2>8. Contradiction.
::: {.proof}
$\operatorname{diam}(T_n) = 2^{-n} d$ and $\operatorname{perim}(T_n) = 2^{-n} p$ with $d, p$ fixed, so the bound in <2>7 is $\sup\abs\varepsilon \cdot dp/4^n$; but $\sup_{z \in T_n}\abs{\varepsilon(z)} \to 0$ (<2>5), so $\abs{\int_{T_n} f} < M/4^n$ for large $n$, contradicting <2>3.
:::

<1>3. By Morera's theorem, $f$ is holomorphic.
::: {.proof}
Morera: if $f$ is continuous on $\Omega$ (<1>1) and $\int_T f = 0$ for every triangle $T \subset \Omega$ (<1>2), then $f$ is holomorphic.
:::
