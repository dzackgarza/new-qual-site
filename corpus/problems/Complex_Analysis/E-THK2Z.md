...
Proof: If $f$ is holomorphic, the Cauchy–Riemann equations $u_x = v_y$, $u_y = -v_x$ hold.
Since $f$ is real-valued, $v \equiv 0$, so $v_x = v_y = 0$; hence $u_x = u_y = 0$ on the domain.

<1>2. $u$ is constant on the domain.
Proof: <1>1 shows both partial derivatives of $u$ vanish.
...
<1>3. Q.E.D. Proof: $f = u$ (as $v \equiv 0$) is constant by <1>2.
:::
