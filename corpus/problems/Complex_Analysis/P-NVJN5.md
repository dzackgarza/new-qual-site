...
::: {.proof}
$f$ is bounded in a punctured neighborhood of $z_0$, so by Riemann's theorem on removable singularities $f$ extends holomorphically across $z_0$; call the extension $\tilde f$, which is holomorphic on all of $\Omega$.
:::

<1>2. For every triangle $\Delta \subseteq \Omega$, $\int_\Delta \tilde f = 0$.
::: {.proof}
$\tilde f$ is holomorphic on the open set $\Omega$, so the Cauchy–Goursat theorem gives vanishing integrals over all closed triangles.
:::
