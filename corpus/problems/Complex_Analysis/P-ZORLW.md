...
::: {.proof}
By <1>1 and the hypothesis $\abs f = 0$ on $\bd\Omega$, $\max_{\bar\Omega}\abs f = 0$, so $f \equiv 0$ on $\bar\Omega$.
:::

<1>3. If $f$ is not constant, then $f$ has a zero in $\Omega$.
::: {.proof}
Contrapositive of <1>2: if $f$ has no zero in $\Omega$, then by <1>2 $f \equiv 0$ on $\bar\Omega$, so $f$ is constant (the zero function).
:::

<1>4. Q.E.D.
::: {.proof}
<1>3 shows that a nonconstant $f$ must vanish somewhere in $\Omega$, which together with the trivial constant case proves the claim.
:::
:::
