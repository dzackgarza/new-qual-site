...
::: {.proof}
$\abs{w - z}^2 = \abs w^2 + \abs z^2 - w\bar z - \bar w z$ and $\abs{1 - \bar w z}^2 = 1 + \abs w^2 \abs z^2 - \bar w z - w \bar z$; subtracting gives $\abs w^2 + \abs z^2 - 1 - \abs w^2\abs z^2 = (\abs w^2 - 1)(1 - \abs z^2)$.
:::

<1>2. (a): For $\abs z, \abs w < 1$, $\abs{\frac{w - z}{1 - \bar w z}} < 1$.
::: {.proof}
By <1>1, $\abs{w - z}^2 - \abs{1 - \bar w z}^2 = (\abs w^2 - 1)(1 - \abs z^2) < 0$ (both factors negative), so $\abs{w - z} < \abs{1 - \bar w z}$; dividing by the positive $\abs{1 - \bar w z}$ (nonzero because $\abs{\bar w z} = \abs w \abs z < 1$) gives the claim.
:::
...
::: {.proof}
$F$ is a rational function whose denominator $1 - \bar w z$ does not vanish on $\DD$ (as $\abs{\bar w z} \leq \abs w \abs z < 1$ for $\abs w < 1$, $\abs z < 1$); hence it is holomorphic there.
:::

<1>5. (b): $F(0) = w$ and $F(w) = 0$.
::: {.proof}
$F(0) = \frac{w - 0}{1 - 0} = w$, and $F(w) = \frac{w - w}{1 - \bar w w} = \frac{0}{1 - \abs w^2} = 0$ (valid since $\abs w < 1$).
:::

<1>6. (b): $F$ maps $\DD$ into itself and $\abs z = 1 \implies \abs{F(z)} = 1$.
::: {.proof}
By <1>2, $\abs{F(z)} < 1$ for $\abs z < 1$; by <1>3 with the roles as given, $\abs{F(z)} = 1$ for $\abs z = 1$ (equality case $\abs z = 1$).
:::

<1>7. (b): $F: \DD \to \DD$ is bijective.
::: {.proof}
$F(F(z)) = \frac{w - F(z)}{1 - \bar w F(z)}$.
:::
...
<1>8. Q.E.D.
::: {.proof}
<1>2–<1>3 prove (a) and <1>4–<1>7 prove (b).
:::
:::
