...
Then $G\da T\inv \circ f: \Delta^* \to \Delta$, and since $\abs{T} < 1$, $z=0$ is a removable singularity for $F$ and $F$ extends holomorphically to $G:\Delta\to \CC$, since a priori $G(0)$ may not be bounded by 1. Supposing $G$ is not constant, $G(\Delta) \subseteq \overline{\Delta}$ by continuity and $G(\Delta)$ is open by the open mapping theorem, and $\Delta = f(\Delta) \subseteq G(\Delta)$, so $G:\Delta\to \Delta$ is a map of the disc.
