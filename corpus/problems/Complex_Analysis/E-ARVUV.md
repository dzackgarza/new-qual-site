...
::: {.proof}
symmetric computation; $\cot(-\pi i a) = i\coth(\pi a)$, so $\pi \cot(-\pi i a) = i\pi \coth(\pi a)$, and dividing by $-2ia$ gives $-\frac{\pi \coth(\pi a)}{2a}$.
:::
...
::: {.proof}
residue theorem, using <1>3–<1>5.
:::

<1>8. $\oint_{\gamma_N} f(z)\,dz \to 0$ as $N \to \infty$.
::: {.proof}
$|\cot(\pi z)|$ is bounded on $\gamma_N$, and $|f(z)| \le C/N^2$ on the contour (since $|z^2 + a^2| \sim N^2$), while the perimeter is $O(N)$, so the integral is $O(1/N)$.
:::

<1>9. Hence $0 = \sum_{k=-\infty}^{\infty} \frac{1}{k^2 + a^2} - \frac{\pi \coth(\pi a)}{a}$.
::: {.proof}
<1>7 and <1>8, taking the limit.
:::
