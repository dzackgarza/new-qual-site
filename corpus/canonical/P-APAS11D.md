...
id: P-APAS11D
kind: problem
title: Murnaghan–Nakayama on $S_5$, Young-subgroup characters, and restriction of $A^{(1,4)}$
classification:
  areas:
  - applied-algebra
  topics:
...
(c) Find the values of the character of $A^{(1,4)}$ on the conjugacy classes of $S_5$.

(d) Decompose $A^{(1,4)}\downarrow_{S_3\times S_2}^{S_5}$ as a sum of irreducible representations of $S_3\times S_2$.
:::
