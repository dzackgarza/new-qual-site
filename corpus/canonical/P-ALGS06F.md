---
schema: qual/card@1
id: P-ALGS06F
kind: problem
title: "Projective, flat, and injective module constructions via tensor and Hom"
classification:
  areas:
  - algebra
  topics:
  - Module Theory
relations: []
review: draft
solved: false
---

::: problem
Let $R$ be a commutative ring and let $M$ and $N$ be $R$-modules. Prove the following.

(a) If $M$ and $N$ are projective $R$-modules, then $M \otimes_R N$ is a projective $R$-module.

(b) If $M$ and $N$ are flat $R$-modules, then $M \otimes_R N$ is a flat $R$-module.

(c) If $M$ is a flat $R$-module and $N$ is an injective $R$-module, then $\operatorname{Hom}_R(M, N)$ is an injective $R$-module.

(d) Let $p$ be a prime number and let $\mathbb{Z}_{(p)}$ denote the localization of $\mathbb{Z}$ with respect to its prime ideal $p\mathbb{Z}$. Show that $\operatorname{Hom}_\mathbb{Z}(\mathbb{Z}_{(p)}, \mathbb{Q}/\mathbb{Z})$ is an injective $\mathbb{Z}$-module.
:::
