...
**(a) Any matrix $B$ commuting with $A$ is a polynomial in $A$:**
Since $\{v, Av, \ldots, A^{n-1}v\}$ is a set of $n$ linearly independent vectors in an $n$-dimensional vector space $V = K^n$, it forms a basis for $V$.
...
Now, let $w \in V$ be any vector. Since $\{v, Av, \ldots, A^{n-1}v\}$ is a basis, $w = q(A)v$ for some polynomial $q(x) \in K[x]$.
...
- **$(\Longrightarrow)$:**
  Suppose such a vector $v$ exists.
...
- **$(\Longleftarrow)$:**
  Suppose $\operatorname{char}_A(x) = m_A(x)$.
