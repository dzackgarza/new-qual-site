...
::: problem
Suppose $F$ is a field. Let $m \leq n$ be two positive integers and
\[
...
\]
and $\overline{\bullet}$ denotes the corresponding coset. (Let's emphasize that the tensor is over $F$, and not over $F[x]$.)

(a) Prove that $\operatorname{Ann}(M_{m,n}) = \langle x^m\rangle$.

(b) Let $d(M_{m,n})$ be the minimum number of generators of $M_{m,n}$ as an $F[x]$-module. Prove that $d(M_{m,n}) = \dim_F(M_{m,n}/x M_{m,n}) = m + n - 1$.
