...
1. Consider the polynomial $f(x) = x^5 - 7 \in \QQ[x]$. By Eisenstein's criterion at the prime $p = 7$, $f(x)$ is irreducible over $\QQ$. Since $\alpha$ is a root of $f(x)$, the minimal polynomial of $\alpha$ over $\QQ$ is $x^5 - 7$, so $[\QQ(\alpha) : \QQ] = 5$.

2. Similarly, consider $g(x) = x^4 - 5 \in \QQ[x]$. By Eisenstein's criterion at the prime $p = 5$, $g(x)$ is irreducible over $\QQ$, so $[\QQ(\beta) : \QQ] = 4$.
