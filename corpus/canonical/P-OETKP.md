...
**(a) Constructing $g \in I$:** Define $g = f^2 = f \cdot f$.
...
**(b) $Z(I)$ is non-empty for any proper ideal $I \subsetneq R$:** Suppose towards a contradiction that $Z(I) = \emptyset$.
...
For any $x \in [0, 1]$, $x \in U_{a_j}$ for some $j \in \{1, \ldots, k\}$, so $g_{a_j}(x) > 0$.
Since all $g_{a_i} \geq 0$, we have $h(x) > 0$ for all $x \in [0, 1]$.
...
**(c) Maximal ideals of $C[0, 1]$ are point-evaluations:** Let $I \subsetneq R$ be a maximal ideal.
