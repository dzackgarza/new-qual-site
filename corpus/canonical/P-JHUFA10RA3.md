...
Let $Tf(x) = \int_{\mathbb{R}^n} K(x,y) f(y) \, dy$, where $K(x,y)$ is a nonnegative measurable function on $\mathbb{R}^n \times \mathbb{R}^n$. Suppose that there are measurable functions $p(x) > 0$ and $q(x) > 0$ on $\mathbb{R}^n$ and real numbers $\alpha, \beta > 0$ for which
