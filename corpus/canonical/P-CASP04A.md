...
(b) Let $(X, d)$ be a metric space. If a Cauchy sequence $\{x_n\}_{n=1}^{\infty}$ in $X$ has a convergent subsequence, then $\{x_n\}_{n=1}^{\infty}$ is convergent.

(c) Let $\gamma_+(t) = e^{i\pi t}$ and $\gamma_-(t) = e^{-i\pi t}$, for $t \in [0, 1]$. Then $\int_{\gamma_+} \frac{2}{(2z-1)^2}\,dz = \int_{\gamma_-} \frac{2}{(2z-1)^2}\,dz$.

(d) Let $f_n \in \mathcal{O}(G)$ be a sequence such that $f_n(G) \subset \mathbb{D} \setminus \{0\}$ for $n = 1, 2, \ldots$, and assume that $f_n \to f$ in $\mathcal{O}(G)$. Then either $f$ is constant or $f(G) \subset \mathbb{D} \setminus \{0\}$.
