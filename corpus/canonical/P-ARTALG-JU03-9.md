---
schema: qual/card@1
id: P-ARTALG-JU03-9
kind: problem
title: Solvability of S_4 and solvability by radicals
classification:
  areas:
  - algebra
  topics: []
solved: false
relations: []
---

(a) Given a group $G$, state the criteria that make $G$ a solvable group. Use your definition to prove that $S_4$ is solvable.

(b) Let $g(x) \in \mathbb{Q}[x]$ have degree 4. Prove or disprove: $g(x)$ is not solvable by radicals.

(c) Let $g(x) \in \mathbb{Q}[x]$ have degree 5. Prove or disprove: $g(x)$ is not solvable by radicals.
