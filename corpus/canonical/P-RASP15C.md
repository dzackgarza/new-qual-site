...
::: problem
Let $X$ be a locally compact Hausdorff space. Let $Y$ be a closed subspace and $\mu$ be a Radon measure on $Y$. Define a linear functional on $C_c(X)$ by $I(f) = \int_Y (f|_Y)\,d\mu$.

Prove that:
(i) $I(f)$ is a positive linear functional;
(ii) The functional $I(f) = \int_X f\,d\nu$ induces a Radon measure $\nu$ (via the Riesz-Markov theorem) which satisfies $\nu(E) = \mu(E \cap Y)$.
