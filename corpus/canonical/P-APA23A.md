...
(b) Prove that for $A, B \in M_n$, if $x^H A x = x^H B x$ for all $x \in \mathbb{C}^n$, then $A = B$. Give an example for which $x^T A x = x^T B x$ for all $x \in \mathbb{C}^n$ but $A \neq B$.
:::
