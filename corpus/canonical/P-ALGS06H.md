---
schema: qual/card@1
id: P-ALGS06H
kind: problem
title: "Cyclic multiplicative group and existence of irreducible polynomials over finite fields"
classification:
  areas:
  - algebra
  topics:
  - Field Theory
relations: []
review: draft
solved: false
---

::: problem
(a) If $\mathbb{F}_q$ is a field with $q$ elements, show that $\mathbb{F}_q^\times$ is a cyclic group.

(b) Show that for each integer $n \geq 1$, there exists an irreducible polynomial over $\mathbb{F}_q$ of degree $n$.

(c) Consider the map $\phi: \mathbb{F}_{q^n} \to \mathbb{F}_{q^n}$ given by $\phi(x) = x^q$. Note that $\phi$ is an $\mathbb{F}_q$-linear endomorphism of the $\mathbb{F}_q$-vector space $\mathbb{F}_{q^n}$. Find the characteristic and minimal polynomials of $\phi$.
:::
