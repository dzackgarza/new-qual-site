---
schema: qual/card@1
id: P-JHUSP03CAA
kind: problem
title: Meromorphic function on Riemann sphere is rational
classification:
  areas:
  - complex-analysis
  topics: []
solved: false
relations: []
---

(a) Let $f : \mathbb{C} \to \mathbb{C}$ be meromorphic with a pole at infinity. Show that $f$ must be a rational function.

(b) Use the above to prove the following: if $f : \Delta \to \mathbb{C}$ is holomorphic with a continuous extension to the boundary of $\Delta$ such that $|f(z)| = 1$ for all $|z| = 1$, then $f(z)$ is the restriction of a rational function.
