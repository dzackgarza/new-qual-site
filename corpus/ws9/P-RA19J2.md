...
id: P-RA19J2
kind: problem
title: 'UGA analysis qualifying exam, January 2019, problem 2'
classification:
  areas:
  - real-analysis
  topics:
...
:::{.problem title="?"}
Consider the following subset of the metric space $(C_b([0,1]),\rho_\infty)$:
$$A:=\{f\in C_b([0,1]):f([0,1])\subseteq[0,1]\}.$$
...
**Goal:** For $A = \{f \in C_b([0,1]) : f([0,1]) \subseteq [0,1]\}$ in $(C_b[0,1], \rho_\infty)$: (a) bounded? diameter? (b) closed? (c) compact?
...
<1>2. (b) $A$ is closed in $C_b[0,1]$.
    ...
    <2>3. Q.E.D.
        Proof: <2>2 shows $f([0,1]) \subseteq [0,1]$, so $f \in A$; $A$ is closed.
...
<1>3. (c) $A$ is NOT compact.
    ...
    <2>4. Q.E.D.
        Proof: <2>2–<2>3 show $A$ is not compact.
