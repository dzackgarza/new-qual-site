...
:::: {.solution}
> **AI-Generated Solution** **Goal:** Prove $\int_a^b f(x)\,d\alpha(x) = \int_a^b f(x)\alpha'(x)\,dx$ when $f \in \mathcal R[a,b]$ and $\alpha \in C^1[a,b]$.
...
<2>1. $\sum_i f(t_i)\Delta\alpha_i = \sum_i f(t_i)\alpha'(s_i)\Delta x_i$ for suitable $s_i \in [x_{i-1}, x_i]$ (Mean Value Theorem applied to $\alpha$ on each subinterval, using $\Delta\alpha_i = \alpha(x_i) - \alpha(x_{i-1}) = \alpha'(s_i)\Delta x_i$). Proof: MVT for $\alpha \in C^1$ on $[x_{i-1}, x_i]$.
...
<2>3. Q.E.D. Proof: <2>2 shows the Stieltjes sum with tags $t_i$ differs from $\sum f(t_i)\alpha'(t_i)\Delta x_i$ (a Riemann sum for $f\alpha'$ with the same partition and tags) by $\to 0$.
Since the latter converges to $\int_a^b f\alpha'$ (Riemann integrability, <1>1), the former converges to the same value.
