---
schema: qual/card@1
id: P-EHBDD
kind: problem
title: In this question, $R$ is a ring and $e \in R$ is an idempotent, so tha…
classification:
  areas:
  - algebra
  topics:
  - semisimplicity
  - jacobson-radical
  - rings
relations: []
review: draft
---

:::{.problem title="?"}
In this question, $R$ is a ring and $e \in R$ is an idempotent, so that $eRe$ is another ring with identity element $e$.

a. What does it mean to say that $R$ is *semisimple*? State the Artin-Wedderburn Theorem.
b. If $V$ is a completely reducible $R$-module of finite length, show that the algebra $\operatorname{End}_R(V)$ is semisimple. Deduce for a semisimple ring $R$ that $eRe$ is semisimple too.
c. Assuming that $R$ is left Artinian, show that $J(eRe) = eJ(R)e$, where $J$ denotes Jacobson radical.
:::
