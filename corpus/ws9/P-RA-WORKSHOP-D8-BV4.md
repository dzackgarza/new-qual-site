...
:::{.problem title="?"}
(January 2020, 6a) Let $\{a_n\}_{n=1}^{\infty}\subseteq\mathbb R$ and a strictly increasing sequence
$\{x_n\}_{n=1}^{\infty}\subseteq(0,1)$ be given. Assume that $\sum_{n=1}^{\infty}a_n$ is absolutely convergent,
and define $\alpha:[0,1]\to\mathbb R$ by
$$
\alpha(x):=
\begin{cases}
a_n,&x=x_n,\\
...
**Goal:** Prove or disprove: $\alpha(x) = a_n$ at $x = x_n$, else $0$, has bounded variation on $[0,1]$, given $\sum|a_n| < \infty$.
...
<1>4. Q.E.D.
    Proof: <1>3 shows $\alpha$ has finite total variation, so $\alpha \in BV[0,1]$. (The factor $2$ is loose but harmless.)
