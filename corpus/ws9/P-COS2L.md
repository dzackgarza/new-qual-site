---
schema: qual/card@1
id: P-COS2L
kind: problem
title: Let $p_1,\dots,p_n$ be distinct points in $\mathbb{C}$ and let $U$ be…
classification:
  areas:
  - real-analysis
  topics:
  - harmonic-functions
relations: []
review: draft
---

:::{.problem title="?"}
Let $p_1,\dots,p_n$ be distinct points in $\mathbb{C}$ and let $U$ be the domain $\mathbb{C}\setminus\{p_1,\dots,p_n\}$. Let $A$ be the vector space of real harmonic functions on $U$ and let $B\subseteq A$ be the subspace of real parts of complex analytic functions on $U$. Find the dimension of the quotient space $A/B$ and give a basis.
:::
