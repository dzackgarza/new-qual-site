...
<1>2. $B_Nf$ vanishes at both endpoints: $B_Nf(0) = f(0) = 0$ and $B_Nf(1) = f(1) = 0$.
    Proof: in the sum, at $x = 0$ only the $k = 0$ term survives and equals $f(0)$; at $x = 1$ only the $k = N$ term survives and equals $f(1)$. Both are $0$ by hypothesis.

<1>3. Every polynomial $p$ with $p(0) = p(1) = 0$ is divisible by $x(1-x)$: $p(x) = x(1-x)q(x)$ for a polynomial $q$.
    Proof: $p(0) = 0$ means the constant term of $p$ is $0$, so $p(x) = x\tilde p(x)$; $p(1) = 0$ means $\tilde p(1) = 0$, so $\tilde p(x) = (1-x)q(x)$. Since $x$ and $1-x$ are coprime, $p = x(1-x)q$.
...
<1>5. Q.E.D.
    Proof: by <1>1, $x(1-x)q_N(x) = B_Nf(x) \to f(x)$ uniformly; take $P_N := q_N$.
