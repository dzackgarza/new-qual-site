...
<1>2. Pointwise equicontinuity implies uniform equicontinuity.
    <2>1. Fix $\varepsilon > 0$. For each $x \in K$ choose $\delta_x > 0$ such that $\|x - y\| < \delta_x \Rightarrow \|f(x) - f(y)\| < \varepsilon/2$ for all $f \in \mathcal F$.
    ...
    <2>3. $K$ is covered by finitely many of them: $K \subseteq \bigcup_{i=1}^n B(x_i, \delta_{x_i}/2)$.
        Proof: compactness of $K$ applied to the open cover of <2>2.
    <2>4. Set $\delta := \min_i \delta_{x_i}/2 > 0$. If $\|x - y\| < \delta$, then for every $f \in \mathcal F$, $\|f(x) - f(y)\| < \varepsilon$.
        Proof: $x$ lies in some $B(x_i, \delta_{x_i}/2)$ by <2>3, so $\|x - x_i\| < \delta_{x_i}/2$; then $\|y - x_i\| \le \|y - x\| + \|x - x_i\| < \delta + \delta_{x_i}/2 \le \delta_{x_i}/2 + \delta_{x_i}/2 = \delta_{x_i}$ since $\delta \le \delta_{x_i}/2$. Both $x$ and $y$ are within $\delta_{x_i}$ of $x_i$, so $\|f(x) - f(x_i)\| < \varepsilon/2$ and $\|f(y) - f(x_i)\| < \varepsilon/2$, hence $\|f(x) - f(y)\| \le \|f(x)-f(x_i)\| + \|f(y)-f(x_i)\| < \varepsilon$ by the triangle inequality. This holds for all $f \in \mathcal F$ simultaneously, and $\delta$ is independent of $x, y, f$.
    <2>5. Q.E.D.
        Proof: <2>4 is uniform equicontinuity.
