...
2. For any $x\in E$ and $\epsilon>0$ there exists some $\delta>0$ such that
   $\lVert f(y)-f(x)\rVert<\epsilon$ for all $y\in E$ with $\lVert x-y\rVert<\delta$.
3. For every convergent sequence $\{x_n\}\subset E$ with
   $\lim_{n\to\infty}x_n=x\in E$, $\lim_{n\to\infty}f(x_n)=f(x)$.
