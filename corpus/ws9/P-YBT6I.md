...
**Goal:** (a) For $f, g \in L^2(\mathbb R)$: $f \ast g$ is bounded and continuous; (b) for $h \in L^1$: $A(f) = f \ast h$ is a bounded operator $L^1 \to L^1$.
...
<1>2. (a) $f \ast g$ is continuous.
    ...
    <2>4. Q.E.D.
        Proof: <2>2–<2>3 show $f \ast g$ is continuous.
...
<1>3. (b) $A(f) = f \ast h$ maps $L^1 \to L^1$ and is bounded with $\|A\| \le \|h\|_1$.
    ...
    <2>3. $\|f \ast h\|_1 \le \|f\|_1\|h\|_1$: $A$ is bounded with $\|A\| \le \|h\|_1$ (indeed $= \|h\|_1$).
        Proof: <2>1–<2>2; the operator norm is exactly $\|h\|_1$ (Young's inequality with $p = q = 1$, $r = 1$).
    <2>4. Q.E.D.
        Proof: <2>3 shows $A$ is a bounded linear operator $L^1 \to L^1$.
