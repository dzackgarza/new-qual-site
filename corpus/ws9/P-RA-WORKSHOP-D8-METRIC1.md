...
**Goal:** Find a metric space $X$ and a closed bounded subset $E \subseteq X$ that is not compact.
...
<1>4. Q.E.D.
    Proof: <1>1–<1>3 exhibit the required example. (Alternative: $X = \mathbb R$ with the discrete metric, $E = X$: $E$ is closed and bounded — bounded in the discrete metric since $d \le 1$ — but not compact, as the cover by singletons has no finite subcover.)
