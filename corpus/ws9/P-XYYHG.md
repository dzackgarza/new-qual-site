...
<2>1. Conjugate by the Cayley map $C(z) = \frac{z - i}{z + i}: \mcu \to D$.
    Proof: $C$ is a biholomorphism (its inverse is $C^{-1}(w) = i\frac{1 + w}{1 - w}$).
<2>2. $g := C \circ f \circ C^{-1} \in \Aut(D)$.
...
<1>3. Q.E.D.
    Proof: <1>1 and <1>2 show $\Aut(\mcu) = \ts{z \mapsto \frac{az+b}{cz+d} \st a,b,c,d \in \RR,\, ad - bc > 0}$, i.e. $\PSL_2(\RR)$.
:::
