...
**Goal:** Determine the $x \in \mathbb R$ for which $\sum_{n=1}^\infty \frac{x^n}{1 + n|x|^n}$ converges.
...
<1>5. For $x < -1$: conditional convergence.
    ...
    <2>4. Q.E.D.
        Proof: <2>3 establishes conditional convergence for every $x < -1$.

<1>6. Q.E.D.
    Proof: <1>1–<1>5: the series converges absolutely for $|x| < 1$, conditionally for $x \le -1$, and diverges for $x \ge 1$ (at $x = 1$ and for $x > 1$). Domain: $(-\infty, 1)$.
