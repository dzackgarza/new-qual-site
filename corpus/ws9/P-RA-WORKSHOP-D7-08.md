...
**Goal:** Prove $\lim_{n\to\infty}\int_0^1 f(x^n)\,dx = f(0)$ for continuous $f: [0,1] \to \mathbb R$.
...
<1>2. The integrands are uniformly bounded.
    Proof: $f$ is continuous on the compact interval $[0,1]$, hence bounded: $|f| \le M$ for some $M$. Then $|f(x^n)| \le M$ for all $n$ and all $x$.
...
<1>4. Q.E.D.
    Proof: <1>3 is exactly the claim.
