...
<1>1. (⟸) Suppose the subsequence condition holds. Show $A$ is compact.
    Proof: let $\{x_n\}$ be a bounded sequence in $X$. By the hypothesis there are a subsequence $\{x_{n_j}\}$ and a vector $\phi \in X$ with $x_{n_j} = \phi + r_{n_j}$ and $Ar_{n_j} \to 0$ in $Y$. Then $Ax_{n_j} = A\phi + Ar_{n_j} \to A\phi$, so $\{Ax_{n_j}\}$ converges in $Y$. Every bounded sequence thus has a subsequence whose image under $A$ converges; this is exactly the definition of compactness.
<1>2. (⟹) Suppose $A$ is compact. Let $\{x_n\}$ be bounded. Show there are $x_{n_j}$ and $\phi$ with $x_{n_j} = \phi + r_{n_j}$ and $Ar_{n_j} \to 0$.
...
<2>3. $A\phi = y$.
    Proof: $A$ is bounded, hence continuous from the weak topology of $X$ to the weak topology of $Y$; so $x_{n_{j_k}} \rightharpoonup \phi$ gives $Ax_{n_{j_k}} \rightharpoonup A\phi$. But $Ax_{n_{j_k}} \to y$ strongly, hence weakly, so weak limits coincide: $A\phi = y$.
