---
schema: qual/card@1
id: P-RA-WORKSHOP-D2-METRIC-07
kind: problem
title: 'Compactness of infinite unions and attainment of distance to a compact set'
classification:
  areas:
  - real-analysis
  topics: []
relations: []
review: draft
---

:::{.problem title="?"}
(June 2003, #1b,c)

(b) Show by example that the union of infinitely many compact subsets of a metric space need not be compact.

(c) If $(X,d)$ is a metric space and $K\subset X$ is compact, define
$$d(x_0,K)=\inf_{y\in K}d(x_0,y).$$
Prove that there exists a point $y_0\in K$ such that $d(x_0,K)=d(x_0,y_0)$.
:::
