---
schema: qual/card@1
id: P-PQG5I
kind: problem
title: "Let $A = \\left\\{x\\in\\ell^2: \\sum_{n\\ge1} n|x_n|^2 \\le 1\\right\\}$ Show that $A$ is compact in the $\\ell^2$ topology."
classification:
  areas:
  - real-analysis
  topics: []
relations: []
review: draft
---

:::{.problem title="?"}
Let
$$A = \left\{x\in\ell^2: \sum_{n\ge1} n|x_n|^2 \le 1\right\}.$$

a. Show that $A$ is compact in the $\ell^2$ topology.

b. Show that the mapping from $A$ to $\mathbb{R}$ defined by
$$x \mapsto \int_0^{2\pi} \left|\sum_{n\ge1} x_n e^{in\theta}\right| \frac{d\theta}{2\pi}$$
achieves its maximum on $A$.
:::
