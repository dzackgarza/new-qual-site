...
id: P-AF5PR
kind: problem
title: "Show that $\\displaystyle \\int_0^\\infty \\frac{x^{a-1}}{1+x^n} dx=\\frac{\\pi}{n\\sin \\frac{a\\pi}{n}}$ using complex analysis, $0< a < n$. Here $n$ is a positive\u2026"
classification:
  areas:
  - complex-analysis
  topics:
...
<1>6. Solve for the integral.
    Proof: From <1>5, $\int_0^\infty \frac{x^{a-1}}{1+x^n}\, dx = \frac{2\pi i}{n} \frac{e^{i(a-n)\pi/n}}{1 - e^{2\pi i a/n}}$. Since $1 - e^{2\pi i a/n} = -2i e^{\pi i a/n} \sin\frac{a\pi}{n}$ and $e^{i(a-n)\pi/n} e^{-i a\pi/n} = e^{-i\pi} = -1$, this simplifies to $\frac{2\pi i}{n} \cdot \frac{-e^{i a\pi/n}}{-2i e^{i a\pi/n} \sin\frac{a\pi}{n}} = \frac{\pi}{n \sin\frac{a\pi}{n}}$.

<1>7. Q.E.D.
    Proof: <1>6 establishes the claimed formula for all $0 < a < n$.
