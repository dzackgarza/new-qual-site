...
Proof: Rouch\'e's theorem with $f(z) = -6z + 3$ and $g(z) = z^4$, using <2>1. <2>3. $-6z + 3$ has exactly one zero in $\abs z < 1$.
...
Proof: Rouch\'e's theorem with $f(z) = z^4$ and $g(z) = -6z + 3$, using <2>1. <2>3. $z^4$ has exactly four zeros in $\abs z < 2$.
Proof: $z = 0$ is a zero of multiplicity 4. <2>4. Conclusion.
Proof: <2>2 and <2>3.
...
<1>6. Q.E.D. Proof: <1>2 gives one root in $\abs z < 1$; <1>5 gives three roots in $1 < \abs z < 2$.
:::
