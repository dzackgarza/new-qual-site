...
id: P-ZAS5Z
kind: problem
title: "Define $f(x, y):=\\left\\{\\begin{array}{ll}{\\frac{x^{1 / 3}}{(1+x y)^{3 / 2}}} & {\\text { if } 0 \\leq x \\leq y} \\\\ {0} & {\\text { otherwise }}\\end{array}\\right.$ Carefully show that $f \\in L^1(\\RR^2)$."
classification:
  areas:
  - real-analysis
  topics:
...
<1>5. Q.E.D.
    Proof: <1>1, <1>2, and <1>4 show $\int|f| < \infty$ with the value $2\int_0^\infty \frac{x^{-2/3}}{(1+x^2)^{1/2}}\,dx$, so $f \in L^1(\RR^2)$.
:::
