...
<1>2. Compute $\overline{f(\bar z)}$ for $z$ near $\bar z_0$.
    ...
    <2>2. The series $\sum \bar a_n (z - \bar z_0)^n$ converges for $\abs{z - \bar z_0} < r$, where $r$ is the radius of convergence of <1>1.
        Proof: $\abs{z - \bar z_0} = \abs{\bar z - z_0}$, so the set of convergence is the mirror image of that of <1>1.
...
<1>4. Q.E.D.
    Proof: $\bar z_0$ was an arbitrary point of $\overline{\Omega}$, so $g$ is analytic throughout $\overline{\Omega}$.
