...
<1>2. The claim holds for non-negative simple functions.
    ...
    <2>3. $\int f_\delta = \sum_i a_i \int (\chi_{E_i})_\delta = \sum_i a_i m(E_i) = \int f$.
        Proof: linearity of the integral and <2>2.
    <2>4. Q.E.D.
        Proof: <2>3.
...
<1>3. The claim holds for all non-negative measurable $f$.
    ...
    <2>3. $\int f_\delta = \lim_k \int (s_k)_\delta = \lim_k \int s_k = \int f$.
        Proof: monotone convergence applied to <2>2, then <1>2 for each $s_k$, then monotone convergence applied to <2>1.
    <2>4. Q.E.D.
        Proof: <2>3.
...
<1>5. Q.E.D.
    Proof: <1>3 covers non-negative measurable $f$ and <1>4 covers signed integrable $f$; the identity is exactly the change of variables $y = x/\delta$, $dx = \delta^n\,dy$.
:::
