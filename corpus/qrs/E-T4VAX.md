...
<1>2. On $\abs{z} = 2$: $\abs{g(z)} < \abs{f(z)}$.
    Proof: $\abs{g(z)} \le 3\abs{z} + 1 = 7$ on $\abs{z} = 2$, while $\abs{f(z)} = \abs{z}^5 = 32$. Since $7 < 32$, the strict inequality holds everywhere on the circle.
...
<1>5. Q.E.D.
    Proof: <1>3 and <1>4 give that $h$ has exactly $5$ zeros in $\abs{z} < 2$, hence in $\abs{z} \le 2$.
