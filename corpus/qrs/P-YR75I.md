...
**Goal:** Suppose $(f_n)$ is a sequence of entire functions with $f_n \to g$ pointwise, and $f_n \to g$ uniformly on every line segment in $\CC$. Show that $g$ is entire and $f_n \to g$ uniformly on every compact subset of $\CC$.
...
<1>3. Q.E.D.
    Proof: <1>1 shows $g$ is entire; <1>2 shows uniform convergence on compacta.
:::
