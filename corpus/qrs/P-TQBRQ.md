...
Proof: By the maximum modulus principle applied to $f$ on $\DD$, since $\abs f = 1$ on the boundary $\abs{z} = 1$.

<1>2. $\abs{f(z)} \geq 1$ for $z \in \DD$.
Proof: $f$ is nonvanishing on $\bar\DD$, so $1/f$ is holomorphic on a neighborhood of $\bar\DD$; the maximum modulus principle applied to $1/f$ gives $\abs{1/f(z)} \leq \max_{\abs{\zeta}=1}\abs{1/f(\zeta)} = 1$, i.e. $\abs{f(z)} \geq 1$.

<1>3. $\abs{f(z)} = 1$ for all $z \in \DD$.
Proof: Combine <1>1 and <1>2.
...
Proof: For $\abs{z} > 1$, $1/\bar z$ lies in $\DD \setminus \theset{0}$ (indeed $\abs{1/\bar z} = 1/\abs z < 1$), where $f \neq 0$ by assumption, so the formula makes sense.
...
Proof: On $\abs{z} < 1$, $F = f$ is holomorphic.
...
Proof: On $\abs z \leq 1$, $\abs F = \abs f \leq 1$ by <1>1 (or by compactness, $f$ bounded).
...
Proof: By Liouville's theorem, the bounded entire function $F$ from <1>5 and <1>6 is constant; restricting to $\DD$ shows $f$ is constant.

<1>8. Q.E.D. Proof: <1>7 proves the claim.
:::
