...
1. **Continuity of the quotient map:**
   By the definition of the quotient topology, a subset $V \subseteq Y$ is open in $Y$ if and only if $q^{-1}(V)$ is open in $X$.
...
2. **Continuous image of a compact space is compact:**
   Let $\mathcal{U} = \{V_\alpha\}_{\alpha \in A}$ be an open cover of $Y$:
   $$
...
3. **Extracting a finite subcover:**
   Since $X$ is compact, there exists a finite subcover $\{U_{\alpha_1}, U_{\alpha_2}, \ldots, U_{\alpha_k}\}$ of $X$, so:
   $$
