...
<1>4. A.e. convergence need not be pointwise.
    <2>1. Take $f_n = \chi_{[0, 1/n)}$ on $[0,1]$.
        Proof: standard example.
    <2>2. $f_n \to 0$ a.e.
        Proof: for every $x > 0$, $f_n(x) = 0$ once $1/n \le x$, so $f_n(x) \to 0$; only the null set $\{0\}$ is excluded.
...
<1>5. $\sum_{n=0}^\infty \frac{x^n}{n!}$ converges uniformly on every compact subset of $\RR$.
    ...
    <2>2. $\sum_{n=0}^\infty \frac{M^n}{n!} = e^M < \infty$.
        Proof: the exponential series converges, e.g. by the ratio test ($\frac{M^{n+1}/(n+1)!}{M^n/n!} = \frac{M}{n+1} \to 0 < 1$).
    <2>3. Q.E.D.
        Proof: Weierstrass M-test from <2>1 and <2>2.
:::
