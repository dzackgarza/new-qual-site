...
**Goal:** Prove that if $f: D(0, R) \to \CC$ is holomorphic with $\abs{f(z)} \leq M$ for some $M > 0$, then
$$\abs{\frac{f(z) - f(0)}{M^2 - \overline{f(0)} f(z)}} \leq \frac{\abs z}{MR} \quad (z \in D(0,R)).$$
...
<1>3. Substitute $g(w) = f(Rw)/M$ into the inequality.
    ...
    <2>2. $\abs{\frac{M\qty(f(Rw) - f(0))}{M^2 - \overline{f(0)} f(Rw)}} \leq \abs w$.
        Proof: <1>2 and <2>1.
    <2>3. $\abs{\frac{f(Rw) - f(0)}{M^2 - \overline{f(0)} f(Rw)}} \leq \frac{\abs w}{M}$.
...
<1>5. Q.E.D.
    Proof: <1>4 is the claim. (When $f$ is constant, both sides vanish and the inequality is trivial; otherwise $\abs{f(0)} < M$ by the maximum modulus principle, so the denominator $M^2 - \overline{f(0)} f(z)$ is nonzero and the Schwarz--Pick form in <1>2 is well-defined.)
