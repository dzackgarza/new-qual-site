...
Proof: For $z = x + iy \in S$, $e^{\pi z} = e^{\pi x} e^{i\pi y}$; since $y \in (0,1)$, the argument $\pi y$ lies in $(0, \pi)$ and the modulus $e^{\pi x}$ is any positive number, so $e^{\pi z}$ ranges over all of $\HH$ (indeed over all nonzero complex numbers with argument in $(0,\pi)$).
...
<1>5. Q.E.D. Proof: <1>4 gives the conformal map $z \mapsto e^{\pi z}$ from the strip to the upper half-plane.
:::
