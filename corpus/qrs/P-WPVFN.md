...
<1>3. $g = 0$ a.e.
    <2>1. Apply <1>2 to $h = \mathrm{sgn}(g) \in L^1$: $\int g\,\mathrm{sgn}(g)\,dx = 0$.
        Proof: $\Lambda(\mathrm{sgn}(g)) = 0$ by <1>2.
    <2>2. $\int |g|\,dx = 0$, so $g = 0$ a.e.
        Proof: $g\,\mathrm{sgn}(g) = |g|$ everywhere (with $\mathrm{sgn}(0) = 0$).
    <2>3. Q.E.D.
        Proof: <2>1 and <2>2.
:::
