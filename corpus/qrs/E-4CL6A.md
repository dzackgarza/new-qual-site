...
2. **Diagonal subsequence:**
   Consider the diagonal sequence $(z_j)_{j=1}^\infty$ defined by $z_j = x_{j, j}$.
...
3. **Cauchy sequence and convergence:**
   Given any $\varepsilon > 0$, choose $m \in \mathbb{N}$ such that $\frac{2}{m} < \varepsilon$.
...
4. **Conclusion:**
   $(z_j)$ is a subsequence of the original sequence $(x_n)$ that converges in $X$.
