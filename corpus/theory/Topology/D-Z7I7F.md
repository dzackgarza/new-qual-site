...
The maps $f$ and $g$ are \dfn{homotopic}, written $f\homotopic g$, if there is a continuous map $H\colon X\cross I \to Y$ with $H(x, 0) = f(x)$ and $H(x, 1) = g(x)$ for all $x\in X$; such a map $H$ is a homotopy from $f$ to $g$.
...
The set of homotopy classes of continuous maps $X\to Y$ is denoted $[X,Y]$.
:::
