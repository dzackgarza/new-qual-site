...
In an abelian category $\mca$, a **complex** $A^\bullet$ is a family of objects $A^i$, $i \in \ZZ$, with maps $\delta^i: A^i \to A^{i+1}$ such that $\delta^{i+1}\circ \delta^i = 0$ for all $i$; equivalently $\im \delta^i \subseteq \ker \delta^{i+1}$.
...
Morphisms of complexes $f, g: A^\bullet \to B^\bullet$ are **homotopic**, written $f \sim g$, if there are maps $h^i: A^i \to B^{i-1}$ with
\[
