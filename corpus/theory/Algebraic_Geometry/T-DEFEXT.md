...
2. An automorphism of $X \times D$ restricting to the identity is, on each affine $\Spec A$, a ring map $a \mapsto a + \varepsilon\, \delta(a)$ of $A[\varepsilon]$; it is a ring homomorphism exactly when $\delta$ is a $k$-derivation, and derivations $A \to A$ are $\Hom_A(\Omega_{A/k}, A)$. These glue to $H^0(X, T_X)$.
