...
For (b), $A_kB_k-A_{k-1}B_{k-1}=A_k(B_{k-1}+b_k)-(A_k-a_k)B_{k-1}=A_kb_k+a_kB_{k-1}$ for every $k\ge1$; summing over $1\le k\le n$ and using $A_0B_0=0$ gives the identity.
:::
