...
Proof: monotonicity gives $\mu^*(E_j) \ge \mu^*(E)$.
...
Proof: if $f = 0$ a.e., then $f\chi_E = 0$ a.e., so $\int_E f = 0$.
...
Proof: $\int f = 0$ and $\int f \ge \frac{1}{n}\mu\{f \ge 1/n\}$ force $\mu\{f\ge 1/n\} = 0$ for every $n$; conversely $f = 0$ a.e. gives $\int f = 0$.
...
Proof: if $f$ is measurable and $|f| \le M$, then $|f| \le M\chi_X \in L^1$, so $f \in L^1$.
...
Proof: write $f = f^+ - f^-$ with $f^\pm \ge 0$; it suffices to approximate $g \in L^1$, $g \ge 0$.
...
Proof: it suffices (by <1>7) to approximate $\chi_E$ for measurable $E$ with $m(E) < \infty$ by step functions.
