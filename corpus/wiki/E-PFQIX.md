...
If $A$ and $B$ are subsets of the topological group $G$, let $A \cdot B$ denote the set of all points $a \cdot b$ for $a \in A$ and $b \in B$. Let $A^{-1}$ denote the set of all points $a^{-1}$, for $a \in A$.
...
(d) Let $H$ be a subgroup of $G$ that is closed in the topology of $G$; let $p: G \to G/H$ be the quotient map. Show that $G/H$ satisfies the regularity axiom. [Hint: Examine the proof of (c) when $A$ is saturated.]

::: {.remark}
Munkres, *Topology*, §22 Supplementary Exercises (Topological Groups) Exercise 7.
:::
