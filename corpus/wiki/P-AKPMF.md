...
<1>2. Case $1 < p < \infty$: define $g(x) = \dfrac{\mathrm{sgn}(f(x))\,|f(x)|^{p-1}}{\|f\|_p^{p/q}}$ (and $g = 0$ where $f = 0$).
    <2>1. $\|g\|_q^q = \dfrac{\int |f|^{q(p-1)}}{\|f\|_p^{pq/q}} = \dfrac{\int |f|^p}{\|f\|_p^{p}} = 1$, since $q(p-1) = p$.
...
<1>4. Case $p = \infty$ (so $q = 1$).
    <2>1. For $0 < \eps < \|f\|_\infty$, the set $\{x : |f(x)| \ge \|f\|_\infty - \eps\}$ has positive measure and contains a measurable subset $E$ with $0 < m(E) < \infty$ (intersect with a large ball).
    ...
    <2>2. Define $g = \dfrac{\mathrm{sgn}(f)\,\chi_E}{m(E)}$; then $\|g\|_1 = 1$ and $\left|\int f g\right| = \dfrac{1}{m(E)}\int_E |f| \ge \|f\|_\infty - \eps$.
        Proof: normalization by $m(E)$; the bound uses <2>1.
    <2>3. Q.E.D. for $p = \infty$.
        Proof: <2>2 gives $\sup_{\|g\|_1 = 1}\left|\int fg\right| \ge \|f\|_\infty - \eps$ for every $\eps > 0$; with <1>1 the sup equals $\|f\|_\infty$.

<1>5. Q.E.D.
    Proof: <1>2, <1>3, and <1>4 cover all conjugate pairs $1 \le p \le \infty$.
:::
