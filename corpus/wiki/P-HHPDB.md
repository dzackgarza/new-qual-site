...
<1>2. Pointwise identity:
    $$(f \ast \phi_t)(x) - f(x) = \int \phi_t(y)\,(f(x-y) - f(x))\,dy = \int \phi(y)\,(f(x - ty) - f(x))\,dy.$$
    Proof: $f \ast \phi_t = \int f(x-y)\phi_t(y)\,dy$; subtract $f(x)\int\phi_t = f(x)$ (by <1>1); the second equality is the change of variables $y \mapsto ty$.

<1>3. Given $\eps > 0$:
    <2>1. Choose $R$ with $\int_{|z| > R}|\phi(z)|\,dz < \dfrac{\eps}{4\|f\|_\infty}$ (possible since $\phi \in L^1$; take $\|f\|_\infty = 0$ as trivial).
...
<1>4. For $t < \eta/R$ and every $x$: $|(f \ast \phi_t)(x) - f(x)| < \eps$.
    <2>1. Split: $|(f\ast\phi_t)(x) - f(x)| \le \int_{|y| \le R/t}|\phi_t(y)|\,|f(x-y) - f(x)|\,dy + \int_{|y| > R/t}|\phi_t(y)|\,|f(x-y) - f(x)|\,dy$.
        Proof: triangle inequality applied to <1>2.
    <2>2. First term: $\le \dfrac{\eps}{2\|\phi\|_1}\int_{|y| \le R/t}|\phi_t(y)|\,dy \le \dfrac{\eps}{2}$.
    ...
    <2>3. Second term: $\le 2\|f\|_\infty \int_{|y| > R/t}|\phi_t(y)|\,dy = 2\|f\|_\infty \int_{|z| > R}|\phi(z)|\,dz < \dfrac{\eps}{2}$.
        Proof: $|f(x-y) - f(x)| \le 2\|f\|_\infty$; change variables $z = ty$; the bound is <1>3<2>1.
    <2>4. Q.E.D.
        Proof: <2>1, <2>2, <2>3 give $|(f\ast\phi_t)(x) - f(x)| < \eps$ for all $x$, i.e. $\|f\ast\phi_t - f\|_\infty < \eps$.

<1>5. Q.E.D.
    Proof: <1>4 shows $f \ast \phi_t \to f$ uniformly as $t \to 0$. (Only boundedness and uniform continuity of $f$ are used.)
:::
