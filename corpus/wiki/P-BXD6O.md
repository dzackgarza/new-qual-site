...
Proof: triangle inequality inside the integral, then Cauchy–Schwarz.
<2>2. $\int |g(x-t) - g(y-t)|^2\,dt = \int |g(u) - g(u - (y-x))|^2\,du = \|g - \tau_{y-x} g\|_2^2$, where $\tau_a g(u) \definedas g(u - a)$.
Proof: substitute $u = x - t$; then $y - t = u - (y-x)$; Jacobian is $1$.
...
Proof: $g$ uniformly continuous and compactly supported, so $|g(u) - g(u-a)| \to 0$ uniformly and lives in a fixed compact set for $|a| \le 1$; dominated convergence.
...
Proof: triangle inequality; the last term is $\|q - g\|_2$ by translation-invariance of the norm.
<2>4. For $|a|$ small, $\|q - \tau_a q\|_2 < \eps/3$, so $\|g - \tau_a g\|_2 < \eps$.
Proof: <2>1 applied to $q$, then <2>3.

<1>4. Q.E.D. Proof: <1>2<2>3 and <1>3 show $|h(x) - h(y)| \le \|f\|_2\|g - \tau_{y-x} g\|_2 \to 0$ as $y \to x$, uniformly in $x, y$ (the bound depends only on $y - x$); hence $h$ is uniformly continuous.
:::
