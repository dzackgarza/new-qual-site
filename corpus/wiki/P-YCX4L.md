...
<1>4. Contradiction.
    Proof: $f$ is continuous at $x$, so $f(x_{n_j}) \to f(x)$ and $f(y_{n_j}) \to f(x)$ by <1>2 and <1>3; hence $|f(x_{n_j}) - f(y_{n_j})| \to 0$, contradicting $|f(x_{n_j}) - f(y_{n_j})| \ge \eps > 0$ for all $j$ (from <1>1).
<1>5. Hence $f$ is uniformly continuous on $K$.
