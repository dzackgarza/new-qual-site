...
<1>4. (3) $\lim_{|x| \to \infty} f(x) = 0$.
    Proof: given $\eps > 0$, choose $R$ with $m(E \setminus [-R, R]) < \eps$ (possible since $m(E) < \infty$). For $|x| > 2R$: if $y \in E \cap (E + x)$, then $y \in E$ and $y - x \in E$; if $y \in [-R,R]$, then $|y - x| \ge |x| - |y| > R$, so $y - x \notin [-R, R]$. Hence $E \cap (E+x) \subseteq (E \setminus [-R,R]) \cup ((E + x) \setminus [-R,R])$, and $f(x) = m(E \cap (E+x)) \le m(E\setminus[-R,R]) + m((E+x)\setminus[-R,R]) = m(E\setminus[-R,R]) + m(E \setminus [-R,R] + x) < 2\eps$ (translation invariance).

<1>5. Q.E.D.
    Proof: <1>2, <1>3, <1>4 establish (1), (2), (3).
:::
