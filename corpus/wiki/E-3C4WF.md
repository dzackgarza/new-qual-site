...
(b) Show that if $(X, d)$ is complete, so is $(\mathcal{H}, D)$. [Hint: Let $A_n$ be a Cauchy sequence in $\mathcal{H}$; by passing to a subsequence, assume $D(A_n, A_{n+1}) < 1/2^n$. Define $A$ to be the set of all points $x$ that are the limits of sequences $x_1, x_2, \ldots$ such that $x_i \in A_i$ for each $i$ and $d(x_i, x_{i+1}) < 1/2^i$. Show $A_n \to \overline{A}$.]
