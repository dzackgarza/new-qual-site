...
<1>2. Consider the points $t_k = \frac{2}{(2k+1)\pi} \in (0, 1]$, $k = 0, 1, 2, \ldots$ (the critical points where $\sin(1/t_k) = \pm 1$). Proof: $1/t_k = (2k+1)\pi/2$, so $\sin(1/t_k) = \pm 1$ alternating with $k$; and $t_k \downarrow 0$.

<1>3. At these points $|f(t_k)| = t_k$ (since $|\sin(1/t_k)| = 1$), and $f$ alternates sign: $f(t_k) = (-1)^k t_k$.
Proof: $f(t_k) = t_k \sin((2k+1)\pi/2) = t_k \cdot (-1)^k$.
...
Proof: $f(t_k) = (-1)^k t_k$ and $f(t_{k+1}) = (-1)^{k+1} t_{k+1}$ have opposite signs (and $f(0) = 0$), so $|f(t_k) - f(t_{k+1})| = t_k + t_{k+1}$ exactly; write $t_K = 0$.
...
Proof: the harmonic tail $\sum \frac{1}{2k+1}$ diverges.

<1>6. Q.E.D.: $V_{[0,1]}(f) = \infty$, so $f \notin \text{BV}[0,1]$.
Proof: <1>4 and <1>5 give arbitrarily large finite-variation lower bounds; $f$ is continuous (including at $0$) but has unbounded variation.
:::
