...
<1>1. The antiderivative of $\frac{1}{x\sqrt{x^2-1}}$ for $x > 1$ is $\operatorname{arcsec}(x) + C = \arccos(1/x) + C$.
    Proof:
    <2>1. Make the trigonometric substitution $x = \sec(\theta)$ for $\theta \in (0, \pi/2)$.
    <2>2. Then $dx = \sec(\theta)\tan(\theta) \, d\theta$, and $\sqrt{x^2 - 1} = \sqrt{\sec^2(\theta) - 1} = \tan(\theta)$.
    <2>3. Substituting:
        $$\int \frac{1}{x\sqrt{x^2 - 1}} \, dx = \int \frac{\sec(\theta)\tan(\theta)}{\sec(\theta)\tan(\theta)} \, d\theta = \int 1 \, d\theta = \theta + C = \operatorname{arcsec}(x) + C.$$
...
<1>3. Evaluation:
    $$\int_1^2 \frac{1}{x\sqrt{x^2-1}} \, dx = \lim_{t \to 1^+} [\operatorname{arcsec}(x)]_t^2 = \operatorname{arcsec}(2) - \lim_{t \to 1^+} \operatorname{arcsec}(t) = \frac{\pi}{3} - 0 = \frac{\pi}{3}.$$
    Proof: $\sec(\pi/3) = 2 \implies \operatorname{arcsec}(2) = \pi/3$, and $\sec(0) = 1 \implies \operatorname{arcsec}(1) = 0$. Q.E.D.
:::
