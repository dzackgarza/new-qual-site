...
<1>1. Split the integral into two parts:
    $$\int \frac{x+1}{\sqrt{4-x^2}} \, dx = \int \frac{x}{\sqrt{4-x^2}} \, dx + \int \frac{1}{\sqrt{4-x^2}} \, dx.$$
    Proof: By linearity of the indefinite integral.
...
<1>3. $\int \frac{1}{\sqrt{4-x^2}} \, dx = \arcsin\left(\frac{x}{2}\right) + C_2$.
    Proof:
    <2>1. Substitute $x = 2\sin(\theta)$ for $\theta \in (-\pi/2, \pi/2)$.
...
<1>4. Combining results:
    $$\int \frac{x+1}{\sqrt{4-x^2}} \, dx = -\sqrt{4-x^2} + \arcsin\left(\frac{x}{2}\right) + C.$$
    Proof: By adding <1>2 and <1>3. Q.E.D.
:::
