...
Proof: by <1>2.2.
<2>2. Group the sum by orbits: for an orbit $O$, each $x \in O$ contributes $|G|/|O|$, and there are $|O|$ such $x$, so the orbit contributes $|G|$.
...
Proof: <1>1.3 and <1>3.3.
<2>2. Hence the number of orbits is $\frac{1}{|G|}\sum_{g \in G} |X^g|$.
Proof: divide by $|G|$.

<1>5. Q.E.D.
Proof: <1>4.2 is Burnside's lemma.
:::
