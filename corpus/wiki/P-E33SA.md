...
<1>3. A function cannot be discontinuous precisely on $\RR \setminus \QQ$ (the irrationals).
    ...
    <2>2. $\RR \setminus \QQ$ is not $F_\sigma$.
        Proof: if $\RR\setminus\QQ = \bigcup_n F_n$ with $F_n$ closed, then $\QQ = \bigcap_n (\RR \setminus F_n)$ would be a $G_\delta$. But a countable dense set cannot be a $G_\delta$: if $\QQ = \bigcap_n U_n$ with $U_n$ open, each $U_n$ is dense (it contains $\QQ$), so by the Baire category theorem $\bigcap_n U_n$ is uncountable (indeed comeager), contradicting countability of $\QQ$.
...
<1>5. Q.E.D.
    Proof: <1>1–<1>4 answer each question. (Second bullet: Thomae's function; third: the $F_\sigma$ obstruction; fourth: $nx/(1+n^2x^2)$.)
:::
