...
<1>3. $\inf_{k\in K} d(k,F) > 0$.
    Proof: by <1>2 the infimum is $\delta(k)$ for some $k \in K$. If $\delta(k) = 0$, then there is a sequence $f_n \in F$ with $d(k, f_n) \to 0$, so $f_n \to k$; since $F$ is closed, $k \in F$, contradicting $K \cap F = \emptyset$. Hence $\delta(k) > 0$.
