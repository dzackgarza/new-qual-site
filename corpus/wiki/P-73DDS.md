...
<1>1. Method 1 ($u$-substitution):
    Proof:
    <2>1. Let $u = \sin(x)$. Then $du = \cos(x) \, dx$.
    <2>2. Transforming the integral:
        $$\int \frac{\cos(x)}{\sin^2(x)} \, dx = \int u^{-2} \, du = -u^{-1} + C = -\frac{1}{u} + C.$$
    <2>3. Substituting back $u = \sin(x)$:
        $$-\frac{1}{\sin(x)} + C = -\csc(x) + C.$$

<1>2. Method 2 (Trigonometric identity):
    Proof:
    <2>1. Rewrite the integrand: $\frac{\cos(x)}{\sin^2(x)} = \frac{\cos(x)}{\sin(x)} \cdot \frac{1}{\sin(x)} = \cot(x)\csc(x)$.
    <2>2. Since $\frac{d}{dx}(\csc(x)) = -\csc(x)\cot(x)$, we have:
