...
<1>3. Lower bound: for each $0 < \eps < M$, $\liminf_{p\to\infty}(\int_a^b f^p)^{1/p} \ge M - \eps$.
    Proof: since $f$ is continuous, it attains $M$ at some $x_0 \in [a,b]$. The set $\{f > M - \eps\}$ is open (continuity) and contains $x_0$, so it contains an interval of length $\delta > 0$. Hence
    \[
    \int_a^b f^p \ge (M-\eps)^p \delta, \qquad \text{so} \qquad \Big(\int_a^b f^p\Big)^{1/p} \ge (M-\eps)\,\delta^{1/p} \to M - \eps .
    \]
<1>4. $\lim_{p\to\infty}(\int_a^b f^p)^{1/p} = M$.
    Proof: <1>2 and <1>3 (letting $\eps \to 0$).
<1>5. Q.E.D.
:::
