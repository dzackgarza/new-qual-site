...
(b) Prove the Borsuk–Ulam theorem by the following argument. Suppose on the contrary that $f: S^n \to \mathbb{R}^n$ satisfies $f(x) \neq f(-x)$ for all $x$. Then define $g: S^n \to S^{n-1}$ by $g(x) = \bigl(f(x) - f(-x)\bigr) / \bigl|f(x) - f(-x)\bigr|$, so $g(-x) = -g(x)$ and $g$ induces a map $\mathbb{RP}^n \to \mathbb{RP}^{n-1}$. Show that part (a) applies to this map.
