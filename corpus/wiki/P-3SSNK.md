...
<1>4. Given $\eps > 0$, choose $\delta > 0$ with $|f(x) - f(0)| < \eps$ for $|x - 0| < \delta$ (continuity at $0$); then for any partition containing $0$ whose interval around $0$ has length $< \delta$:
    <2>1. The upper and lower Stieltjes sums satisfy $U(P) - L(P) \le 2\eps$ (the jump is $2$).
        Proof: the only contributing interval $[x_j, x_{j+1}] \ni 0$ has $\Delta\alpha = 2$ and oscillation of $f$ on it $\le 2\eps$; every other interval has $\Delta\alpha = 0$.
