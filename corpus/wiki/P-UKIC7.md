...
<1>1. Reduce to a non-negative function with a minimum at $x_0$.
    Proof: set $h = g - f$; then $h \ge 0$, $h(x_0) = 0$, and $h$ is differentiable with $h' = g' - f'$. It suffices to show $h'(x_0) = 0$.
...
<1>4. $h'(x_0) = 0$, hence $f'(x_0) = g'(x_0)$.
    Proof: $h$ is differentiable at $x_0$, so $h'(x_0) = h'_+(x_0) = h'_-(x_0)$; by <1>2 and <1>3 this common value satisfies $0 \le h'(x_0) \le 0$, so $h'(x_0) = 0$. Since $h' = g' - f'$, the claim follows.
<1>5. Q.E.D.
:::
