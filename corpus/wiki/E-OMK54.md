...
<1>1. Translation and dilation invariance of the Lebesgue integral: $\int \tau_h f = \int f$ where $\tau_h f(x) = f(x + h)$, and $\int f_\delta = \int f$ where $f_\delta(x) = \delta^{-n}f(x/\delta)$.
    <2>1. Both hold for indicators: $m(E + h) = m(E)$ and $m(\delta E) = \delta^n m(E)$.
        Proof: translation and dilation invariance of Lebesgue measure (for measurable $E$).
    <2>2. Both hold for non-negative simple functions, then for non-negative measurable $f$ by monotone convergence, then for $f \in L^1$ by positive/negative parts.
