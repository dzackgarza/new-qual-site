...
<1>1. Proof that $f$ is injective:
    Proof:
    <2>1. Let $x_1, x_2 \in X$ and assume $f(x_1) = f(x_2)$.
    ...
    <2>4. Therefore, $x_1 = x_2$. By definition, $f$ is injective.

<1>2. Proof that $g$ is surjective:
    Proof:
    <2>1. Let $x \in X$ be arbitrary. We must find an element $y \in Y$ such that $g(y) = x$.
...
<1>3. Counterexample to surjectivity of $f$ and injectivity of $g$:
    Proof:
    <2>1. Let $X = \{1\}$ and $Y = \{1, 2\}$.
