...
1. **Residue Calculation:**
   The only singularity of $f(z)$ in the upper half-plane is a simple pole at $z = ia$.
   The residue is:
   $$
...
2. **Arc Estimate (Jordan's Lemma):**
   On $C_R$, $z = R e^{i\theta}$, $|z| = R$, and $|e^{iz}| = e^{-R \sin \theta}$.
