...
Proof: the $f_k$ are uniformly bounded in $L^p$ (since $|f_k| \le 1$ on the finite-measure set $X$), so weak convergence follows from convergence against a dense set of test functions.

<1>2. For every $g \in C^1(X)$ (or any $g \in L^q$), $\int_{-\pi}^{\pi} \sin(kx)\, g(x)\, dx \to 0$.
Proof: this is the Riemann–Lebesgue lemma.
...
Proof: by periodicity, $\int_{-\pi}^{\pi} |\sin(kx)|^p\, dx = \int_{-\pi}^{\pi} |\sin(x)|^p\, dx > 0$ (substitute $u = kx$ and use $2\pi$-periodicity).

<1>2. Hence $\|f_k\|_{L^p} \not\to 0$.
Proof: <1>1 shows the norm is a fixed positive constant.

<1>3. Therefore $f_k$ does not converge strongly to $0$ in $L^p(X)$.
Proof: strong convergence to $0$ would force $\|f_k\|_{L^p} \to 0$, contradicting <1>2.
