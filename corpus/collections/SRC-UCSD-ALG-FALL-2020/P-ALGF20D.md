...
::: {.proof}
the all-ones matrix is the outer product of the all-ones vector with itself.
:::
...
::: {.proof}
$A^2 = nA$, so $A$ satisfies $x^2 - nx = x(x - n)$, which has distinct roots.
:::
...
::: {.proof}
<1>6 and <1>7, specialized to the case $n = 0$ in $F$.
:::
