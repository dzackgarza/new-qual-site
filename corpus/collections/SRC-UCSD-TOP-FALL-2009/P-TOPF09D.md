...
\]
Proof: Universal Coefficient Theorem over a field (Ext vanishes).
<2>3. Thus $\dim_{\mathbb{Q}} H^k(M; \mathbb{Q}) = \dim_{\mathbb{Q}} H_k(M; \mathbb{Q}) = b_k$.
Proof: finite-dimensional vector spaces are isomorphic to their duals.
<2>4. Combining <2>1 and <2>3 gives $b_i = b_{3-i}$ for all $i \in \{0, 1, 2, 3\}$.
Proof: $b_i = \dim H_i(M; \mathbb{Q}) = \dim H^{3-i}(M; \mathbb{Q}) = b_{3-i}$.

<1>3. Compute $\chi(M)$:
\[
\chi(M) = b_0 - b_1 + b_2 - b_3 = b_0 - b_1 + b_1 - b_0 = 0.
\]
Proof: $b_3 = b_0$ and $b_2 = b_1$ from <1>2.
...
\]
Proof: Universal Coefficient Theorem torsion cancellation for Euler characteristics.
