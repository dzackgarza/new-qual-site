...
Proof: factor the monic polynomials.
...
Proof: a root of a monic polynomial with coefficients in $C$ is integral over $C$; since $C$ is integral over $A$, it is integral over $A$.

<1>4. Hence each $\alpha_i$ and each $\beta_j$ is integral over $A$.
Proof: <1>2 and <1>3 (the roots of $fg$ are exactly the $\alpha_i$ and $\beta_j$).
...
Proof: the integral elements form a ring.
...
Proof: the same argument with the $\beta_j$.
