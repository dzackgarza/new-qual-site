...
::: {.proof}
<1>1 and linearity of convolution.
:::
...
::: {.proof}
weakly convergent sequences are bounded, and the support condition.
:::

<1>4. For fixed $x$, $f_n(x) - f(x) = \int K(x - y)(\psi_n(y) - \psi(y))\,dy = \langle \psi_n - \psi, \overline{K(x - \cdot)} \rangle$.
::: {.proof}
<1>2, writing the integral as an inner product.
:::
...
::: {.proof}
convolution with an $L^1$ kernel on a bounded domain is compact.
:::

<1>7. A compact operator maps weakly convergent sequences to strongly convergent sequences.
::: {.proof}
standard fact (a compact operator is completely continuous).
:::
