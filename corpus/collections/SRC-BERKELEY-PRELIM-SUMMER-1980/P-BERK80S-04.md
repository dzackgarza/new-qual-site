...
::: {.proof}
The group $G$ acts by conjugation on its set of subgroups. The stabilizer of $H$ is exactly $N_G(H)$, so the orbit-stabilizer theorem gives
\[
...
Hence, even in the largest possible union, after counting the identity once, each conjugate can contribute at most $|H|-1$ further elements. Thus
\[
\left|\bigcup_{x\in G}xHx^{-1}\right|
\le 1+m(|H|-1)
\le 1+[G:H](|H|-1).
\]
Since $H$ is proper, $[G:H]>1$. Using $|G|=[G:H]|H|$,
\[
1+[G:H](|H|-1)
=|G|-[G:H]+1
<|G|.
\]
Therefore the union of all conjugates of $H$ cannot equal $G$. Hence some element of $G$ lies in no subgroup $xHx^{-1}$.
:::
:::
