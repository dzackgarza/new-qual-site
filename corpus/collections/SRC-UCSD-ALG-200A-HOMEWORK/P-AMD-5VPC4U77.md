...
Proof: $\theta$ is an automorphism of $H$, so $\Phi$ has inverse $(h, k) \mapsto (\theta^{-1}(h), k)$.
...
Proof: definition (note the direction: $\psi_3 = \psi \circ \rho$).
<2>2. $\Psi$ is a homomorphism.
Proof: in $H \rtimes_{\psi_3} K$, $(h_1, k_1)(h_2, k_2) = (h_1 \psi_3(k_1)(h_2), k_1 k_2) = (h_1 \psi(\rho(k_1))(h_2), k_1 k_2)$; applying $\Psi$ gives $(h_1 \psi(\rho(k_1))(h_2), \rho(k_1 k_2)) = (h_1 \psi(\rho(k_1))(h_2), \rho(k_1)\rho(k_2))$, which equals the product $(h_1, \rho(k_1))(h_2, \rho(k_2))$ in $H \rtimes_\psi K$.
<2>3. $\Psi$ is bijective.
Proof: $\rho$ is an automorphism of $K$, so $\Psi$ has inverse $(h, k) \mapsto (h, \rho^{-1}(k))$.
