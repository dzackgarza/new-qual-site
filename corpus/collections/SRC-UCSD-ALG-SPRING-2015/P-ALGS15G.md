...
Proof: $M$ has rank $1$ (all rows equal), so $\dim \ker M = 2$; the trace is $3$, so the third eigenvalue is $3$.

<1>2. Case 1: $\operatorname{char} F \neq 3$.
<2>1. The eigenvalues are $0, 0, 3$, all distinct from each other (since $3 \neq 0$).
Proof: <1>1 and $\operatorname{char} F \neq 3$.
<2>2. $M$ is diagonalizable.
Proof: $M$ is symmetric (self-adjoint), hence diagonalizable; equivalently, the eigenspace for $0$ has dimension $2$ and for $3$ has dimension $1$, summing to $3$.
