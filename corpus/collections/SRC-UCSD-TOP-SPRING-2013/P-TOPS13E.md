...
Proof: $\eta$ is nullhomotopic in $\RR^3 \setminus \{0\}$ (the loop $\eta$ bounds a disk in the $xy$-plane not passing through $0$... more precisely, $\eta$ is homotopic to a constant via the homotopy that shrinks the circle to a point in the plane $z=0$ avoiding the origin), so $h = g \circ \eta$ is nullhomotopic.

<1>5. Hence $h$ has degree $0$.
Proof: a nullhomotopic map $S^1 \to S^1$ has degree $0$.

<1>6. But $h$ has odd degree.
<2>1. $h(s + 1/2) = g(\eta(s + 1/2)) = g(-\eta(s)) = -g(\eta(s)) = -h(s)$.
Proof: $\eta(s + 1/2) = -\eta(s)$ and $g(-x) = -g(x)$.
...
<1>7. Contradiction.
Proof: <1>5 says $\deg h = 0$ but <1>6 says $\deg h$ is odd.
