...
::: problem
Let $G$ be a finite group and $\pi_0$, $\pi_1$ be two irreducible representations of $G$.
Prove or disprove the following assertion: $\pi_0$ and $\pi_1$ are equivalent if and only if $\det\pi_0(g)=\det\pi_1(g)$ for all $g\in G$.
:::
...
1. **Determinants:** Since each $\pi_j$ is 1-dimensional, the determinant of a $1 \times 1$ matrix is the scalar itself: $\det \pi_j(g) = \pi_j(g)$.
   For $g = x$:
   $$
...
2. **Higher-dimensional Counterexample (matching determinants but inequivalent):** To disprove the $\Longleftarrow$ direction (i.e. having identical determinants implies equivalence): Consider the quaternion group $G = Q_8 = \{\pm 1, \pm i, \pm j, \pm k\}$.
   ...
   To get identical determinants: Consider $G = S_3$.
   ...
   Let $\pi_0 = \rho$.
   For $\pi_1$, consider $\rho \otimes \chi_{\text{sgn}}$.
   On $S_3$, $\rho \cong \rho \otimes \chi_{\text{sgn}}$, so these are equivalent.

   For an explicit non-isomorphic pair with equal determinants: Let $G = A_4$ (the alternating group on 4 elements).
   ...
   For $G = D_{10} = \langle r, s \mid r^5 = 1, s^2 = 1, srs = r^{-1} \rangle$: $D_{10}$ has two distinct 2-dimensional irreducible representations $\rho_1, \rho_2$:
