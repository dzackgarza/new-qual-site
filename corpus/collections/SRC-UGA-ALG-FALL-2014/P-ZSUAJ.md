...
::: {.proof}
the union of a chain of ideals is an ideal; if the union were $R$, then $x$ would lie in some member $K$ of the chain, and since $xR = R$ (<1>1), we would get $R = xR \subseteq K$, contradicting that $K$ is proper.
:::
...
::: {.proof}
<1>4 produces a maximal ideal $M$.
:::
