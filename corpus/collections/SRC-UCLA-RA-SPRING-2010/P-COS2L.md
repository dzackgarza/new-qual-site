...
<1>3. $\ker \omega = B$.
::: {.proof}
$u \in B$ iff $*du$ is exact (<1>1) iff all its periods vanish (<1>2), i.e. $\omega(u) = 0$.
:::
...
::: {.proof}
by the first isomorphism theorem, $A/B \cong \operatorname{im}\omega = \RR^n$ (<1>3, <1>4); <1>4 shows the listed classes are linearly independent and span.
:::
