...
<2>6. Thus: $$G_2 = \langle c, d \mid c^2 = d^2 \rangle \cong \langle c, d \mid c^2 d^{-2} = 1 \rangle.$$
::: {.proof}
<2>7. The van Kampen theorem for the amalgamated free product identifies $\pi_1(K)$ with $\pi_1(M_1) *_{\pi_1(\partial M_1)} \pi_1(M_2)$; the amalgamating subgroup is generated by $t$, and the two inclusions send $t$ to $c^2$ and $d^2$, so the relation $c^2 = d^2$ is imposed.
:::
...
::: {.proof}
  <2>4. The computations in <2>2 show $\Psi \circ \Phi$ and $\Phi \circ \Psi$ are the identity on the generators, so $\Phi$ and $\Psi$ are mutually inverse isomorphisms.
:::
...
::: {.proof}
<2>5. The subgroup $\langle a \rangle$ is normal with quotient $\langle b \rangle \cong \mathbb{Z}$, and conjugation by $b^n$ acts on $a^m$ by $a^{(-1)^n m}$; this is exactly the semidirect product $\mathbb{Z} \rtimes_\theta \mathbb{Z}$ with $\theta(n)(m) = (-1)^n m$, so the normal form $a^m b^n \mapsto (m, n)$ is an isomorphism.
:::

<1>5. Conclusion: $G_1 \cong G_2 \cong G_3$.
::: {.proof}
<2>1. Steps <1>1–<1>4 prove that all three presentations/models describe the same group $\pi_1(K)$.
:::
:::
