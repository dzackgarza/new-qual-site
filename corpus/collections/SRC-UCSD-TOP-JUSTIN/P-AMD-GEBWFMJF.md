...
<2>5. Since $H(u)$ has Lebesgue measure zero in $\mathbb{R}^n$, $\mu(A_i \cap H(u)) = 0$, so: $$\mu(A_i \cap H^+(u)) + \mu(A_i \cap H^+(-u)) = \mu(A_i) \quad \text{for each } i \in \{1, \dots, n\}.$$
::: {.proof}
<2>6. The two half-spaces $H^+(u)$ and $H^+(-u)$ are complementary, overlapping only on the hyperplane $H(u)$; since $H(u)$ has measure zero, the measures of the two intersections add to $\mu(A_i)$.
:::
...
::: {.proof}
<2>5. The Dominated Convergence Theorem applies because the integrands are dominated by $\chi_{A_i}$ and $\mu(A_i) < \infty$, so each $f_i$ is continuous; hence $F$ is continuous.
:::
