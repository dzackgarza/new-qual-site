...
Proof: existence of projective resolutions in $R\text{-}\mathbf{Mod}$.
...
Proof: definition of tensor product double complex.
...
Proof: $(d^h + d^v)^2 = (d^h)^2 + (d^h d^v + d^v d^h) + (d^v)^2 = 0 + 0 + 0 = 0$.
...
Proof: projective modules are flat.
...
\]
Proof: flatness commutes with homology.
...
\]
Proof: Weibel Theorem 2.7.2 (Acyclic Assembly Lemma).
...
\]
Proof: definition of $\operatorname{Tor}_n^R(M, N)$ via projective resolution of $N$.
...
\]
Proof: Acyclic Assembly Lemma on vertical augmentation.
...
Proof: definition of $\operatorname{Tor}_n^R(N, M)$ via projective resolution of $M$.
