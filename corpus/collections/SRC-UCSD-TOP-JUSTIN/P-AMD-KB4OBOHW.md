...
<1>3. We have
$$
\boxed{\operatorname{Ext}_R^0(M,M)\cong M,\qquad
\operatorname{Ext}_R^i(M,M)=0\ (i>0).}
$$
::: {.proof}
...
$$
because $x-y=0$ in $M$. Thus $\operatorname{Ext}^0\cong M$ and $\operatorname{Ext}^1\cong M$, not zero. Therefore the displayed claim must be corrected: actually
$$
\boxed{\operatorname{Ext}_R^0(M,M)\cong M,\quad \operatorname{Ext}_R^1(M,M)\cong M,\quad \operatorname{Ext}_R^i(M,M)=0\ (i\ge2).}
$$
:::
...
$$
The first map is injective, so $H^0=0$. The kernel of the second map is the diagonal $\{(a,a)\}$, and modulo the image $tM\cdot(1,1)$ this gives $M/tM\cong\mathbb Z$, hence $H^1\cong\mathbb Z$. The second map has image $tM$, so its cokernel is also $M/tM\cong\mathbb Z$, not zero. Thus the corrected result is
$$
\boxed{\operatorname{Ext}_R^0(N,M)=0,\quad
\operatorname{Ext}_R^1(N,M)\cong\mathbb Z,\quad
