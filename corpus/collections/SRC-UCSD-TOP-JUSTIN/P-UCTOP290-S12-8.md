---
schema: qual/card@1
id: P-UCTOP290-S12-8
kind: problem
title: "No retraction from CP^n onto CP^k for k < n, and the mod-2 analogue for RP^n"
classification:
  areas:
  - topology
  topics:
  - Cohomology Ring
  - Retractions
relations: []
review: draft
---

::: problem
Assume the fact that $H^*(\mathbb{C}P^n;\mathbb{Z}) = \mathbb{Z}[\alpha]/(\alpha^{n+1})$, where $\alpha$ has degree $2$, and $H^*(\mathbb{R}P^n;\mathbb{Z}_2) = \mathbb{Z}_2[\beta]/(\beta^{n+1})$, where $\beta$ has degree $1$.
(These are "truncated polynomial rings".)
Show that if $k < n$ there is no retraction from $\mathbb{C}P^n$ to its standard submanifold $\mathbb{C}P^k$, and similarly for $\mathbb{R}P^n$.
:::
