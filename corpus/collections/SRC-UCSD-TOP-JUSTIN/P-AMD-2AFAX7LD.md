...
::: {.proof}
<2>2. Stereographic projection is a bijection with an explicit formula, and both it and its inverse are continuous, so it is a homeomorphism.
:::

<1>2. Under the homeomorphism $\phi$, $S^3 \setminus \{p_0, p_1\}$ is homeomorphic to $\mathbb{R}^3 \setminus \{\phi(p_1)\}$.
::: {.proof}
<2>1. A homeomorphism $\phi \colon X \to Y$ restricts to a homeomorphism $X \setminus \{x\} \to Y \setminus \{\phi(x)\}$ for any point $x \in X$.
Here $X = S^3 \setminus \{p_0\}$ and $x = p_1 \in X$ since $p_1 \neq p_0$.
:::

<1>3. $\mathbb{R}^3 \setminus \{\phi(p_1)\}$ is homeomorphic to $\mathbb{R}^3 \setminus \{0\}$.
::: {.proof}
<2>1. Translation by $-\phi(p_1)$, given by $v \mapsto v - \phi(p_1)$, is a homeomorphism of $\mathbb{R}^3$ that maps $\phi(p_1)$ to the origin $0$.
<2>2. Restricting this homeomorphism to $\mathbb{R}^3 \setminus \{\phi(p_1)\}$ gives a homeomorphism onto $\mathbb{R}^3 \setminus \{0\}$.
:::
...
::: {.proof}
<2>6. The three identities in <2>5 are exactly the defining conditions of a strong deformation retraction: $H(\cdot, 0) = \operatorname{id}$, $H(\cdot, 1) = \iota \circ r$, and $H$ fixes $S^2$ pointwise throughout.
:::

<1>5. Homotopy equivalence is an equivalence relation preserved under homeomorphisms.
::: {.proof}
<2>1. Every homeomorphism is a homotopy equivalence, and a deformation retraction is a homotopy equivalence.
<2>2. Composing homotopy equivalences yields a homotopy equivalence: $$S^3 \setminus \{p_0, p_1\} \cong \mathbb{R}^3 \setminus \{\phi(p_1)\} \cong \mathbb{R}^3 \setminus \{0\} \simeq S^2.$$
:::

<1>6. Q.E.D.
::: {.proof}
<2>1. Combining <1>1 through <1>5 establishes $S^3 \setminus \{p_0, p_1\} \simeq S^2$.
:::
:::
