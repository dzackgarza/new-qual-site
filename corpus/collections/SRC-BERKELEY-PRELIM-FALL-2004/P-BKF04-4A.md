...
::: {.solution}
First suppose that A is diagonalizable. If C is an invertible $n \times n$ matrix, then $f ( C A C ^ { - 1 } ) = C f ( A ) C ^ { - 1 }$, so both sides of the “if and only if” are unchanged by conjugation. Thus we may assume A is diagonal. Then $f ( A )$ is diagonal for any $f .$ Hence if $f ( A )$ is nilpotent, then $f ( A ) = 0$
