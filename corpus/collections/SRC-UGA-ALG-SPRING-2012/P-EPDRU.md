...
\]
Thus $x - y \in M_t$.
Proof: commutativity and zero-divisor freeness of $R$.
<2>3. For any $a \in R$ and $x \in M_t$ with $rx = 0$ ($r \neq 0$):
\[
r(ax) = a(rx) = a(0) = 0.
\]
Thus $ax \in M_t$.
Proof: $R$-action on modules.
<2>4. Therefore $M_t$ is an $R$-submodule of $M$.
Proof: submodule criterion (<2>1 through <2>3).
...
Proof: setup for torsion in quotient.
...
Proof: $R$ has no non-zero zero divisors.
