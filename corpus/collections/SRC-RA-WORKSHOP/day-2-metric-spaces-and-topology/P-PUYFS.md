...
::: {.proof}
for $x, x' \in X$ and any $f \in F$, $d(x,f) \le d(x,x') + d(x',f)$, so $\delta(x) \le d(x,x') + \delta(x')$; symmetrically $\delta(x') \le d(x,x') + \delta(x)$, hence $|\delta(x) - \delta(x')| \le d(x,x')$.
:::
<1>2. $\delta$ attains its minimum on the compact set $K$.
::: {.proof}
$\delta$ is continuous (<1>1), and a continuous function on a compact set attains its minimum: choose $k_n \in K$ with $\delta(k_n) \to \inf_K \delta$; a subsequence $k_{n_j} \to k \in K$ (sequential compactness in the metric space), and continuity gives $\delta(k) = \inf_K\delta$.
:::
<1>3. $\inf_{k\in K} d(k,F) > 0$.
::: {.proof}
by <1>2 the infimum is $\delta(k)$ for some $k \in K$.
:::
...
::: {.proof}
take $\eps \da \inf_{k\in K} d(k,F) > 0$ from <1>3; then $d(k,f) \ge d(k,F) \ge \eps$ for all $k \in K$, $f \in F$.
:::
<1>5. Q.E.D.
