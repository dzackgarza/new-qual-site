...
Proof: for $x \neq 0$, the ratio $1/(1+x^2) \in (0,1)$ and the geometric formula gives $x^2 \cdot \frac{1}{1 - 1/(1+x^2)} = x^2 \cdot \frac{1+x^2}{x^2} = 1 + x^2$.
At $x = 0$ every term is $0$.
...
Proof: by <1>3, $R_N(0) = 0$ but $R_N(x) = (1+x^2)^{-N} \to 1$ as $x \to 0^+$; in fact $\sup_{x \in (0,b]} (1+x^2)^{-N} = 1$ (approached as $x \to 0$). So the suprema of the tails do not tend to $0$, which is exactly the negation of uniform convergence.
(Equivalently, the pointwise limit $S$ of <1>2 is discontinuous at $0$ while partial sums are continuous.)

<1>6. Q.E.D. Proof: <1>4 and <1>5 are the two claims.
:::
