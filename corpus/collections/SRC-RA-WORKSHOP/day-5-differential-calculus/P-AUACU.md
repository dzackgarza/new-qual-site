...
::: {.proof}
by <1>2, $|f(x) - f(y)| \le M|x - y| < M\delta$; choose $\delta = \eps/M$.
:::

<1>4. $\lim_{x \to a^+} f(x)$ exists.
::: {.proof}
the Cauchy criterion <1>3 is equivalent to the existence of the limit (pick a sequence $x_n \to a^+$; <1>3 makes $(f(x_n))$ Cauchy in the complete space $\RR$, so it converges, and the limit is independent of the sequence by <1>3 again).
:::
:::
