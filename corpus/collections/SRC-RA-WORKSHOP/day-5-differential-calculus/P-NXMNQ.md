...
::: {.proof}
$\frac{f(x) - f(0)}{x - 0} = \frac{xg(x)}{x} = g(x) \to g(0)$ as $x \to 0$ (continuity of $g$ at $0$; also $f(0) = 0\cdot g(0) = 0$).
:::

<1>2. ($\Rightarrow$) If $f$ is differentiable at $0$ (and $f(0) = 0$), define $g(x) = \frac{f(x)}{x}$ for $x \neq 0$ and $g(0) = f'(0)$.
::: {.proof}
then $f(x) = xg(x)$ for all $x$ (for $x = 0$ both sides are $0$).
:::

<1>3. $g$ is continuous at $0$: $\lim_{x \to 0} g(x) = \lim_{x \to 0}\frac{f(x)}{x} = \lim_{x \to 0}\frac{f(x) - f(0)}{x - 0} = f'(0) = g(0)$.
::: {.proof}
<1>2 and the definition of the derivative (using $f(0) = 0$).
:::

<1>4. Q.E.D.
::: {.proof}
<1>1 and <1>3 give both directions.
:::
:::
