...
are connected.
::: {.proof}
The projection $X_b\to X$ is a homeomorphism, and the projection $Y_x\to Y$ is a homeomorphism. The spaces $X$ and $Y$ are connected by hypothesis.
:::
...
is connected.
::: {.proof}
By <1>1, both $X_b$ and $Y_x$ are connected. They meet at the point $(x,b)$, so their union is connected.
:::
...
::: {.proof}
If $(x,y)\in X\times Y$, then $(x,y)\in Y_x\subseteq T_x$. Hence
\[
...
::: {.proof}
By <1>2 each $T_x$ is connected, by <1>3 they share a common point, and by <1>4 their union is $X\times Y$. A union of connected subspaces with a common point is connected.
:::
