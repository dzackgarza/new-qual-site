...
Proof: the irreducible representations of $S_2$ are trivial $(2)$ and sign $(1,1)$; of $S_1$ only trivial $(1)$.
<2>2. By the Pieri rule, $s_{(1,1)} s_{(2)} = s_{(3,1)} + s_{(2,1,1)}$.
Proof: multiplying by $s_{(2)}$ adds a horizontal strip of size $2$ to $(1,1)$.
<2>3. Then $s_{(3,1)} s_{(1)} = s_{(4,1)} + s_{(3,2)} + s_{(3,1,1)}$ and $s_{(2,1,1)} s_{(1)} = s_{(3,1,1)} + s_{(2,2,1)} + s_{(2,1,1,1)}$.
Proof: multiplying by $s_{(1)}$ adds a single box in all valid ways.
<2>4. Hence $V = S^{(4,1)} \oplus S^{(3,2)} \oplus 2 S^{(3,1,1)} \oplus S^{(2,2,1)} \oplus S^{(2,1,1,1)}$.
Proof: collect terms; $S^{(3,1,1)}$ appears with multiplicity $2$.
<2>5. Dimensions: $\dim S^{(4,1)} = 4$, $\dim S^{(3,2)} = 5$, $\dim S^{(3,1,1)} = 6$, $\dim S^{(2,2,1)} = 5$, $\dim S^{(2,1,1,1)} = 4$.
Proof: hook-length formula.
...
Proof: the decomposition from <1>1.4.
<2>2. $\operatorname{End}_{S_5}(V) \cong \bigoplus_\lambda M_{m_\lambda}(\CC)$.
Proof: Schur's lemma / the endomorphism algebra of a semisimple module is a product of matrix algebras.
<2>3. $\dim \operatorname{End}_{S_5}(V) = \sum_\lambda m_\lambda^2 = 1 + 1 + 4 + 1 + 1 = 8$.
Proof: sum the squares of the multiplicities.
...
Proof: the center of $M_m(\CC)$ is $\CC$ (scalar matrices).
<2>2. Hence $\dim Z(\operatorname{End}_{S_5}(V)) = \text{number of distinct irreducibles} = 5$.
Proof: there are five distinct irreducible summands.

<1>4. Q.E.D.
Proof: <1>1.4 gives the decomposition; <1>2.3 gives $\dim \operatorname{End} = 8$; <1>3.2 gives $\dim Z = 5$.
:::
