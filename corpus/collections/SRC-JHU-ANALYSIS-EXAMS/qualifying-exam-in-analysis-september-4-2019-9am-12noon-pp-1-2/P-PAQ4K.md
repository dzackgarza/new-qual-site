...
::: {.solution}
<1>1. Part (a): **Yes**, it is always true that $\int_0^1 f(x)\,dx \le 1$:
<2>1. Each $f_n$ is positive and continuous on $[0, 1]$, hence non-negative and Lebesgue measurable.
...
<1>2. Part (b): **No**, it is not always true that $\int_0^1 f(x)\,dx \ge 1$:
<2>1. We construct a counterexample of strictly positive continuous functions $g_n$ on $[0, 1]$ whose integral is $1$ but whose pointwise limit is $0$.
...
- For any $x \in (0, 1]$: choose $N > 1/x$. For all $n \ge N$, $1/n \le 1/N < x$, so $T_n(x) = 0$.
...
<1>3. Conclusion:
(a) True by Fatou’s Lemma.
(b) False: standard escaping spike sequence yields $\int_0^1 f(x)\,dx = 0 < 1$. Q.E.D.
Proof: <1>1 and <1>2.
:::
