...
Proof: assume such a function exists.
...
Proof: $\widehat f(\xi) = \int f(x) e^{-2\pi i x \xi}\,dx$ is entire in $\xi$ (the integral converges for all complex $\xi$ since $f$ has compact support).
...
Proof: an entire function that vanishes on a set with an accumulation point (e.g. an interval) is identically zero.

<1>4. Hence $\widehat f \equiv 0$, so $f \equiv 0$ (by Fourier inversion).
Proof: <1>3 and the Fourier inversion theorem.
