...
::: {.problem}
Does there exist an open dense subset of $\mathbb R$ with Lebesgue measure equal to one? Either construct an example or prove that one does not exist.
:::
...
\]
Then $U$ is open. It contains $\mathbb Q$, so it is dense in $\mathbb R$. Moreover, by countable subadditivity,
\[
m(U)\le \sum_{n=1}^\infty 2^{-n-2}=\frac14<\infty.
\]
Since $U$ contains a nonempty open interval, $m(U)>0$. Thus
\[
0<m(U)<\infty.
\]
:::
...
\]
A nonzero dilation is a homeomorphism of $\mathbb R$, so $V$ is again open and dense. By the scaling property of Lebesgue measure,
\[
m(V)=a^{-1}m(U)=1.
\]
