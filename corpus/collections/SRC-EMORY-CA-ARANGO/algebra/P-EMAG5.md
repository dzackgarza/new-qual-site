...
::: {.proof}
the cosets of $Z(G)$ are the powers of $gZ(G)$.
:::

<1>3. Any two elements $g^k z_1$ and $g^\ell z_2$ commute.
::: {.proof}
$(g^k z_1)(g^\ell z_2) = g^{k+\ell} z_1 z_2 = (g^\ell z_2)(g^k z_1)$, since $z_1, z_2$ are central and powers of $g$ commute.
:::
...
::: {.proof}
for a non-central $g_i$, $C_G(g_i) \neq G$, so $[G : C_G(g_i)] > 1$ divides $|G| = p^n$, hence is a power of $p$ greater than $1$.
:::

<1>3. Hence $p \mid |Z(G)|$.
::: {.proof}
$|G| = p^n$ and each term in the sum is divisible by $p$, so $|Z(G)| = |G| - \sum [G : C_G(g_i)]$ is divisible by $p$.
:::
...
<1>2. $|Z(G)| \in \{p, p^2\}$.
::: {.proof}
$Z(G)$ is a subgroup of $G$ of order dividing $p^2$, and it is nontrivial.
:::

<1>3. If $|Z(G)| = p^2$, then $G = Z(G)$ is abelian.
::: {.proof}
$Z(G) \subseteq G$ and $|Z(G)| = |G|$.
:::
