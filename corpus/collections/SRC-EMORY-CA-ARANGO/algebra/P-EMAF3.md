...
Proof: splitting fields are normal.

<1>2. Since $f$ is separable and $L$ is generated by the roots of $f$, each generator is separable over $F$; hence $L/F$ is separable.
Proof: a splitting field of a separable polynomial is separable (it is the compositum of separable simple extensions $F(r_i)/F$).
...
Proof: <1>3 and <1>4 (a subgroup of $S_n$ of order $n!$ is all of $S_n$).
