...
::: {.proof}
the extension is cyclic of degree $4$ (the Galois group is generated by $\sqrt{2+\sqrt{2}} \mapsto \sqrt{2-\sqrt{2}}$, of order $4$).
:::
...
::: {.proof}
the splitting field is generated by one root and $\omega$.
:::

<1>3. $[K : \QQ] = 6$, and $\operatorname{Gal}(K/\QQ) \cong S_3$.
::: {.proof}
$[\QQ(\sqrt[3]{5}) : \QQ] = 3$ (Eisenstein at $5$) and $[\QQ(\omega) : \QQ] = 2$, with $K = \QQ(\sqrt[3]{5}, \omega)$ of degree $6$; the Galois group is $S_3$ (the full symmetric group on the three roots).
:::

<1>4. A proper Galois sub-extension is $L = \QQ(\omega)$.
::: {.proof}
$\QQ(\omega)/\QQ$ is Galois (it is the splitting field of $x^2 + x + 1$), of degree $2$, with Galois group $\ZZ/2$.
:::
