---
schema: qual/card@1
id: P-TRIV-CA22
kind: problem
title: 'Mathematical Trivium — Complex Analysis problem 22'
classification:
  areas: [complex-analysis]
  topics: []
relations: []
review: draft
audit:
- event: source-checked
  by: gpt-5.6-sol
  date: 2026-09-14
  note: Checked against Complex Analysis, Problem 22, in the deterministic MinerU Flash extraction assets/attachments/Big_List_of_Math_Problems_extracted.md. The source hint refers to Figure 4, whose graphical content is absent from Flash. The integral itself is fully extracted; only the hinted contour is unresolved.
---

::: problem
Compute $\int _ { 0 } ^ { 1 } { \frac { 1 } { \left( x ^ { 2 } - x ^ { 3 } \right) ^ { 1 / 3 } } }$ (hint: use the contour in figure 4);

The source supplies Figure 4 as the hinted integration contour; its graphical content is not present in the deterministic extraction.
:::
