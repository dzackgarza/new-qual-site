...
::: {.proof}
the preimage of an open set under a covering map is a covering space of that set.
:::

<1>2. The connected components of $p^{-1}(U)$ are in bijection with the cosets of $\operatorname{im} i_*$ in $\pi_1(S)$.
::: {.proof}
for a covering space, the components of the preimage of a connected open set correspond to the orbits of $\pi_1(U)$ acting on the fiber, i.e. to the cosets $\pi_1(S)/\operatorname{im} i_*$ (the image of $\pi_1(U)$ under the inclusion).
:::
...
::: {.proof}
the number of cosets is $1$ exactly when the subgroup is the whole group.
:::
