...
Proof: $T^n = (S^1)^n$ has contractible universal cover $\mathbb{R}^n$ and $\pi_1 = \ZZ^n$, with all higher homotopy groups trivial.

<1>2. Hence $[X, T^n] \cong \operatorname{Hom}(\pi_1(X), \pi_1(T^n)) = \operatorname{Hom}(\pi_1(X), \ZZ^n)$.
Proof: for a CW complex $X$ and a $K(G,1)$, homotopy classes of maps are classified by homomorphisms on $\pi_1$.

<1>3. $\pi_1(X)$ is finite, so every homomorphism $\pi_1(X) \to \ZZ^n$ is trivial.
Proof: $\ZZ^n$ is torsion-free, so a finite group has no nontrivial homomorphism into it.
