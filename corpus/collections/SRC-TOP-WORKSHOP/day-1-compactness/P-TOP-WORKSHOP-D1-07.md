...
<1>1. Continuity of inverse from closed sets:
    *Proof:*  
    Let $C\subseteq X$ be closed. Since $X$ is compact, $C$ is compact.
    The image $f(C)$ is compact because $f$ is continuous, and compact subsets of Hausdorff
    spaces are closed.
    So $f$ sends closed sets in $X$ to closed sets in $Y$.

<1>2. Inverse image of open sets:
    *Proof:*  
    For any open $U\subseteq X$,
    $$f^{-1}(f(U))=U.$$
    Its complement in $Y$ is
