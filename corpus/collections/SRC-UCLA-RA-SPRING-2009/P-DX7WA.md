...
:::
<2>4. Jensen's formula (a).
::: {.proof}
combine <2>2 and <2>3: $\frac{1}{2\pi}\int_0^{2\pi}\log|f(Re^{i\theta})|\,d\theta = \log|F(0)| = \log|f(0)| + \sum_{k=1}^m \log\frac{R}{|a_k|}$.
:::
...
::: {.proof}
<1>1 gives $\sum_{|a_n| < R}\log\frac{R}{|a_n|} = \frac{1}{2\pi}\int_0^{2\pi}\log|f(Re^{i\theta})|\,d\theta - \log|f(0)| \le \log C + R^\lambda - \log|f(0)|$.
:::

<1>4. Counting estimate: $N(R) = \#\{n : |a_n| \le R\}$ satisfies $N(R) \le C' R^\lambda$ for $R \ge 1$.
::: {.proof}
each zero with $|a_n| \le R/2$ contributes $\ge \log 2$ to the sum in <1>3 (as $\log(R/|a_n|) \ge \log 2$), so $N(R/2)\log 2 \le \log C + R^\lambda - \log|f(0)| \le C'' R^\lambda$; replace $R/2$ by $R$.
:::
...
<1>6. Q.E.D.
::: {.proof}
<1>1–<1>2 prove (a); <1>3–<1>5 prove (b).
:::
:::
