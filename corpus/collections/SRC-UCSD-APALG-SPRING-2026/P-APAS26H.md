...
::: {.proof}
the convolution algebra is the group algebra, which is semisimple.
:::
...
::: {.proof}
the trivial representation is always present.
:::
...
::: {.proof}
construct a candidate subalgebra.
:::
...
::: {.proof}
a subgroup $H$ of odd order has $\mathbb{C}[H] \cong \bigoplus_j M_{m_j}(\mathbb{C})$ where the number of $1$-dimensional factors equals the number of linear characters of $H$, which is $|H^{\mathrm{ab}}|$, an odd number.
:::
