...
Proof: $\Phi(f + g) = ((f+g)(e_1), \ldots) = (f(e_1) + g(e_1), \ldots) = \Phi(f) + \Phi(g)$, and $\Phi(qf) = (qf(e_1), \ldots) = q\Phi(f)$ for $q \in \QQ$.

<1>3. $\Phi$ is injective.
Proof: if $\Phi(f) = 0$, then $f(e_i) = 0$ for all $i$; since $\{e_1, \ldots, e_s\}$ generates $\ZZ^s$, $f = 0$.

<1>4. $\Phi$ is surjective.
Proof: given $(q_1, \ldots, q_s) \in \QQ^s$, define $f: \ZZ^s \to \QQ$ by $f(\sum_i n_i e_i) = \sum_i n_i q_i$; this is a well-defined homomorphism with $\Phi(f) = (q_1, \ldots, q_s)$.
