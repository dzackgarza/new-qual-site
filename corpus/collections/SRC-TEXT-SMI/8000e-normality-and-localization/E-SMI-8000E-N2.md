...
Proof: $R$ is a UFD, so every nonzero nonunit factors into irreducibles.

<1>3. Some irreducible factor $p_i$ lies in $P$.
Proof: $P$ is prime and $p_1 \cdots p_k = a \in P$, so some $p_i \in P$.

<1>4. $(p_i)$ is a prime ideal.
Proof: in a UFD, an irreducible element generates a prime ideal.

<1>5. $(p_i) \subseteq P$ and $(p_i)$ is prime, so by minimality of $P$, $(p_i) = P$.
Proof: $P$ is minimal among nonzero primes, and $(p_i)$ is a nonzero prime contained in $P$.
