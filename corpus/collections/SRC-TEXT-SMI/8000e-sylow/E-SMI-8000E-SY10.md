...
- event: solution-reviewed
  by: chatgpt
  date: 2026-09-11
  note: "Reduced to a Sylow p-subgroup and proved by induction that a finite p-group has subgroups of every intermediate p-power order, quotienting by a central subgroup of order p and lifting."
---
...
p\nmid m.
$$
The hypothesis $p^s\mid |G|$ says $s\le r$. By the existence part of
Sylow's theorem, $G$ has a Sylow $p$-subgroup $P$ of order $p^r$. It is
therefore enough to prove the following claim.

<1>1. Every finite $p$-group of order $p^r$ has a subgroup of order $p^s$ for each $0\le s\le r$.
::: proof
We argue by induction on $r$.

For $r=0$ the group is trivial, so the assertion is immediate. Assume
$r>0$ and that the assertion holds for all $p$-groups of smaller order.
Let $P$ have order $p^r$.

The center $Z(P)$ is nontrivial. Choose
$$
1\ne z\in Z(P).
$$
The order of $z$ is a positive power $p^a$. Then
$$
c=z^{p^{a-1}}
$$
has order exactly $p$. Hence
$$
C=\langle c\rangle
$$
is a central, and therefore normal, subgroup of $P$ with
$$
|C|=p.
$$
...
P/C
$$
is a $p$-group of order $p^{r-1}$. If $s=0$, take the trivial subgroup; if
$s=r$, take $P$ itself. Now suppose
$$
1\le s<r.
$$
By the induction hypothesis, $P/C$ has a subgroup
$$
\overline H\le P/C
$$
of order
...
<1>2. Apply the claim inside a Sylow subgroup of $G$.
::: proof
The Sylow subgroup $P$ has order $p^r$ with $s\le r$. By step <1>1, it
contains a subgroup $H$ of order $p^s$. Since
$$
H\le P\le G,
$$
the same $H$ is a subgroup of $G$. Hence
$$
\boxed{p^s\mid |G|\Longrightarrow G\text{ has a subgroup of order }p^s.}
$$
:::
:::
