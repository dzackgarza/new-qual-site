...
::: {.problem}
Find a countable abelian group whose endomorphism ring has the same cardinality as the set of real numbers. Justify your answer.
:::

::: {.solution}
Let G be a vector space of dimension $\aleph _ { 0 }$ over $\mathbb { F } _ { 2 }$ . Then G is countable, since it is a countable union of finite subspaces. Let $v _ { 1 } , v _ { 2 } , . . .$ . be a basis. For each $S \subseteq \{ 1 , 2 , 3 , \dots \}$ there is an endomorphism of $G$ mapping each $v _ { i }$ to $v _ { i }$ or 0 according to whether $i \in S$ Different subsets S give different endomorphisms, so # End $G \geq 2 ^ { \aleph _ { 0 } }$ . On the other hand,
