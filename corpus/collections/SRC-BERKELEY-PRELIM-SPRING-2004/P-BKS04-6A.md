...
::: {.solution}
Let $A = \mathbb { Z } [ x ] / ( x ^ { 2 } + a x + b )$ and $B = \mathbb { Z } \times \mathbb { Z }$ . If $x ^ { 2 } + a x + b$ is irreducible in the UFD $\mathbb { Z } [ x ]$ , then $( x ^ { 2 } + a x + b )$ is a prime ideal, so A is a domain. But B is not a domain. Thus we may assume $x ^ { 2 } + a x + b = ( x - c ) ( x - d )$ for some $c , d \in \mathbb { Z }$
