...
Proof: Theorem 4.12 (the canonical map into the double dual).
...
Proof: $\overline f : B^* \to A^*$ is precomposition with $f$, and $f^{**}$ is precomposition with $\overline f$.
...
Proof: <1>1 applied to $B$.

<1>5. Hence $f^{**} \circ \theta_A = \theta_B \circ f$, so the diagram commutes.
Proof: <1>3 and <1>4 agree for every $a \in A$ and $g \in B^*$.
