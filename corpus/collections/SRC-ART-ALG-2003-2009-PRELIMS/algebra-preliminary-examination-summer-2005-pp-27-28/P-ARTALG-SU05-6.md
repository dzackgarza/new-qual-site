...
Proof: three Jordan forms with eigenvalue $-2$ and Jordan blocks of sizes $(1,1,1)$, $(2,1)$, and $(3)$ respectively.

<1>3. Each has characteristic polynomial $(x + 2)^3$.
Proof: all eigenvalues are $-2$ with algebraic multiplicity $3$.
...
Proof: the minimal polynomial is the product of the elementary divisors (the invariant factors), and the elementary divisors are the $(x+2)^k$ for the Jordan blocks of size $k$.

<1>5. The three matrices are pairwise non-similar.
Proof: similar matrices have the same elementary divisors (or the same rational canonical form / Jordan form); the three sets of elementary divisors are distinct, so no two are similar.
