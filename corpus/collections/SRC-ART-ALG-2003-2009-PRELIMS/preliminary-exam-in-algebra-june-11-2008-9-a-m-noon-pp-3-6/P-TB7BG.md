...
Each companion matrix $M_i$ is the companion matrix of the invariant factor $x^{\beta_i}$, which is the $\beta_i \times \beta_i$ matrix $$\begin{bmatrix}0&&&&\\1&0&&&\\&1&\ddots&&\\&&\ddots&0&\\&&&1&0\end{bmatrix}$$ with ones on the subdiagonal and zeros elsewhere.
