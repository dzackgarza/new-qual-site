...
Proof: for a closed set $C \subseteq X$, $q^{-1}(q(C)) = C \cup A$ (if $C$ meets $A$) or $C$ (if $C$ is disjoint from $A$); in either case $q^{-1}(q(C))$ is closed in $X$ (since $A$ and $C$ are closed), so $q(C)$ is closed in $X/A$ by the definition of the quotient topology.
<2>2. The image of a normal space under a closed continuous surjection is normal.
Proof: standard theorem (if $f: X \to Y$ is a closed continuous surjection and $X$ is normal, then $Y$ is normal).
