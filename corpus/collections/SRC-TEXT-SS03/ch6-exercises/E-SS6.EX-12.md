...
::: {.proof}
<1>3 and (a); more directly, if $F = O(e^{c|s|})$ then $F$ has order $\le 1$, so by Hadamard factorization $F(s) = e^{as+b}\prod_n (1 - s/z_n)$; but the product over the zeros $0, -1, -2, \ldots$ is $1/\Gamma(s)$ (up to a factor), and $1/\Gamma$ is not $O(e^{c|s|})$, contradiction.
:::
