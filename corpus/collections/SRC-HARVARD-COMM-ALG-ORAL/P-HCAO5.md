...
Proof: canonical inclusion of polynomial rings.
...
\]
Proof: Division Algorithm for polynomials over a field.
<2>4. Since $\deg r(x) < 1$, $r(x)$ is a constant polynomial $c \in E$.
Proof: polynomials of degree $< 1$ are constants.
...
\]
Proof: evaluation homomorphism properties.
<2>2. By hypothesis, $p(a) = 0$, so $c = 0$.
Proof: hypothesis $p(a) = 0$.
