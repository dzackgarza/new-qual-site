---
schema: qual/card@1
id: P-CH10-48
kind: problem
title: Order of a finite group with $\mathbb Z_{10}$ and $\mathbb Z_{15}$ as homomorphic images
classification:
  areas:
  - algebra
  topics: []
relations: []
review: draft
---

::: {.problem}
Suppose that $\mathbb { Z } _ { 1 0 }$ and $\mathbb { Z } _ { 1 5 }$ are both homomorphic images of a finite group G. What can be said about $| G |$?
Generalize.
:::

::: {.solution}
If $\mathbb { Z } _ { 1 0 }$ is a homomorphic image of $G$, 10 divides the order of G. Similarly, 15 divides the order of $G$ . Hence the order of G is divisible by $l c m ( 1 0 , 1 5 ) = 3 0$ . In general, the order of G is divisible by the least common multiple of the orders of all its homomorphic images.
:::
