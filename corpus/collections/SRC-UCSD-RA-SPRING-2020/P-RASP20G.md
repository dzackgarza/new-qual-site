...
::: proof
positive definiteness of the norm and <1>2.
:::
...
\]
::: proof
expanding the norm squared and using $\langle x_n, x_m \rangle \ge \|x_n\|^2$ from <2>2.
:::

<1>13. $\{x_n\}$ is a Cauchy sequence in $H$: <2>1. Since $x_n\in K_n\subseteq K_1$ for every $n$ and $K_1$ is bounded, the non-decreasing sequence $\{\|x_n\|\}$ is bounded above. Hence $L = \lim_{n\to\infty} \|x_n\| < \infty$ exists.
::: proof
...
::: proof
<1>12 and <2>1. <2>3. Thus $\{x_n\}$ is a Cauchy sequence.
:::
::: proof
<2>2.
:::
