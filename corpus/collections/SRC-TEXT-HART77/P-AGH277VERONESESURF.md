...
a. The complete linear system $\abs{D}$ gives an embedding of $\PP^2$ in $\PP^5$, whose image is the **Veronese surface**.

b. Show that the subsystem defined by $x^2, y^2, z^2, y(x - z), (x - y)z$ gives a closed immersion of $X$ into $\PP^4$.
   The image is called the Veronese surface in $\PP^4$.
...
c. Let $\nu \subseteq \abs{D}$ be the linear system of all conics passing through a fixed point $P$.
   ...
   Since $\tilde X$ is the union of all these lines, we say $\tilde X$ is a **ruled surface**.
:::
