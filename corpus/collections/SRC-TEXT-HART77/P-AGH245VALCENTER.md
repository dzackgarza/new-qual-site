...
c. Prove the converses of (a) and (b).
   *Hint:* while (a) and (b) follow easily from the valuative criteria, their converses require some comparison of valuations in different fields.

d. If $X$ is proper over $k$, and if $k$ is algebraically closed, show that $\Gamma(X, \OO_X) = k$.
   *Hint:* let $a \in \Gamma(X, \OO_X)$ with $a \not\in k$. Show that there is a valuation ring $R$ of $K/k$ with $a\inv \in \mfm_R$, then use (b) to get a contradiction.
:::
