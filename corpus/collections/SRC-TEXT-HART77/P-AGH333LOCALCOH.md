...
a. Show that $\Gamma_{\mfa}(\wait)$ (II, Ex. 5.6) is a left-exact functor from the category of $A$-modules to itself. We denote its right derived functors, calculated in $\Mod(A)$, by $H_{\mfa}^i(\wait)$.

b. Now let $X=\Spec A$, $Y=V(\mfa)$. Show that for any $A$-module $M$,
\[
