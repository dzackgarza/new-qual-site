...
Define the **homogeneous coordinate ring** $S(X)$ of $X$ for the given embedding to be $A[x_0, \ldots, x_r]/I$, where $I = \Gamma_*(\mci_X)$.
Recall that a scheme $X$ is **normal** if its local rings are integrally closed domains.
A closed subscheme $X \subseteq \PP^r_A$ is **projectively normal** for the given embedding if $S(X)$ is an integrally closed domain.
