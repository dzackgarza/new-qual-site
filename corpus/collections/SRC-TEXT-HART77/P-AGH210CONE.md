...
::: problem
Let $Y \subseteq \PP^n$ be a nonempty algebraic set, and let
$\theta: \AA^{n+1} \sm \ts{(0,\ldots,0)} \to \PP^n$
send the point with affine coordinates $(a_0,\ldots,a_n)$ to the point with homogeneous coordinates $\tv{a_0 : \cdots : a_n}$.
...
::: solution
**Part 1.**
The set $C(Y)$ is algebraic because it is of the form $C(Y) = V(I(Y))$.
...
**Part 2.**
An algebraic set is irreducible exactly when its ideal is prime, and by part 1 the two ideals coincide: $I(Y) = I(C(Y))$.
