...
a. Let $D=\sum n_i P_i$ be a divisor with support in $X_{\reg}$, i.e., all $P_i \in X_{\reg}$. Then define $\deg D=\sum n_i$. Let $\mcl(D)$ be the associated invertible sheaf on $X$, and show that
$$
\chi(\mcl(D))=\deg D+1-p_a .
$$

b. Show that any Cartier divisor on $X$ is the difference of two very ample Cartier divisors. Use (II, Ex. 7.5).
