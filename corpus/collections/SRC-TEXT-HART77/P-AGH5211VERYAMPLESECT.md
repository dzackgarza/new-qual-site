...
::: problem
Let $X$ be a ruled surface over the curve $C$, defined by a normalized bundle $\mathcal{E}$, and let $\mfe$ be the divisor on $C$ for which $\mathcal{L}(\mfe) \cong \bigwedge^2 \mathcal{E}$ (See 2.8.1).
Let $\mfb$ be any divisor on $C$.
