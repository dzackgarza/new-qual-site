---
schema: qual/card@1
id: P-AGH4310NOTEVERYSECANTMULTISECANT
kind: problem
title: A general $(n-2)$-plane spanned by points of $X$ meets $X$ in no further point
classification:
  areas:
  - algebraic-geometry
  topics:
  - Embeddings
  - Linear Systems
  - Curves
relations: []
review: draft
---

::: problem
Generalize the statement that "not every secant is a multisecant" as follows. If $X$ is a curve in $\PP^n$, not contained in any $\PP^{n-1}$, and if $\characteristic k=0$, show that for almost all choices of $n-1$ points $P_1, \ldots, P_{n-1}$ on $X$, the linear space $L^{n-2}$ spanned by the $P_i$ does not contain any further points of $X$.
:::
