...
Hint: For any invertible sheaf $\mcl$ on $X$, cover $X$ by open sets $U_i$ on which $\mcl$ is free, and fix isomorphisms $\varphi_i: \mco_{U_i} \iso \ro{\mcl}{U_i}$. Then on $U_i \intersect U_j$, we get an isomorphism $\varphi_i\inv \circ \varphi_j$ of $\mco_{U_i \intersect U_j}$ with itself. These isomorphisms give an element of $\check{H}^1(\mathfrak{U}, \mco_X^*)$. Now use (Ex. 4.4).
:::
