...
::: problem
a. Let $f: X \to Y$ be a finite morphism of noetherian schemes. For any quasi-coherent $\mco_Y$-module $\mcg$, $\sheafhom_Y(f_* \mco_X, \mcg)$ is a quasi-coherent $f_* \mco_X$-module, hence corresponds to a quasi-coherent $\mco_X$-module, which we call $f^{!} \mcg$ (II, Ex. 5.17e).
...
\]
Hint: First construct a map $\Ext_X^i(\mcf, f^{!} \mcg) \to \Ext_Y^i(f_* \mcf, f_* f^{!} \mcg)$. Then compose with a suitable map from $f_* f^{!} \mcg$ to $\mcg$.
...
Hints: First do $i=0$. Then do $\mcf=\mco_X$, using (Ex. 4.1). Then do $\mcf$ locally free. Do the general case by induction on $i$, writing $\mcf$ as a quotient of a locally free sheaf.
:::
