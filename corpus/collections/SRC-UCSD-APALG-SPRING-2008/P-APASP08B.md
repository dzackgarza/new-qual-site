...
Proof: the Fourier transform is the isomorphism $\mathbb{C}[G] \cong \bigoplus_\rho \operatorname{End}(V_\rho)$.

<1>2. Under this isomorphism, $\hat f$ is the tuple $(\hat f(\rho))_\rho$ of matrices $\hat f(\rho) = \sum_{g} f(g)\rho(g)$.
Proof: definition of the Fourier transform.

<1>3. If $f$ is nilpotent, then $f^n = 0$ for some $n$, so $\hat f(\rho)^n = \widehat{f^n}(\rho) = 0$ for each $\rho$.
Proof: the Fourier transform is a ring homomorphism, so $\widehat{f^n} = \hat f^n$.
...
Proof: $\mathbb{C}[G]$ is semisimple (Maschke's theorem), so it is a direct sum of matrix algebras, which have no nonzero nilpotent ideals; in fact a direct sum of matrix algebras has no nonzero nilpotent elements at all.

<1>6. Hence $f = 0$, so $\hat f = 0$.
Proof: <1>5 (the only nilpotent element of a semisimple algebra is $0$).
