...
<2>2. A neighborhood basis of $\infty$ in $S_\Omega^*$ consists of sets of the form $(S_\Omega \setminus [0, \alpha]) \cup \{\infty\} = (\alpha, \Omega) \cup \{\infty\}$ for $\alpha < \Omega$ (by <1>2). <2>3. Under $f$, the image of $(\alpha, \Omega) \cup \{\infty\}$ is $(\alpha, \Omega) \cup \{\Omega\} = (\alpha, \Omega]$, which is precisely the standard basic open neighborhood of $\Omega$ in $\overline{S}_\Omega$.
<2>4. Therefore $f$ is an open continuous bijection, so $f$ is a homeomorphism.
Q.E.D.
:::
