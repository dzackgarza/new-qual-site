...
\]
::: {.proof}
properties of topological closure.
:::
...
::: {.proof}
definition of $U_n$ in <1>1. <2>7. Since $x \in V_0 \subseteq A$, we have $x \in A$.
:::
Thus $\overline{A} = A$, so $A$ is closed in $X_0$.
::: {.proof}
<2>3 through <2>6. <2>8. Since $X_0$ is a connected component of $X$, $X_0$ is connected.
:::
The only non-empty clopen subset of a connected space is the space itself, so $A = X_0$.
::: {.proof}
connectedness of $X_0$.
:::
