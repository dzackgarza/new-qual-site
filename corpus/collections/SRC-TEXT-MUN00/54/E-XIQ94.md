...
Proof: the lift on a rectangle is determined by the lift on one of its edges (by the unique lifting property), and to be well-defined and continuous, the lift on each new rectangle must match the previously defined lifts on the shared boundary.
...
Proof: this ensures the lift is well-defined and continuous across the whole square.
...
Proof: this is the order used in the proof.
