...
::: {.proof}
the inverse is $h^{-1}(x_1, \ldots, x_n) = ((x_1, \ldots, x_{n-1}), x_n)$.
:::

<1>3. $h$ is continuous.
::: {.proof}
a map into a product is continuous iff each coordinate is continuous; the coordinates of $h$ are the projections, which are continuous.
:::

<1>4. $h^{-1}$ is continuous.
::: {.proof}
similarly, the coordinates of $h^{-1}$ are projections (and the tuple of the first $n-1$ projections), all continuous.
:::
