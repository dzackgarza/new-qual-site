...
Proof: definition of the product topology.
<2>2. Every such basic open set meets $\RR^\infty$.
Proof: given finitely many non-$\RR$ factors $U_{i_1}, \ldots, U_{i_k}$, choose $x_{i_j} \in U_{i_j}$ and set all other coordinates to $0$; this gives an eventually-zero sequence in the basic open set.
<2>3. Hence $\RR^\infty$ is dense in $\RR^\omega$.
Proof: <2>2 shows every nonempty basic open set meets $\RR^\infty$.
