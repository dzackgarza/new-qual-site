...
::: {.proof}
the universal property of $\beta(X)$ (every continuous map from $X$ to a compact Hausdorff space extends to $\beta(X)$).
:::
...
::: {.proof}
<1>2 and <1>3 (the image is closed and contains the dense subset $X$).
:::

<1>5. $g$ is closed: a continuous map from a compact space to a Hausdorff space is closed.
::: {.proof}
<1>2 (compactness of $\beta(X)$ and Hausdorffness of $Y$).
:::
