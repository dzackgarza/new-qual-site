...
Proof: set-theoretic distribution of intersections over unions.
<2>2. **Closed sets in $X$:**
A subset $F \subseteq X$ is closed in $X$ if and only if $F \cap X_i$ is closed in $X_i$ for all $i \ge 1$.
Proof: complement $X \setminus F$ is open $\iff (X \setminus F) \cap X_i = X_i \setminus (F \cap X_i)$ is open in $X_i$.
...
Proof: transitivity of closed subspaces.
...
Proof: characterization of continuity via preimages of open sets.
...
Proof: $T_1$ property from normality of $X_i$.
...
Proof: Tietze Extension Theorem on normal spaces.
