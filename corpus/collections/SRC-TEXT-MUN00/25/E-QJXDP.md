...
::: {.proof}
definition of the infinite broom in Munkres Figure 25.1. <2>2. The space $X = \left(\bigcup_{n=1}^\infty L_n\right) \cup \{p\}$ is equipped with the subspace topology from $\mathbb{R}^2$.
:::
::: {.proof}
definition of $X$.
:::
...
<1>3. Show that $X$ is **weakly locally connected** at $p$: <2>1. A space $X$ is weakly locally connected at $p$ if every neighborhood $U$ of $p$ contains a connected subspace $C$ of $X$ such that $p \in \operatorname{int}(C)$ (or $C$ is a connected neighborhood of $p$).
::: {.proof}
Munkres §25 definition of weak local connectedness.
:::
<2>2. The space $C = X$ is connected by <1>1.
::: {.proof}
<1>1. <2>3. $X$ is an open neighborhood of $p$ in $X$, so $p \in \operatorname{int}(X) = X$.
:::
::: {.proof}
