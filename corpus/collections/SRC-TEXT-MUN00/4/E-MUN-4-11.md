...
Proof: such an integer $n$ exists (the floor of $m/2$).
<2>2. Then $2n < m < 2n + 2$, so $m = 2n + 1$.
Proof: $m$ is an integer strictly between the consecutive even integers $2n$ and $2n + 2$, so it equals the unique odd integer $2n + 1$ between them.
...
Proof: it has the form $2n + 1$.
<2>3. $p^n$ is odd for all $n \in \ZZ_+$.
Proof: induction on $n$ using <1>2.2 (product of two odds is odd).
...
Proof: such an $n$ exists since $a$ is rational and positive.
<2>2. $m$ and $n$ are not both even.
Proof: if both were even, then $m/2$ and $n/2$ are positive integers with $(n/2) a = m/2 \in \ZZ_+$, contradicting the minimality of $n$.

<1>4. (d) $\sqrt 2$ is irrational.
<2>1. Suppose $\sqrt 2 = m/n$ with $m, n \in \ZZ_+$ not both even.
Proof: by (c), applied to $a = \sqrt 2 > 0$.
<2>2. Then $m^2 = 2n^2$, so $m^2$ is even, hence $m$ is even.
Proof: the square of an odd number is odd (by (b)), so $m^2$ even forces $m$ even.
<2>3. Write $m = 2k$; then $4k^2 = 2n^2$, so $n^2 = 2k^2$ is even, hence $n$ is even.
Proof: same parity argument.
<2>4. Contradiction: $m$ and $n$ are both even, contradicting <1>4.1.
Proof: <1>4.2 and <1>4.3 force both even.

<1>5. Q.E.D.
Proof: <1>4.4 shows $\sqrt 2$ is irrational.
:::
