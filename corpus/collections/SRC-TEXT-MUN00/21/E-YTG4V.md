...
Proof: it is the composition of addition with the continuous map $y \mapsto -y$ (negation is continuous).

<1>2. Quotient $x/y = x \cdot (1/y)$ is continuous on $\mathbb{R} \times (\mathbb{R} - \{0\})$.
Proof: it is the composition of multiplication with the continuous reciprocal map (c).

<1>3. Q.E.D.
Proof: <1>4 (a), <1>5 (b), <1>2 (c), <1>1–<1>2 (d).
:::
