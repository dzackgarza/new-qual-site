...
Proof: if $\bigcap_{i=1}^{\infty} (X - U_i) = \varnothing$, then $\{U_i\}$ would be a countable subcover, contradicting <1>2.
...
Proof: Lindelöfness implies the countable intersection property of closed sets has nonempty intersection.
...
Proof: <1>7 and the fact that $\mathcal{U}$ covers $X$.
...
Proof: recall the structure of the proof.
...
Proof: a countable intersection of elements of the union may involve one element from each of countably many different collections in the chain, and there is no single collection in the chain containing all of them.
