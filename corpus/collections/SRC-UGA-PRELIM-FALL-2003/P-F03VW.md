...
Proof: $M^t$ is linear.
<2>2. If $w_1, w_2 \in W$, then $w_1 = M^t y_1$ and $w_2 = M^t y_2$ for some $y_1, y_2 \in \mathbb{R}^m$. Then $w_1 + w_2 = M^t(y_1 + y_2) \in W$ since $y_1 + y_2 \in \mathbb{R}^m$.
...
Proof: <2>1.
<2>5. Thus $x \cdot w = 0$ for all $w \in W$, so $x \in W^\perp$.
Proof: <2>3 and <2>4.
...
<2>4. Choose $y = Mx \in \mathbb{R}^m$. Then $(Mx) \cdot (Mx) = \|Mx\|^2 = 0$.
Proof: setting $y = Mx$ in <2>3.
<2>5. By positive definiteness of the Euclidean norm, $\|Mx\|^2 = 0 \implies Mx = 0$.
Proof: standard property of inner products.
<2>6. Hence $x \in V$.
Proof: definition of $V$.

<1>5. Conclusion:
$V = W^\perp = \{x \in \mathbb{R}^n : x \cdot w = 0 \text{ for all } w \in W\}$. Q.E.D.
Proof: <1>3 and <1>4.
:::
