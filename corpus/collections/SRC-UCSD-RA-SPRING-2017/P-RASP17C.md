...
Proof: the finite-rank truncation.

<1>2. $T_N$ is a finite-rank operator (its range is spanned by $u_1, \ldots, u_N$).
Proof: <1>1.

<1>3. $\|T - T_N\|_{op} \le \sup_{n > N} |\lambda_n|$.
Proof: $T - T_N$ is the diagonal operator with coefficients $\lambda_n$ for $n > N$ and $0$ for $n \le N$, so its operator norm is $\sup_{n > N} |\lambda_n|$ (by part 2).
...
Proof: <1>5 (a norm limit of finite-rank operators is compact).

<1>7. Q.E.D.
Proof: <1>4 (1), <1>2 (2), <1>6 (3).
:::
