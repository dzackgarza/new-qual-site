...
Proof: change of variables $y = A_t x$, with Jacobian $|\det A_t| = 1$.
...
Proof: continuous functions with compact support are uniformly continuous.
...
Proof: <1>2 and the dominated convergence theorem (the support is bounded).

**Part 3.**

<1>1. $C_c(\mathbb{R}^2)$ is dense in $L^2(\mathbb{R}^2)$.
Proof: standard density result.
...
Proof: triangle inequality and part 1 (isometry).
