...
::: {.proof}
rearrange: $A^5 - 2A^4 - A^3 = A^3(A^2 - 2A - I) = 0$.
:::
...
::: {.proof}
the trace is rational, so the sum of the irrational eigenvalues must be rational, forcing equal multiplicities.
:::
...
::: {.proof}
<1>2 and <1>7 (the minimal polynomial divides $x^3(x^2 - 2x - 1)$, so the blocks for $1 \pm \sqrt{2}$ have size $1$).
:::
