...
Proof: vector-valued function definition.
...
\end{pmatrix}.
\]
Proof: definition of total derivative matrix representation with respect to standard bases.
...
\]
Proof: product and trigonometric differentiation rules.
...
\end{pmatrix}.
\]
Proof: assembling partial derivatives into $2 \times 2$ matrix.
...
- $\sin(0) = 0$,
- $0 \cdot \cos(0) = 0$.
Proof: trigonometric values $\cos(0) = 1, \sin(0) = 0$.
<2>2. Thus:
\[
...
\end{pmatrix}.
\]
Proof: substituting values from <2>1 into <2>3.
