...
::: {.proof}
<2>1. Rewrite trigonometric functions in terms of $\sin(x)$ and $\cos(x)$: $$\tan(x) + \cot(x) = \frac{\sin(x)}{\cos(x)} + \frac{\cos(x)}{\sin(x)} = \frac{\sin^2(x) + \cos^2(x)}{\sin(x)\cos(x)} = \frac{1}{\sin(x)\cos(x)}.$$ <2>2. Thus the integrand is $\frac{1}{\tan(x) + \cot(x)} = \sin(x)\cos(x)$.
:::
