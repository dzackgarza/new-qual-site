...
Proof: a positive harmonic function on a simply connected region has a harmonic conjugate (its harmonic conjugate is obtained by integrating the closed form $-\phi_y\, dx + \phi_x\, dy$).

<1>2. $F$ is nowhere zero on $G$.
Proof: $\operatorname{Re} F = \phi > 0$, so $F \neq 0$.

<1>3. Since $G$ is simply connected and $F$ is nowhere zero, $F$ has an analytic logarithm: $F = e^H$ for some analytic $H$ on $G$.
Proof: a nowhere-vanishing analytic function on a simply connected region has an analytic logarithm.

<1>4. Write $H = u + iv$ with $u, v$ harmonic (the real and imaginary parts of an analytic function are harmonic).
Proof: real and imaginary parts of an analytic function are harmonic.

<1>5. Then $\phi = \operatorname{Re} F = \operatorname{Re}(e^{u + iv}) = e^u \cos v$.
Proof: $e^{u+iv} = e^u(\cos v + i\sin v)$, so its real part is $e^u \cos v$.

<1>6. Replace $v$ by $v + \pi/2$ (equivalently, absorb a constant into $H$) to get $\phi = e^u \sin v$.
Proof: $\cos v = \sin(v + \pi/2)$, and $v + \pi/2$ is still harmonic.
