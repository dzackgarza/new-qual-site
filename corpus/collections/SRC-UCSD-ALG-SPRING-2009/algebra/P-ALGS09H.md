...
::: {.proof}
substitute $x = y^2$ into $x^2 = y^3$.
:::
<2>2. Hence $y = 0$ or $y = 1$.
::: {.proof}
solve $y^3(y-1) = 0$ over $\CC$.
:::
...
::: {.proof}
the Nullstellensatz (over algebraically closed $\CC$).
:::
<2>2. $I(\theset{(0,0)}) = (x, y)$ and $I(\theset{(1,1)}) = (x-1, y-1)$.
::: {.proof}
the ideal of a point $(a,b)$ is the maximal ideal $(x-a, y-b)$.
:::
<2>3. Hence $\operatorname{rad} I = (x,y) \cap (x-1, y-1)$.
::: {.proof}
$I(V(I)) = I(\theset{(0,0)}) \cap I(\theset{(1,1)})$.
:::
...
<1>4. Q.E.D.
::: {.proof}
<1>2.3 gives $\operatorname{rad} I = (x,y) \cap (x-1,y-1)$; <1>3.3 shows $I$ is not radical.
:::
:::
