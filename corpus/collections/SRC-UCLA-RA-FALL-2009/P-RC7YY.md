...
Then we have $$0 = \langle (S-\lambda I)x,(S-\lambda I)x\rangle = \langle Sx-\lambda x, Sx-\lambda x\rangle = ||Sx||^2+|\lambda|^2||x||^2-2\text{Re}(\lambda\langle x,Sx\rangle)$$ $$= (1+|\lambda|^2)||x||^2 - 2\text{Re}(\lambda\langle x,Sx\rangle).$$ Thus we have $$(1+|\lambda|^2)||x||^2 = 2\text{Re}(\lambda\langle x,Sx\rangle) \le 2|\lambda|\,|\langle x,Sx\rangle| \le 2|\lambda|\,||x||\,||Sx|| = 2|\lambda|\,||x||^2.$$ Since we are assuming $x\ne0$ this implies $(1+|\lambda|^2) \le 2|\lambda|$, which is impossible for $|\lambda|<1$.
Thus $S-\lambda I$ is injective and therefore invertible.
$\square$
