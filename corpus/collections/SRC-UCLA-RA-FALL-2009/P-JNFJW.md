...
\]
::: {.proof}
chain rule and path integration of holomorphic functions.
:::
...
\]
::: {.proof}
real part commutes with integration over real intervals.
:::
...
\]
::: {.proof}
integral of a strictly positive continuous function is strictly positive.
:::
<2>5. Therefore $\operatorname{Re}\left(\frac{f(z_2) - f(z_1)}{z_2 - z_1}\right) \neq 0$, which implies $f(z_2) - f(z_1) \neq 0$, so $f(z_1) \neq f(z_2)$.
Thus $f$ is one-to-one.
::: {.proof}
non-zero real part implies non-zero complex number.
:::
