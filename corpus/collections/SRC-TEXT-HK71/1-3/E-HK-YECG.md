...
Proof: the zero matrix sends every vector to $0$.
...
Proof: $A$ is nonzero but singular, so its rank is $1$ (a nonzero $2 \times 2$ matrix of determinant $0$ has rank $1$).

<1>2. Hence the solution space $\{X : AX = 0\}$ is $1$-dimensional.
Proof: by rank–nullity, $\dim \ker A = 2 - \operatorname{rank}(A) = 1$.

<1>3. Let $(x_1^0, x_2^0)$ be any nonzero solution.
Proof: such a solution exists since $\ker A \neq 0$.

<1>4. Then $(x_1, x_2)$ is a solution iff $(x_1, x_2) = y(x_1^0, x_2^0)$ for some scalar $y$.
Proof: the solution space is $1$-dimensional and spanned by $(x_1^0, x_2^0)$.

<1>5. Q.E.D.
Proof: <1>1 (a), <1>3 (b), and <1>4 (c).
:::
