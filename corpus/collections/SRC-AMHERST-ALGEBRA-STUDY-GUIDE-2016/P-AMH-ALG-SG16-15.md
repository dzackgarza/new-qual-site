...
::: {.problem}
(February 2007) Fix an integern≥ 2, and write Sn for the permutation group on n letters. Let φ :Sn→G be a homomorphism, where G is a group of odd order. (I.e., G is a ﬁnite group with an odd number of elements.) (a) Prove that every transposition (i.e., 2-cycle) τ∈Sn is in kerφ. That is, prove that φ(τ) =e. (b) Prove that φ is the trivial homomorphism; i.e., prove that for all σ∈Sn, we have φ(σ) =e.
:::
