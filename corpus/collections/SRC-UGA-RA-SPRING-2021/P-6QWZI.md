...
::: {.proof}
the sets $\{g > M\}$ decrease to $\{g = \infty\}$ as $M \to \infty$, which has measure $0$ by <1>2; since $\mu(X) < \infty$, continuity from above gives $\mu\{g > M\} \to \mu\{g = \infty\} = 0$.
:::
...
<1>5. Q.E.D.
::: {.proof}
<1>4 is exactly the claim.
:::
(Only the a.e. convergence and finiteness of $\mu$ are used.)
:::
