...
::: {.proof}
tensor product of $F$-vector spaces.
:::
...
::: {.proof}
<1>1 (the elements with $i = 0$ or $j = 0$ survive).
:::
...
::: {.proof}
Nakayama's lemma (the minimal number of generators equals the dimension of the quotient by the maximal ideal $(x)$).
:::
...
::: {.proof}
<1>2 (a) (the action is $x \cdot (a \otimes b) = (xa) \otimes (xb)$).
:::
...
::: {.proof}
standard computation of the Kronecker product of nilpotent Jordan blocks (the largest block has size $m$, appearing $n - m + 1$ times, and each smaller size $k < m$ appears twice).
:::
