...
Proof: orbit–stabilizer theorem applied to the conjugation action.

<1>3. $Z(G) \subseteq C_G(g)$.
Proof: every element of the center commutes with $g$, hence centralizes $g$.

<1>4. Hence $[G : C_G(g)] \le [G : Z(G)] = n$.
Proof: $C_G(g) \supseteq Z(G)$, so the index of $C_G(g)$ is at most the index of $Z(G)$.
