...
- Since ideals are closed under sums, $H\da {1\over G} \da \sum g_k \in I$.
  But $H$ is a unit: by definition $HG = \id$, so $H$ has a multiplicative inverse $G$ in $R$, hence $H$ is a unit.
...
- Define $\mfm_{x_0} \da \ts{f\in R \st f(x_0) = 0}$, which is an ideal: it is the kernel of the evaluation map $\operatorname{ev}_{x_0}: R \to \RR$, $f \mapsto f(x_0)$, which is a ring homomorphism (evaluation at a point preserves sums and products), and the kernel of a ring homomorphism is an ideal.
