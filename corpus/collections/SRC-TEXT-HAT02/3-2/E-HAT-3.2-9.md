...
Proof: $\operatorname{Ext}$ of a free abelian group vanishes.
...
Proof: $\operatorname{Hom}(F, \mathbb{Z}_p) \cong \operatorname{Hom}(F, \mathbb{Z}) \otimes \mathbb{Z}_p$ for free $F$, and $H^n(X; \mathbb{Z}) \cong \operatorname{Hom}(H_n(X), \mathbb{Z})$ (since $\operatorname{Ext}(H_{n-1}, \mathbb{Z}) = 0$ by freeness).
...
Proof: the cup product is natural with respect to coefficient maps, and the reduction map $\mathbb{Z} \to \mathbb{Z}_p$ induces a ring homomorphism $H^*(X; \mathbb{Z}) \to H^*(X; \mathbb{Z}_p)$.
