...
:::
<2>2. Hence $b = 1$ in $H_1(Y)$ (since $\varepsilon = \pm 1$).
::: {.proof}
<2>1. <2>3. Therefore $H_1(Y) = \langle a, b, c \mid b = 1\rangle = \ZZ^2$ (free abelian on $a$ and $c$).
:::
::: {.proof}
<2>2.
:::
...
::: {.proof}
<1>2 and <1>4; one is torsion-free, the other has a $\ZZ/2$ summand.
:::

<1>6. Hence $\pi_1(Y) \not\cong \pi_1(Z)$.
::: {.proof}
isomorphic groups have isomorphic abelianizations, but <1>5 shows the abelianizations differ.
:::
