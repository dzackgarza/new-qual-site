...
- event: source-checked
  by: claude-opus-5
  date: 2026-09-16
  note: Moved the statement into a problem block and stated the lens space and its cellular chain complex from Hatcher, Algebraic Topology, Example 2.43 (pp. 144-145).
---

::: {.problem}
Given an integer $m > 1$ and integers $\ell_1, \cdots, \ell_n$ relatively prime to $m$, the lens space $L = L_m(\ell_1, \cdots, \ell_n)$ is the orbit space $S^{2n-1}/\mathbb{Z}_m$ of the unit sphere $S^{2n-1} \subset \mathbb{C}^n$ with the action of $\mathbb{Z}_m$ generated by the rotation $\rho(z_1, \cdots, z_n) = (e^{2\pi i \ell_1/m} z_1, \cdots, e^{2\pi i \ell_n/m} z_n)$.
It has a CW structure with one cell $e^k$ for each $k \leq 2n-1$, whose cellular chain complex is
$$
...
$$
with boundary maps alternately $0$ and multiplication by $m$.
