...
Proof: a free group is the fundamental group of a finite graph.

<1>2. A subgroup $H \le F$ of index $n$ corresponds to a connected $n$-fold covering space $\tilde\Gamma \to \Gamma$.
Proof: covering space theory (subgroups of $\pi_1$ correspond to connected covers).
...
Proof: each vertex/edge of $\Gamma$ has $n$ preimages.
...
Proof: <1>3 (the number of vertices and edges is fixed, so only finitely many combinatorial types exist).
...
Proof: every finitely generated group is a quotient of a finitely generated free group.

<1>7. Subgroups of $G$ of index $n$ correspond bijectively to subgroups $H \le F$ with $N \le H$ and $[F : H] = n$.
Proof: the correspondence theorem for the quotient $F \to F/N = G$.
