...
Proof: <1>3 (the spectral sequence collapses to the nerve).

<1>5. The nerve $N(\mathcal{U})$ is a simplicial complex with $n$ vertices, so its homology vanishes in degrees $\ge n$.
Proof: a simplicial complex with $n$ vertices has no simplices of dimension $\ge n$, so its homology is zero in degrees $\ge n$.
