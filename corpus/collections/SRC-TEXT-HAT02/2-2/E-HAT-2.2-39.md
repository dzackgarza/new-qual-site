...
::: {.solution}
<1>1. The preceding exercise (E-HAT-2.2-38) gives: from a commutative diagram with exact rows and every third vertical map an isomorphism, one obtains a long exact sequence
$$\cdots \to E_{n+1} \to B_n \to C_n \oplus D_n \to E_n \to B_{n-1} \to \cdots.$$
Proof: the algebraic lemma.
...
<1>4. Applying the algebraic lemma (<1>1) yields the relative Mayer–Vietoris sequence
$$\cdots \to H_{n+1}(X, Y) \to H_n(A \cap B) \to H_n(A) \oplus H_n(B) \to H_n(X, Y) \to \cdots$$
(equivalently, the long exact sequence of the pair $(X, Y)$).
Proof: <1>1 and <1>3.
