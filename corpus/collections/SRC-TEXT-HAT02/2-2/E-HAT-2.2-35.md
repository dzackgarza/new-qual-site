...
::: {.proof}
<1>4 and <1>5 (the map is injective since its kernel is the image of $H_2(\RR^3) = 0$).
:::
...
::: {.proof}
it is injective (<1>6), and its cokernel maps into $H_1(\RR^3) = 0$ (<1>5), so it is also surjective.
:::

<1>10. Hence $H_1(N) = H_1(X)$ is a direct summand of the free abelian group $H_1(S)$.
::: {.proof}
<1>9 and <1>7; a direct summand of a free abelian group is free abelian.
:::
