...
::: {.proof}
definition of the $\pi_1(Y)$-action on homotopy classes.
:::

<1>2. On $\pi_n(Y, y_0)$, the action of $\gamma$ is the automorphism $\beta_\gamma$ (conjugation by $\gamma$).
::: {.proof}
the standard $\pi_1$-action on higher homotopy groups.
:::

<1>3. The induced map $(\gamma f)_* : \pi_n(X) \to \pi_n(Y)$ equals $\beta_\gamma \circ f_*$.
::: {.proof}
the action of $\gamma$ on the map $f$ corresponds, on the induced homomorphism, to post-composing with the action $\beta_\gamma$ on $\pi_n(Y)$.
:::
...
::: {.proof}
the classification of maps into a $K(\pi,1)$.
:::
...
::: {.proof}
<1>4 (the action $\beta_\gamma$ on $\pi_1(K(\pi,1)) = \pi$ is conjugation by $\gamma$, an inner automorphism).
:::
