...
\]
and therefore $x=0$.
Substitution into either sphere equation gives
\[
1+y^2+z^2=4,
\]
hence $y^2+z^2=3$.
...
For each $i=1,2$, the circle $C\subset S_i$ separates the sphere $S_i$ into two open disks.
Let their closures be
\[
D_{i,+},D_{i,-}.
\]
Each $D_{i,\pm}$ is a closed disk whose boundary is $C$, and its interior is disjoint from the other three disk interiors.
Thus
\[
...
The attaching map of each $2$-cell restricts to a homeomorphism from its boundary circle onto $C$.
Hence its degree on the quotient
\[
C/C^0\cong S^1
\]
is $\pm1$.
Orient each $2$-cell so that this degree is $+1$.
The cellular boundary matrix is therefore
\[
