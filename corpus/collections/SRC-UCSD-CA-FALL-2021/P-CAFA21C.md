...
Proof: hypothesis that $a, b$ have finite order.
...
\]
Proof: definition of the order of an entire function.
<2>3. Combining with the bound from <1>1:
\[
|f(z)| \le 1 + 2\exp(|z|^{\rho + \varepsilon}) \le 3\exp(|z|^{\rho + \varepsilon}) \quad \text{for all } |z| \ge R.
\]
Proof: <1>1 and <2>2. <2>4. Taking logarithms yields $\limsup_{r \to \infty} \frac{\log \log M_f(r)}{\log r} \le \rho + \varepsilon$ for every $\varepsilon > 0$.
...
Proof: Cauchy's differentiation formula shows $f^{(k)}(0) = 0$ for all $k > d$.
