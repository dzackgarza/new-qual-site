---
schema: qual/card@1
id: P-UCLARA17F-04
kind: problem
title: UCLA analysis Fall 2017, Problem 4
classification:
  areas:
  - real-analysis
  topics: []
relations: []
review: draft
audit:
- event: source-checked
  by: gpt-5.6-sol
  date: 2026-09-13
  note: Transcribed from the official UCLA Analysis Qualifying Exam, Fall 2017, and reconciled with the retained UCLA Analysis Qualifying Exam Solutions compendium.
---

::: {.problem}
Problem 4. Consider the Banach space V = C([−1, 1]) of all real-valued continu-
ous functions on [−1, 1] equipped with the supremum norm defined as
                    kf k = sup{|f (x)| : x ∈ [−1, 1]}   for f ∈ V .
Let B = {f ∈ V : kf k ≤ 1} be the closed unit ball in V .
  Show that there exists a bounded linear functional Λ : V → R such that Λ(B) is
an open subset of R.
:::
