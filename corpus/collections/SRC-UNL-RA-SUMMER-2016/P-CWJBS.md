...
::: {.proof}
<1>6 bounds $d(x_n, x_m)$ by the tail $\sum_{k \ge n} d(x_k, x_{k+1})$ of a convergent series, which tends to $0$.
:::

<1>8. Q.E.D.: $(x_k)$ converges in $X$.
::: {.proof}
<1>6 and <1>7 show $(x_k)$ is Cauchy; completeness (<1>3) gives convergence.
:::
:::
