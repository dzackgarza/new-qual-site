...
::: {.solution}
<1>1. $f$ is continuous on $A$:
<2>1. Each $f_n$ is analytic on $A$, hence continuous on $A$.
...
<1>2. $f$ is analytic on $A$ by Morera’s Theorem:
<2>1. Let $T \subset A$ be any closed triangle whose interior is contained in $A$.
...
<1>3. $f_n' \to f'$ uniformly on compact subsets $K \subset A$:
<2>1. Let $K \subset A$ be an arbitrary compact subset.
Proof: setup.
<2>2. Since $A$ is open, $r_0 = \operatorname{dist}(K, \mathbb{C} \setminus A) > 0$. Choose $r = r_0 / 2 > 0$.
...
<2>5. Let $\varepsilon_n = \sup_{\zeta \in A} |f_n(\zeta) - f(\zeta)|$. By uniform convergence of $f_n \to f$ on $A$, $\lim_{n\to\infty} \varepsilon_n = 0$.
Proof: definition of uniform convergence.
<2>6. For all $\zeta \in \gamma_z$, $|\zeta - z| = r$. Applying the $ML$-inequality:
\[
...
<1>4. Conclusion:
$f$ is analytic on $A$, and $f_n' \to f'$ uniformly on compact subsets of $A$. Q.E.D.
Proof: <1>2 and <1>3.
:::
