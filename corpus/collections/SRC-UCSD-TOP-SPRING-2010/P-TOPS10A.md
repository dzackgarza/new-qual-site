...
Proof: take an arbitrary map.

<1>2. $f$ lifts to a map $\tilde f: M \to S^{n+1}$.
Proof: $M$ is simply connected, so $f_*(\pi_1(M)) = 0 \subseteq \pi_1(\RP^{n+1}) = \ZZ/2$; by the lifting criterion, $f$ lifts to the universal cover $S^{n+1}$ of $\RP^{n+1}$.

<1>3. $\tilde f$ is nullhomotopic.
<2>1. $S^{n+1}$ is $n$-connected: $\pi_i(S^{n+1}) = 0$ for all $i \le n$.
Proof: standard fact about spheres.
<2>2. $M$ is an $n$-dimensional CW complex.
Proof: hypothesis.
<2>3. Hence any map $M \to S^{n+1}$ is nullhomotopic.
Proof: by cellular approximation, $\tilde f$ is homotopic to a map into the $n$-skeleton of $S^{n+1}$, which is a single point (since $S^{n+1}$ has no cells in dimensions $1, \ldots, n$); equivalently, obstruction theory: the obstructions to nullhomotopy live in $H^i(M; \pi_i(S^{n+1})) = 0$ for $i \le n$.
