...
Proof: tensoring torsion modules with $\mathbb{Q}$ gives 0. <2>4. Thus $\dim_{\mathbb{Q}} H_i(X; \mathbb{Q}) = b_i$, and:
\[
\chi(X) = \sum_i (-1)^i b_i.
\]
...
Proof: definition of $t_i$ in <1>1. <2>4. For any finitely generated abelian group $A \cong \mathbb{Z}^b \oplus T$, $\operatorname{Tor}_1(A, \mathbb{Z}_2) \cong \operatorname{Tor}_1(T, \mathbb{Z}_2) \cong T \otimes \mathbb{Z}_2$.
...
\]
Proof: <2>2. <2>4. Therefore $\chi_2(X) = \sum_i (-1)^i b_i = \chi(X)$.
