...
<1>2. Compute $\chi(X)$ via the Universal Coefficient Theorem over $\mathbb{Q}$: <2>1. By the Universal Coefficient Theorem for homology:
\[
...
::: {.proof}
tensoring torsion modules with $\mathbb{Q}$ gives 0. <2>4. Thus $\dim_{\mathbb{Q}} H_i(X; \mathbb{Q}) = b_i$, and:
:::
\[
\chi(X) = \sum_i (-1)^i b_i.
\]
::: {.proof}
<2>1, <2>2, <2>3, and definition of $\chi(X)$.
:::

<1>3. Compute $\chi_2(X)$ via the Universal Coefficient Theorem over $\mathbb{Z}_2$: <2>1. By the Universal Coefficient Theorem with coefficients in $\mathbb{Z}_2$:
\[
...
::: {.proof}
definition of $t_i$ in <1>1. <2>4. For any finitely generated abelian group $A \cong \mathbb{Z}^b \oplus T$, $\operatorname{Tor}_1(A, \mathbb{Z}_2) \cong \operatorname{Tor}_1(T, \mathbb{Z}_2) \cong T \otimes \mathbb{Z}_2$.
:::
::: {.proof}
...
::: {.proof}
<2>4 applied to $A = H_{i-1}(X; \mathbb{Z})$.
:::
...
<1>4. Show $\chi(X) = \chi_2(X)$: <2>1. Substitute the dimension formula into the definition of $\chi_2(X)$:
\[
...
\]
::: {.proof}
<2>2. <2>4. Therefore $\chi_2(X) = \sum_i (-1)^i b_i = \chi(X)$.
:::
::: {.proof}
<2>1, <2>3, and <1>2.
:::
