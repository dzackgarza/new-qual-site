...
<1>1. Recall the Courant--Fischer formula
\[
\lambda_k(A)=\min_{\substack{S\subseteq\mathbb C^n\\ \dim S=n-k+1}}
\ \max_{0\ne x\in S}\frac{x^HAx}{x^Hx}.
\]
::: {.proof}
This is the min--max characterization of the $k$-th largest eigenvalue of a Hermitian matrix.
:::

<1>2. Fix $1\le k\le n-2$ and choose a subspace $S\subseteq\mathbb C^n$ with
\[
\dim S=n-k+1
\]
such that
...
\dim T=n-k-1=n-(k+2)+1.
\]
For every nonzero $x\in T$,
\[
\frac{x^HCx}{x^Hx}=rac{x^HAx}{x^Hx}\le \lambda_k(A).
\]
::: {.proof}
Because $T\subseteq K=\ker B^H$, we have $Cx=Ax$ for every $x\in T$. Since $T\subseteq S$, the Rayleigh quotient of $A$ on $T$ is bounded above by its maximum on $S$, namely $\lambda_k(A)$.
:::
...
\]
::: {.proof}
Apply Courant--Fischer to $C$ at index $k+2$:
\[
\lambda_{k+2}(C)
=\min_{\substack{U\subseteq\mathbb C^n\\ \dim U=n-k-1}}
\ \max_{0\ne x\in U}\frac{x^HCx}{x^Hx}.
\]
Using the particular subspace $U=T$ from <1>3 gives
\[
\lambda_{k+2}(C)
\le \max_{0\ne x\in T}\frac{x^HCx}{x^Hx}
\le \lambda_k(A).
\]
This holds for every $1\le k\le n-2$.
:::
:::
