...
\]
denote the Rayleigh quotient of a Hermitian matrix $M$ on nonzero vectors $x$.

<1>1. Let
\[
K:=\ker B^H\subseteq \mathbb C^n.
\]
Then $\operatorname{codim}K\le 2$, and for every $x\in K$,
\[
Cx=Ax.
\]
::: {.proof}
Since $B$ has rank $2$, so does $B^H$. By rank-nullity,
\[
\dim K=n-2.
\]
For $x\in K$ we have $B^Hx=0$, and hence
\[
Cx=(A+BB^H)x=Ax+B(B^Hx)=Ax.
\]
:::

<1>2. For each $1\le k\le n-2$,
\[
\lambda_{k+2}(C)\le \lambda_k(A).
\]
::: {.proof}
We use the Courant--Fischer min--max formula in the form
\[
\lambda_j(M)
=\min_{\substack{L\subseteq\mathbb C^n\\ \dim L=n-j+1}}
\ \max_{0\ne x\in L} R_M(x)
\]
for a Hermitian matrix $M$ whose eigenvalues are ordered decreasingly.

Fix $k\le n-2$. Let $L\subseteq\mathbb C^n$ be any subspace with
\[
\dim L=n-k+1.
\]
Because $\dim K=n-2$, the dimension formula gives
\[
\dim(L\cap K)
\ge \dim L+\dim K-n
=(n-k+1)+(n-2)-n
=n-k-1.
\]
Choose a subspace $L'\subseteq L\cap K$ of dimension exactly $n-k-1$.
Since $C$ and $A$ agree on $K$, for every nonzero $x\in L'$,
\[
R_C(x)=R_A(x).
\]
Therefore
...
\le \max_{0\ne x\in L}R_A(x).
\]
Now minimize the left-hand side over all $(n-k-1)$-dimensional subspaces and the right-hand side over all $(n-k+1)$-dimensional subspaces. Courant--Fischer yields
\[
\lambda_{k+2}(C)
\le \lambda_k(A).
\]
