...
\]
for a Hermitian matrix $M$ and nonzero $x\in\mathbb C^n$.

<1>1. The Courant--Fischer theorem gives
\[
\lambda_j(M)=\min_{\substack{L\subseteq\mathbb C^n\\ \dim L=n-j+1}}
\ \max_{0\ne x\in L} R_M(x).
\]
::: {.proof}
This is the standard min--max characterization of the eigenvalues of a Hermitian matrix, ordered decreasingly.
:::

<1>2. Since $\operatorname{rank}B=2$, the subspace
\[
K:=\ker(B^H)
\]
has dimension $n-2$.
...
\[
\dim K=n-\operatorname{rank}(B^H)=n-2.
\]
If $x\in K$, then $B^Hx=0$, hence
\[
BB^Hx=0,
\]
so from $C=A+BB^H$ we get $Cx=Ax$.
:::
...
<1>4. Choose any subspace
\[
L'\subseteq L\cap K
\]
of dimension $n-k-1$. Then
\[
\max_{0\ne x\in L'}R_C(x)
=
\max_{0\ne x\in L'}R_A(x)
...
\]
::: {.proof}
Because $L'\subseteq K$, <1>2 gives $Cx=Ax$ for every $x\in L'$, hence the Rayleigh quotients agree on $L'$. Since $L'\subseteq L$, the maximum of $R_A$ over $L'$ is at most the maximum over $L$.
:::
...
\]
For each $(n-k+1)$-dimensional subspace $L$, <1>3--<1>4 produce an $(n-k-1)$-dimensional subspace $L'\subseteq L\cap K$ satisfying
\[
\max_{0\ne x\in L'}R_C(x)
\le
\max_{0\ne x\in L}R_A(x).
...
\max_{0\ne x\in L}R_A(x).
\]
Taking the minimum over all $L$ with $\dim L=n-k+1$ yields
\[
\lambda_{k+2}(C)
\le
...
=\lambda_k(A).
\]
This proves the claim for every $1\le k\le n-2$.
:::
:::
