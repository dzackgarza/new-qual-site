...
Proof: definition of linear combination.
...
\]
Proof: linearity of differentiation and harmonicity of each $f_k$.
...
Proof: linear combinations of continuous functions are continuous.
...
Thus $w \in \mathbb{D}$.
Proof: $u(w) > 0 \ge \sup_{z\in\partial\mathbb{D}} u(z)$.
...
Proof: Strong Maximum Principle for harmonic functions.
...
Proof: boundary value of a continuous constant function.
<2>6. This contradicts the hypothesis that $u < 0$ (or $u \le 0$) on $\partial\mathbb{D}$.
Proof: $C > 0$ contradicts $C \le 0$.
