...
\]
::: {.proof}
<2>3 and <2>4. <2>6. Since $\chi$ is irreducible, $\langle \chi, \chi \rangle = 1$, so $\langle \chi^*, \chi^* \rangle = 1$.
:::
::: {.proof}
<2>5 and irreducibility of $\chi$.
:::
...
::: {.proof}
<1>1 and <2>2. <2>4. Thus $A \cong B$ as $\mathbb{C}[G]$-modules.
:::
::: {.proof}
<2>1 and <2>3. <2>5. Consequently, there exists an invertible matrix $T \in \operatorname{GL}_n(\mathbb{C})$ such that $T^{-1}A(\sigma)T = B(\sigma)$ for all $\sigma \in G$.
:::
::: {.proof}
