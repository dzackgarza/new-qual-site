...
Proof: smooth approximation of continuous maps.
...
Proof: $\pi_1(S^4) = 0$ and relative smooth approximation.
...
G \pitchfork K.
\]
Proof: relative Thom Transversality Theorem.
...
\]
Proof: codimension formula for transverse preimages: $\operatorname{codim}_{D^2}(G^{-1}(K)) = \operatorname{codim}_{S^4}(K) = 4 - 1 = 3$, so $\dim(G^{-1}(K)) = 2 - 3 = -1$.
...
\]
Proof: empty preimage for dimension $< 0$.
...
Proof: definition of null-homotopy in $S^4 \setminus K$.
