---
title: Splitting fields and normal extensions
order: 20
problems:
  topics:
  - Splitting Fields
  - Field Theory
---

# Splitting fields and normal extensions

[[D-LZTAK]]

[[FD-TP2IZ]] [[FD-JJFZ3]]

[[FD-LHTRR]]

[[PR-OZYUC]]

[[D-XD5NG]]

[[PR-TZN4M]]

::: {.remark title="Normality is not transitive"}
A tower $M/L/K$ with $M/L$ and $L/K$ both normal need not have $M/K$ normal.
The standard example is $\QQ(2^{1/4})/\QQ(\sqrt 2)/\QQ$: each step is quadratic hence normal, but $\QQ(2^{1/4})/\QQ$ omits the complex fourth roots of $2$.

This is the failure that makes the Galois correspondence interesting: the intermediate fields fixed by *normal* subgroups are exactly the ones normal over the base, and the rest are not.
:::
