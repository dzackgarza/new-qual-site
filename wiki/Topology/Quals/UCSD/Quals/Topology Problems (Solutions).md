...
3. Let $\alpha: S^n \to S^n,~ \alpha(p) = -p$ be the antipodal map on $S^n$.
...
8. **REVISIT** Let $f,g : S^1 \rightarrow X$, $P = X \cup_f B^2 \cong X \coprod B^2 / \sim$, where $x \sim f(x)$, $Q = X \cup_g B^2$.
...
11. Let $X = S^1\times S^1$ and $\gamma$ a loop based at $x_0$.
...
2. Let $x_0$ be the image of $0$, show that there is an order 6 homeomorphism $f: T \to T$ fixing $x_0$.
...
1. Let $i: \mathbb{RP}^2 \to \mathbb{RP}^3$, induced by $S^2 \hookrightarrow S^3$ as the equator.
   Show that $i \not\simeq \text{const}$.
...
13. The symbol $\oplus \in \mathbb{R}^2$

14. $S^2 - \{p_i\}_{i=1}^4$

15. $T - \{p_0\}$

16. $S^2 / \mathbb{Z}_2$ via the antipodal map

17. $S^2/\mathbb{Z}_3$ via a $2\pi/3$ rotation about the $z$-axis.

18. $S_2 \cup \{(0,0,z) \mid -1 \leq z \leq 1 \}$
...
11. Show that the Mayer-Vietoris sequence is natural, i.e. If $X\mapsvia{f} Y$ where $f(A) \subset C$ and $f(B) \subset D$, then this commutes:
    $$
    \begin{CD}
...
6. $S_1 \vee S_1$ with two discs attached via $(ab)^3$ and $(ab)^6$

7. This identification space: ![assets/2018-02-11 16_28_51-290.pdf](assets/2018-02-11 16_28_51-290.pdf.png)

8. This identification space: ![assets/1518395440173](assets/1518395440173.png)

9. This identification space: ![assets/1518395458668](assets/1518395458668.png)

10. Describe a CW complex structure for the lens space $L(p, 1)$ and compute $\pi_1, H_*$ for it.

## Degree

1. Let $p(x) = \sum_i^na_ix^i$, view $p: \CC \union \infty \selfmap$ and determine its topological degree

2. Let $p(z) = \frac{\prod_i^n z-a_i}{\prod_j^m z-b_j}$ with all $a_i, b_j$ distinct.
...
5. Show that $H_*(X; \QQ) = H_*(X;\ZZ)\tensor \QQ$ $H^*(X; \ZZ) = \hom(H_*(X; \ZZ), \QQ)$

6. Construct a space $X$ such that $H_*(X; \ZZ) = (\ZZ, \ZZ_6, \ZZ_{12}, \ZZ \oplus \ZZ_4, 0 \cdots)$ Compute $H^*(X; \ZZ)$
...
10. Let $G$ be a topological group.
...
5. Let $R = \ZZ[x,y]$, and $M = R/(x-y), N = R/(x,y)$.
...
6. Let $\Lambda_*$ be the exterior algebra generated by the symbols $\theset{dx_i}^n$ over a field $k$.
...
7. Define $M$ as the group ring $R = \ZZ[\ZZ_2]$ with the action $(\cdot) \times -1$.
   Construct a free resolution of $M$ and compute $\tor_R^*(M, M)$.

8. Show $\tor_R^*(\cdot, \cdot)$ is symmetric in the following way: Given $M, N$, take free resolutions, view $M_* \into M$ as a chain map and tensor with $N_*$ to get a chain map$\psi: M_* \tensor_R N_* \into M \tensor_R N_*$.
...
9. Prove that for a SES $0\into A\into B\into C$, the group $\ext(C,A)$ classifies extensions of $C$ by $A$ up to isomorphism.
...
[[P-XGHXK]] [[P-XPDMQ]] [[P-WAI3F]] [[P-TIRTE]] [[P-3TOOU]] [[P-6OVVU]] [[P-XTPAP]] [[P-E53UO]]

## Fundamental Group

[[P-QNUFT]] [[P-3QUC7]] [[P-BBI5H]]

[[P-NESQN]]

[[P-6DZST]] [[P-DPYAI]] [[P-XWNF6]] [[P-OX7OF]] [[P-V4A37]] [[P-5WRVK]] [[P-LKYOC]]

## Group Actions

## Covering Spaces

[[P-AY6TZ]] [[P-MZZJH]] [[P-JZXST]] [[P-DKDKR]] [[P-LJTUV]] [[P-G6GOO]] [[P-O2J6S]] [[P-IXL2P]] [[P-63J4S]]

## Simplicial Homology

[[P-QFLQB]] [[P-CLHXQ]] [[P-557LL]] [[P-MA7SC]] [[P-ALVYE]]
