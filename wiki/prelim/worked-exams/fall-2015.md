...
\end{array}\right),
$$ 
where we note that $R_3 = 2R_1 + R_2$ but $R_1 \neq \lambda R_2$ and so the first two rows span the correct subspace. The RREF, which has the same rowspace, is $$\tilde A = \left(\begin{array}{rrrr}
1 & 0 & -2 & -3 \\
0 & 1 & 3 & 4 \\
0 & 0 & 0 & 0
\end{array}\right)$$
...
1. We first need the derivatives of $\arctan$, since
  ...
  $$
  - Since each $f_n$ is bounded, choose $M_n<\infty$ such that $\abs{f_n(x)}\leq M_n$ for all $x$.
  - Uniform convergence with $\varepsilon=1$ gives $N$ such that $n\geq N \implies \abs{f_n(x)-g(x)}<1$ for every $x$. In particular, $\abs{g(x)-f_N(x)}<1$, and hence
  $$
  ...
  for every $n\geq N$. Thus
  \[
  M=\max\theset{M_1,\ldots,M_{N-1},M_N+2}
  \]
  is a uniform bound for the whole sequence.
...
1. Note that $8=2^3$ is a prime power $p^n$, so we can get this as a quotient of a polynomial algebra. In particular, since $p=2$, we'll want to look at $\FF_2[t]$, and we'll want to quotient it by a polynomial of degree $n=3$ that is irreducible in the base field $\FF_2$.
  ...
  $$
    ...
    A cubic over a field is reducible if and only if it has a root in that field. The polynomial
  \[
  f(t)=t^3+t+1
  \]
  satisfies $f(0)=1$ and $f(1)=1$ in $\FF_2$, so it has no root in $\FF_2$ and is irreducible. Therefore
  $$
