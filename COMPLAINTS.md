...
### `P-WVWEP` omits data needed for its representation-theory questions

- **Object and need:** `P-WVWEP` asks for representations of $\GL_2(\FF_q)$, its one-dimensional representations, and simplicity of unspecified related groups.
...
### `P-ZCY6S` omits its character-theory setting, and `P-ZGEFJ` is a proof fragment

- **Object and need:** `P-ZCY6S` asks about character orthogonality without specifying finite groups or complex representations. `P-ZGEFJ` contains a completed proof body instead of posing the $IS\le A$ statement.
- **Observed evidence:** The standard orthogonality relations and multiplicity formula are for finite groups over $\CC$. The $IS$ theorem is recoverable exactly from its existing definitions.
- **Impact and owner:** Both cards can be repaired without mathematical ambiguity.
- **Repair:** State the finite-complex character setting on `P-ZCY6S`; restore a posed submodule theorem on `P-ZGEFJ` and attach the proof.
