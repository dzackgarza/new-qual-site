...
### `P-GAAX7` reverses the universal property of the derived subgroup

- **Object and need:** `P-GAAX7` attempts to prove that $G'=Z(G)$ for a nonabelian group of order $p^3$.
- **Observed evidence:** The stored proof states that if $N\trianglelefteq G$ and $G/N$ is abelian, then $N\le G'$. The correct universal property is the reverse inclusion: $G'\le N$. The subsequent step $G'\le Z(G)$ relies on the correct direction, not the displayed statement. The body is also an attempted proof stored as the problem itself.
- **Impact and owner:** The theorem is correct, but the public proof states a false characterization of the commutator subgroup. The owning file is `corpus/problems/Algebra/P-GAAX7.md`.
- **Uncertainty:** None.
- **Repair:** Restore the theorem as a posed problem and prove $|Z(G)|=p$, $G/Z(G)$ abelian, hence $G'\le Z(G)$; nonabelianness gives $G'\ne1$, so equality follows.
