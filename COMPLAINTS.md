...
### `E-AMD-Y3DIXRGP` has interrogative punctuation on an imperative exercise

- **Object and need:** `E-AMD-Y3DIXRGP` is an imperative proof/counterexample prompt and should read as a declarative instruction.
- **Observed evidence:** The exercise currently ends `Show that the stabilizer of an element need not be a normal subgroup?`.
- **Impact and owner:** This is a public copy-edit papercut on `corpus/problems/Algebra/E-AMD-Y3DIXRGP.md`; it does not alter the mathematics.
- **Uncertainty:** None; the punctuation is directly visible in the selected card, whose only collection appearance is `SRC-UNSORTED-ALGEBRA`.
- **Repair:** Replace the terminal question mark by a period while adding the reviewed counterexample solution.
