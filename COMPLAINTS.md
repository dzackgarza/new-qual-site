...
### Algebra P-UMKJ4 and P-USAQT: incomplete Smith proof and false UFD implication

- `P-UMKJ4` stores a long incomplete proof fragment as the problem body, depends on an external PDF/image, and leaves the key divisibility step in Smith normal form to that external source. The card should be replaced by a self-contained theorem/proof over a PID, including the termination/divisibility argument.
- `P-USAQT` asks to prove `Spec(R) subseteq MaxSpec(R) => R is a UFD` for an arbitrary ring. This is false: `Z/4Z` has all prime ideals maximal but is not an integral domain. The recoverable correct statement is that if `R` is an integral domain and every prime ideal is maximal, then `(0)` is maximal, hence `R` is a field and therefore a UFD.
