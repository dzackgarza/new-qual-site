...
### ag-notes migration omits mathematical questions present in the deployed source

**Assessment:** incomplete. Direct source comparison on 2026-09-12 establishes omitted mathematical questions. This is a counterexample to complete migration, not an exhaustive inventory of omissions.

**Source boundary:** the deployed HTML tree `/var/www/ag_notes/` on `zack@159.223.102.204` is distinct from `/var/www/Notes/Class_Notes/2022/Fall/Orals/`. The latter is the source named by the existing migration plan and image ledger. The deployed tree contains the pages below; a filename search of the later vault returned neither McKernan nor Stacks pages. Content was read directly from the deployed HTML through Pandoc, rather than inferred from a migration status.

| Source page and section | Source mathematics | Target comparison |
| --- | --- | --- |
| `0_Study Guides/Notes/Mckernan AG 1.html`, Lecture 12 | Prove existence and uniqueness of a rational normal curve through $n+3$ points in linear general position in $\mathbb P^n$. | [P-AGH434RATIONALNORMALCURVE](corpus/collections/SRC-TEXT-HART77/P-AGH434RATIONALNORMALCURVE.md) concerns projective normality, quadratic equations, and classification of nondegenerate curves of minimal degree. Its complete statement contains no interpolation problem through prescribed points. [T-DIVMAPPN](corpus/theory/Algebraic_Geometry/T-DIVMAPPN.md) constructs maps from linear systems; it also does not state this interpolation result. |
| Same page, Lecture 1 | Prove that a set of $d\leq 2n$ points in linear general position in $\mathbb A^n$ is defined by quadratic polynomials. | No corresponding problem was located. The Hartshorne rational-normal-curve problem concerns the ideal of a curve, not equations for this finite point configuration. |
| `0_Study Guides/500 Stacks and Moduli.html`, Definitions | Questions about the fppf topology, algebraic spaces, torsors, the Amitsur complex (spelled “Amtisur”), étale equivalence relations, and locally closed substacks. | No matching questions or definitions were located. [D-CRVMOD](corpus/theory/Algebraic_Geometry/D-CRVMOD.md) discusses coarse/fine moduli and twisting; [PR-SCHFOP](corpus/theory/Algebraic_Geometry/PR-SCHFOP.md) defines the functor of points. Reading these complete cards confirms that their related subject matter does not supply the listed questions. |

- **Searched:** all authored `corpus/`, `wiki/`, and `publications/` text for rational normal curves, general position, quadratic polynomials, the relevant dimension expressions, and the stack/descent terms above. Follow-up searches included Veronese, representability, Hilbert schemes, descent, and stack terminology. Related target cards were read as mathematics. Target baseline: `06827ff626085ce6b83767496d0d9f0b0bbec5c7`.
- **Found:** source questions with no located target counterparts; the nearest target statements have different mathematical conclusions.
- **Conclusion:** the evidence supports incomplete migration of the deployed site. Treating the later vault as the complete source does not account for these deployed pages.
- **Confidence:** high for this incompleteness finding.
- **Gaps:** other source pages, attachment contents, image-only mathematics, and every worked solution were not compared exhaustively. This finding does not measure the total missing content or certify the remaining migration.

**Reproducible source identity:** SHA-256 of the McKernan HTML is `faa0bb34318c166f766f3d43b9d94769c5782c5e00ae5242d4f8084a56c8749e`; the Stacks HTML is `cc6823542daab040c682c23b56e1a4a625a5c5786206fd2c08efe0363af6b526`.

**Owner and expected repair:** algebraic-geometry corpus curation. Preserve each omitted question in authored content and check the deployed HTML tree as a source in its own right. Correct source mathematical errors explicitly during authorship. Repair remains open in this entry; this assessment does not migrate the missing content.
