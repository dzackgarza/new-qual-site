...
### `P-MVETM` has variable and exponent errors in the perfect-field proof

- **Object and need:** `P-MVETM` proves that in characteristic $p$, if every irreducible polynomial is separable then Frobenius is surjective.
- **Observed evidence:** The body switches from $F$ to an undefined $K$, writes a malformed Bézout exponent, and does not explicitly use that a proper irreducible factor of $x^p-a$ has degree $\ell<p$, which is what gives $\gcd(\ell,p)=1$.
- **Impact and owner:** The intended theorem is correct, but the written proof is not reliable as-is. The owning file is `corpus/problems/Algebra/P-MVETM.md`.
- **Repair:** Work entirely over $F$, treat $a=0$ separately, take a proper irreducible factor of degree $1\le\ell<p$, and use Bézout on $p$ and $\ell$ to recover the unique root $\beta$ inside $F$.

### `P-MXATB` has incorrect powers, conjugation notation, and a cycle factorization

- **Object and need:** `P-MXATB` proves that a $p$-cycle and a transposition generate $S_p$ for prime $p$.
- **Observed evidence:** It says $\sigma^k$ is a $p$-cycle for $1\le k\le p$, but $\sigma^p=1$; it writes conjugation without the inverse on one side; and the displayed expression of an arbitrary cycle as a product of star transpositions is incorrect.
- **Impact and owner:** The theorem is correct, but several displayed steps are false. The owning file is `corpus/problems/Algebra/P-MXATB.md`.
- **Repair:** Label the $p$ points by $\ZZ/p$, use conjugates of the given transposition to obtain the edge-transpositions of a connected $p$-cycle graph (primality makes the step size generate $\ZZ/p$), and invoke the standard fact that transpositions along a connected graph generate the full symmetric group.
