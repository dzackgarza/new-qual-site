...
### Algebra P-T7W8G and P-TA426: missing Galois/characteristic hypotheses

- `P-T7W8G` argues with `char(F)=p` and Frobenius but never states that hypothesis; its stored proof also has malformed coefficient/exponent indexing in the final p-th-power reconstruction. The intended theorem is the characteristic-p perfectness criterion.
- `P-TA426` asks for the fundamental theorem of Galois theory for a merely finite extension `L/K`; the subgroup/intermediate-field correspondence in that form requires `L/K` to be finite Galois.
