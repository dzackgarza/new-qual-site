...
### `E-D62SD` has the Vandermonde determinant sign reversed

- **Object and need:** `E-D62SD` displays the Vandermonde matrix whose columns are $(1,\lambda_j,\ldots,\lambda_j^{k-1})^T$; its determinant must match that orientation.
- **Observed evidence:** The card claims $\det A=\prod_{i<j}(\lambda_i-\lambda_j)$. For $k=2$, however, the displayed matrix is $\begin{pmatrix}1&1\\ \lambda_1&\lambda_2\end{pmatrix}$, with determinant $\lambda_2-\lambda_1$. Thus the claimed sign is already false in the smallest nontrivial case. The standard Vandermonde formula for this orientation is $\prod_{i<j}(\lambda_j-\lambda_i)$.
- **Impact and owner:** The title and exercise formula in `corpus/problems/Algebra/E-D62SD.md` are false as written, so a proof cannot be attached without correcting both.
- **Uncertainty:** The sign error is decisive from the displayed $2\times2$ specialization. The card appears only in `SRC-UNSORTED-ALGEBRA`, which provides no external source text whose alternate orientation must be preserved.
- **Repair:** Replace each factor $(\lambda_i-\lambda_j)$ by $(\lambda_j-\lambda_i)$ in the title and statement, then attach a proof for the displayed matrix and remove this complaint after verification.
