...
### `E-AMD-V3WFKTM6` incorrectly asserts strict containment for every ring

- **Object and need:** `E-AMD-V3WFKTM6` should distinguish the universal inclusion $\maxspec(R)\subseteq\spec(R)$ from the existence of rings for which that inclusion is strict.
- **Observed evidence:** The card says `Show that $\maxspec(R) \subsetneq \spec(R)$`, with no hypothesis on $R$. For a field $k$, the only prime ideal is $(0)$ and it is maximal, so $\maxspec(k)=\spec(k)=\{(0)\}$. The related card `P-USAQT` already states the coherent exercise: prove inclusion, give one strict example, and one equality example.
- **Impact and owner:** The selected problem is false as written and therefore cannot receive a correct proof without repairing its statement and title. The owning file is `corpus/problems/Algebra/E-AMD-V3WFKTM6.md`.
- **Uncertainty:** The counterexample is decisive. `SRC-UNSORTED-ALGEBRA` is the card's only collection appearance and has no external provenance, so the exact historical source wording is unavailable; `P-USAQT` is corroborating local evidence for the likely intended formulation, not an external oracle.
- **Repair:** Under TODO §7 / issue #2, change the card to the universally true inclusion together with an explicit request for a strict example, then prove it. A field may be noted as an equality example if useful.
