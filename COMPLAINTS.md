...
### `P-ZNZ4X` omits the algebraically closed base-field hypothesis for Nullstellensatz

- **Object and need:** `corpus/problems/Algebra/P-ZNZ4X.md` asks to prove $I(Z(I))=\sqrt I$ without specifying the coefficient field.
- **Observed evidence:** The equality is Hilbert's Nullstellensatz for ideals in $k[x_1,\dots,x_n]$ when $k$ is algebraically closed. Over a non-algebraically-closed field it can fail; for example in $\RR[x]$, the ideal $(x^2+1)$ has empty real zero set, so $I(Z(I))=(1)$ while $\sqrt{(x^2+1)}=(x^2+1)$.
- **Impact and owner:** The card is false as written. The owning file is `corpus/problems/Algebra/P-ZNZ4X.md`.
- **Repair:** State $R=k[x_1,\dots,x_n]$ with $k$ algebraically closed before asserting $I(Z(I))=\sqrt I$.
