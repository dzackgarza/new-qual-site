...
### Spring 2011 Applied Algebra solutions were stored with literal newline escapes

- **Object and need:** `P-ALGS11E`, `P-ALGS11F`, `P-ALGS11G`, and `P-ALGS11H` in `SRC-UCSD-ALG-SPRING-2011`; authored solution sections must be actual Markdown blocks so the corpus parser recognizes them.
- **Observed evidence:** each card already contained a complete reviewed solution introduced earlier, but the entire solution tail was serialized with literal `\n` text instead of line breaks. Consequently `just unsolved-in SRC-UCSD-ALG-SPRING-2011` reported positions 5--8 as unsolved. The four cards were repaired in commits `bdd20392f`, `fecdef52e`, `4806cdb36`/`77204966e`, and `842c029cd`; `just unsolved-in` then returned no rows for the collection.
- **Impact and owner:** the malformed serialization made complete solutions invisible to the authoring parser and caused false unsolved-card work. The card contents owned the repair; the earlier writer/serialization path that emitted literal escapes was not identified.
- **Uncertainty:** the exact component that produced the literal escapes was not established. In `P-ALGS11G`, a first naive decode also split LaTeX commands beginning with `\n` such as `\ne` and `\nmid`; corrective commit `77204966e` restored those commands before the stream proceeded.
- **Repair:** the four cards now contain ordinary Markdown line breaks and parse as solved cards. Future repair of similarly malformed content must distinguish literal newline escapes from LaTeX commands rather than globally replacing every `\n` byte pair.
