...
### `P-KPEPM` allows nonprime $p$ in $\FF_p$

- **Object and need:** `P-KPEPM` asks for pairs $n,p\ge1$ such that $\SL_n(\FF_p)$ is solvable.
- **Observed evidence:** $\FF_p$ denotes a field with $p$ elements only when $p$ is prime (more generally finite fields have prime-power cardinality). The intended notation here is clearly prime $p$.
- **Impact and owner:** The parameter domain is malformed. The owning file is `corpus/problems/Algebra/P-KPEPM.md`.
- **Repair:** State explicitly that $p$ is prime; then the solvable cases are $n=1$ for all $p$, together with $(n,p)=(2,2),(2,3)$.

### `P-KXSW3` omits surjectivity onto the abelianization

- **Object and need:** `P-KXSW3` defines the composite $G\hookrightarrow H\twoheadrightarrow H/[H,H]$ and asks to prove $G=H$, but never assumes this map is surjective.
- **Observed evidence:** Without surjectivity the conclusion is false (take $G=1$ in any nontrivial finite $p$-group). The title indicates the intended hypothesis: $G$ generates $H/[H,H]$.
- **Impact and owner:** A necessary hypothesis is missing from the problem statement. The owning file is `corpus/problems/Algebra/P-KXSW3.md`.
- **Repair:** Add surjectivity, equivalently $G[H,H]=H$, then prove a proper $G$ would lie in a maximal subgroup $M$ with $[H,H]\subseteq M$, contradiction.
