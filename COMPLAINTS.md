...
### `E-AXWM3` uses undefined `$Q$` instead of the corpus rational-field notation

- **Object and need:** `E-AXWM3` should use the same explicit rational-field notation in its statement as in its title.
- **Observed evidence:** The title says `$\\mathbb{Q}$`, while the exercise body says `over $Q$`; no definition of a field named `$Q$` occurs on the card.
- **Impact and owner:** This is a public notation papercut in `corpus/problems/Algebra/E-AXWM3.md`; the intended base field is unambiguous from the title.
- **Uncertainty:** None material. The card appears only in `SRC-UNSORTED-ALGEBRA`, so there is no external provenance to preserve an alternative notation.
- **Repair:** Replace `$Q$` by `$\\mathbb{Q}$` while attaching the reviewed cyclotomic Galois-group proof.
