...
### Algebra solution-fragment defects in `P-W3PZY`, `P-WB2SR`, and `P-WBK5B`

- **Object and need:** These three unsolved Algebra cards have recoverable intended statements but malformed authored text.
- **Observed evidence:** `P-W3PZY` omits the characteristic-$p$ hypothesis in the body, writes the polynomial with the wrong variable, and contains malformed coefficient/exponent calculations. `P-WB2SR` reverses the ranges for the copy index and basis index and defines standard-coordinate vectors using a scalar $1$ as though $1\in V$. `P-WBK5B` writes `\Inn(S^n)` instead of `\Inn(S_n)`.
- **Impact and owner:** The underlying theorems are standard, but the current problem bodies cannot be accepted verbatim. The owning files are the three corresponding cards under `corpus/problems/Algebra/`.
- **Repair:** Restate each theorem precisely, then replace the fragments by complete proofs/constructions.
