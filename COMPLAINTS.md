...
### `E-AMD-VGBOZDXZ` has malformed `$p$`-group notation

- **Object and need:** `E-AMD-VGBOZDXZ` should state the standard conjugation-orbit theorem for a finite `$p$`-group.
- **Observed evidence:** The exercise currently reads `finite $p\\dash$group`, while its title correctly says `finite $p$-group`; the same malformed token was previously repaired on the separate card `E-AMD-TM3LMADH`.
- **Impact and owner:** The public statement is typographically corrupted. The owning file is `corpus/problems/Algebra/E-AMD-VGBOZDXZ.md`; its hint explicitly requests the standard conjugation action and orbit count.
- **Uncertainty:** Verified directly from this card. Its only appearance is in `SRC-UNSORTED-ALGEBRA`, which supplies no external provenance.
- **Repair:** Under TODO §7 / issue #2, replace the malformed phrase by `$p$-group` and prove the stated theorem on this card.
