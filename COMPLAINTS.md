...
### `P-SB67D` does not specify which $p$th root of unity is used

- **Object and need:** `P-SB67D` asks whether $\ZZ[t]/(t^p-1)\to\ZZ[w]$ is an isomorphism, assuming only $w^p=1$.
- **Observed evidence:** The answer depends on $w$: for $w=1$ the kernel is generated by the image of $t-1$, while for a primitive $p$th root $\zeta_p$ the kernel is generated by the image of the cyclotomic polynomial $\Phi_p(t)$.
- **Impact and owner:** The literal card has no unique answer. The title indicates the intended primitive cyclotomic specialization. The owning file is `corpus/problems/Algebra/P-SB67D.md`.
- **Repair:** State that $w=\zeta_p$ is a primitive $p$th root of unity, then identify the induced map and its nonzero kernel.
