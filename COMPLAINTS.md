...
### `P-FX46W` omits the coefficient-field hypothesis for Maschke and invariant inner products

- **Object and need:** `P-FX46W` asks why finite-group representations decompose into irreducibles and asks for an invariant inner product.
- **Observed evidence:** Complete reducibility is not true over an arbitrary coefficient field when the characteristic divides $|G|$, and a positive-definite invariant inner product is naturally a real/complex statement. The card specifies neither a field nor characteristic.
- **Impact and owner:** The public statement is underspecified and false under natural modular-representation interpretations. The owning file is `corpus/problems/Algebra/P-FX46W.md`.
- **Uncertainty:** The surrounding representation wiki and nearby cards use finite-dimensional complex representations for Maschke/character theory, so that is the locally supported intended setting.
- **Repair:** State the card for finite-dimensional complex representations of a finite group, prove complete reducibility by averaging a Hermitian inner product, and define irreducibility explicitly.
