...
### Algebra P-UE7LL and P-UFCQH: statement/notation defects

- `P-UE7LL` is a proof fragment for groups of order `p^2 q` but discusses the impossible cases `p=1` or `q=1` even though `p,q` are meant to be primes, and later writes `n_1` where `n_q` is intended. The theorem is recoverable as the standard statement for primes `p,q`.
- `P-UFCQH` asks about real matrices conjugate over `CC` but writes the target matrix space as `Mat(n cross N, R)`, mixing dimensions and an undefined `R`; the intended comparison is similarity in `M_n(CC)` versus `M_n(RR)` for real matrices.
