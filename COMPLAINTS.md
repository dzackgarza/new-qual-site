...
### Algebra P-TFKBW: cyclicity criterion reverses divisibility

`P-TFKBW` asks for cyclicity from having at most `n` elements whose orders are divisible by `n`. The standard theorem uses elements whose orders divide `n`, equivalently solutions of `x^n=e`, and requires the bound for every positive integer `n`. The literal wording fails even for cyclic groups (for example `C_6` has three elements whose orders are divisible by `2`). The card should be repaired to the standard order-dividing criterion.
