...
### `E-AMD-YITMZNBK` omits the nonzero hypothesis in the PID irreducible/maximal criterion

- **Object and need:** The standard PID criterion is for a nonzero element $x$: $x$ is irreducible if and only if the proper principal ideal $(x)$ is maximal.
- **Observed evidence:** The card says only `if $x\in R$ a PID`. If $R=k$ is a field and $x=0$, then $(0)$ is the unique maximal ideal of $k$, but $0$ is not irreducible. The sentence also omits the grammatical relation between $x\in R$ and `a PID`.
- **Impact and owner:** The exercise is false as written in the field case, so `corpus/problems/Algebra/E-AMD-YITMZNBK.md` requires a statement repair before a correct biconditional proof can be attached.
- **Uncertainty:** The counterexample is decisive. The card's only collection appearance is `SRC-UNSORTED-ALGEBRA`, so there is no external source text to recover; the standard theorem is corroborated by Stacks, Example 9.3.3, for the irreducible-to-maximal direction.
- **Repair:** State `Let $R$ be a PID and let $0\neq x\in R$` before the biconditional, then prove both directions and remove this complaint in the fixing commit.
