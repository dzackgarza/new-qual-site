...
### UCLA Fall 2007 Basic Exam Problem 9 overstates continuous dependence on all of $\mathbb R$

- **Object and need:** `P-UCLAB07F-09`, UCLA Basic Examination Fall 2007, Problem 9(b); the source asks for uniform convergence on all of $\mathbb R$ from convergence of initial data, bounded continuous $F$, and uniqueness.
- **Observed evidence:** take $F(y,x)=\tanh y$, $x_0=y_0=0$. The zero solution is unique, while the solution with $u_n(0)=1/n$ is $u_n(x)=\operatorname{arsinh}(e^x\sinh(1/n))$. Thus $u_n(0)\to0$ but, for every fixed $n$, $u_n(x)\to\infty$ as $x\to\infty$, so $\sup_{x\in\mathbb R}|u_n(x)|=\infty$.
- **Impact and owner:** part (b) is false as printed if “uniformly” means on the whole real line. The problem card owns the mathematical erratum while preserving the source wording.
- **Uncertainty:** none about the counterexample. The likely intended conclusion is uniform convergence on compact $x$-intervals under the usual continuous-dependence hypotheses.
- **Repair:** `P-UCLAB07F-09` preserves the source statement and now contains the explicit bounded smooth counterexample and compact-interval correction.
