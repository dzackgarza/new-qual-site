...
### `P-J5U34` omits separability in its quintic Galois-group criterion

- **Object and need:** `P-J5U34` asks for necessary and sufficient conditions for the Galois group of a degree-$5$ polynomial to have order divisible by $3`, over an arbitrary field.
- **Observed evidence:** In positive characteristic an irreducible quintic can be inseparable, so its splitting-field automorphism group need not act as a transitive subgroup of $S_5$ on five distinct roots. The intended permutation-group argument requires a separable quintic; the later request for an irreducible example strongly suggests that setting.
- **Impact and owner:** Without separability the standard transitive-subgroup criterion is not valid as stated. The owning file is `corpus/problems/Algebra/P-J5U34.md`.
- **Repair:** State the criterion for a separable quintic, and for the sharp classification assume irreducibility: then $3\mid |G|$ iff $G$ is $A_5$ or $S_5$.
