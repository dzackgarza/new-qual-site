...
`corpus/problems/Algebra/P-OLLG3.md` stores a worked solution fragment as the problem and its last displayed implication incorrectly writes the transformed right-hand side using an extra factor of $A$. The intended theorem is invariance of the solution set under simultaneous elementary row operations on $A$ and $b$.
